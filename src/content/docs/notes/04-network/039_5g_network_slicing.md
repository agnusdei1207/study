...
- 배경/필요성: 단일 공용 물리 네트워크 인프라에서는 대용량 미디어(eMBB), 저지연 자율주행/스마트팩토리 제어(URLLC), 대규모 센서 네트워크(mMTC) 등 상호 이질적인 서비스들이 동일한 무선·전송·코어 자원을 공유함에 따라 트래픽 버스트 발생 시 상호 간섭과 서비스 품질(SLA) 저하가 발생할 수 있고 전용망 구축 시 중복 인프라 비용이 소요되는 한계를 극복하기 위해, SDN/NFV 가상화 기술을 기반으로 무선(RAN), 백홀 전송(Transport), 코어(5GC) 전 구간의 자원을 서비스 SLA별로 논리 분할·격리하는 End-to-End 네트워크 슬라이싱(Network Slicing)을 도입하여 **단일 물리망 기반의 다중 테넌트 전용 가상망 구축과 결정론적 저지연 및 대역폭 보장**을 달성할 필요

#### 한줄 요약
- RAN-Transport-Core 전 구간을 가상화하여 서비스 요구사항별 독립 가상망을 제공해야 한다.
...
- **다계층 자원 격리(Isolation)**: 특정 슬라이스(eMBB) 트래픽이 급증해도 인접 슬라이스(URLLC) 간섭을 최소화
- **소프트웨어 정의 라이프사이클 관리**: CSMF/NSMF 오케스트레이터를 통한 **온디맨드 자동 프로비저닝 및 동적 확장**

#### 한줄 요약
- E2E 전 구간 제어, 하드/소프트 다계층 자원 격리, 자동 확장 생애주기 관리를 제공해야 한다.
...
#### 한줄 요약
- NSMF와 도메인별 NSSMF가 연동하여 무선, 전송, 코어 전 구간의 슬라이스 자원을 유기적으로 제어해야 한다.
...
```text
[E2E 네트워크 슬라이스 생성·보증 흐름] (진행 ①→⑤, ③ 격리 방식 결정 분기)
  │
  ├─ [CSMF] (① B2B 고객 대역폭·지연 SLA 요구 접수)
  │
  ├─ [NSMF] (② E2E 슬라이스 템플릿 생성 후 도메인별 NSSMF 하달)
  │
  ├─ [도메인 NSSMF] (③ RAN PRB 예약, Transport FlexE/SRv6 터널링, Core 전용 UPF/SMF 기동)
  │
  ├─ [NSSF] (④ 접속 단말 S-NSSAI 검증 후 전용 슬라이스·AMF 바인딩)
  │
  └─ [NWDAF] (⑤ 실시간 SLA 모니터링 기반 폐루프 동적 자원 보정)
```

분기 결과: ③단계의 자원 예약 방식에 따라 물리 자원을 고정 분할하는 하드 격리 경로와 가중치 기반 동적 분배를 수행하는 소프트 격리 경로로 분기되며, NWDAF 폐루프 분석을 통해 SLA 위반 위험 발생 시 동적 스케일링으로 보정하는 경로로 귀결된다.

#### 한줄 요약
- SLA 요구사항에 따라 하드 또는 소프트 슬라이싱을 적용하고 NWDAF 폐루프 모니터링을 통해 자원을 보증해야 한다.

## Ⅴ. 종류 및 비교

<details><summary>용어 설명</summary>

- **Hard Slicing vs Soft Slicing**: 물리 PRB/타임슬롯을 물리 고정 할당하는 격리(Hard)와 가중치 큐잉 기반 동적 분배(Soft).
...
| **자원 격리 수준** | 논리적 큐 우선순위 (전체 트래픽 폭주 시 침범)| **물리/논리적 자원 격리 (간섭 억제)** |
| **SLA 보장 수준** | 상대적 우선순위 (Best-Effort 기반 한계) | **결정론적 저지연 및 대역폭 보장** |
| **오케스트레이션** | 네트워크 장비별 CLI/QoS 정책 수동 설정 | **CSMF/NSMF 기반 온디맨드 자동 프로비저닝** |
| **주요 대표 용도** | 단순 웹/음성 트래픽 우선순위 차등 | **자율주행(V2X), 스마트 팩토리, 원격 의료, 특화망**|

#### 한줄 요약
- 단순 홉별 QoS 제어를 넘어 전 구간 자원 격리와 자동 오케스트레이션을 제공해야 한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 대용량 eMBB 트래픽 폭증 시 무선 구간 간섭으로 URLLC 1ms 지연 초과 | 무선 **`PRB 하드 파티셔닝(Hard Reservation)` 및 선점형 스케줄링** | 무선 간섭 방지 및 URLLC 1ms 지연 보증 |
| 슬라이스 내 급격한 트래픽 증가로 인한 가상 NF 과부하 및 SLA 위반 | **`NWDAF 연계 AI 폐루프(Closed-Loop) 오케스트레이션` 적용** | 부하 발생 전 선제적 자원 증설(Auto-scaling) |
| 공용 슬라이스 침해 시 인접 핵심 산업 슬라이스로의 횡적 침투 위협 | 슬라이스 간 **`IPsec/mTLS 격리 터널링` 및 독립 UDM/AUSF 인증** | 슬라이스 간 침해 전파 차단 및 보안성 강화 |
| 수백 개 슬라이스 운영 시 도메인 간 오케스트레이션 복잡도 증가 | **`3GPP 표준 NEST (Network Slice Template)` 및 자동화 파이프라인** | 배포 소요 시간 대폭 단축 |

#### 한줄 요약
- PRB 하드 파티셔닝, NWDAF 폐루프 제어, IPsec 기반 격리, NEST 표준 템플릿을 적용하여 운영 신뢰성을 확보해야 한다.

## Ⅶ. 결론

<details><summary>용어 설명</summary>

- **FlexE (Flexible Ethernet)**: 물리 이더넷 링크를 시분할 다중화(TDM)하여 지연 간섭 없이 독립적인 하드 파이프 슬라이스를 구성하는 전송 기술.

</details>

- B2B 기업 특화망, 자율주행 V2X, 국가 공공안전망(PS-LTE/5G) 및 원격 의료 등 미션 크리티컬 산업 서비스를 지탱하는 **5G 및 차세대 6G 통신의 핵심적인 비즈니스 인에이블러(Business Enabler) 기술**로 자리잡음.
- 향후 AI 기반 자율 제어 통신망으로 발전해 나가는 가운데, 실무 구축 시에는 **미션 크리티컬 워크로드의 PRB 하드 예약(Hard Slicing)**, **전송 구간 FlexE/SRv6 터널링**, **5GC NSSF/S-NSSAI 매핑**, **NWDAF(네트워크 데이터 분석) 연계 폐루프(Closed-Loop) 자동 스케일링**을 결합하여 엔드투엔드 SLA를 완성.

#### 한줄 요약
- 5G 네트워크 슬라이싱은 RAN-Transport-Core 전 구간을 가상화하여 서비스별 차등화된 SLA를 보장하는 차세대 핵심 통신 가상화 기술로 정착되어야 한다.
