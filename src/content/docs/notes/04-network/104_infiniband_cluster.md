...
- 배경/필요성: 수만 개의 GPU 가속기가 병렬로 결합되는 초대규모 슈퍼컴퓨팅(HPC) 및 프론티어급 거대언어모델(LLM) 사전 훈련 환경에서, 전통적인 이더넷 기반 패킷 교환 방식은 스토어 앤 포워드(Store-and-Forward) 지연, 버퍼 오버플로우로 인한 패킷 드롭 및 재전송으로 인해 심각한 테일 레이턴시(Tail Latency)와 GPU 유휴(Idle) 병목을 유발함에 따라, 링크 계층 크레딧 기반 흐름 제어(Credit-Based Flow Control), 나노초($\approx 100ns$)급 컷스루(Cut-Through) 스위칭 및 전용 HCA/서브넷 관리자(SM)를 결합한 인피니밴드(InfiniBand) 클러스터 패브릭을 도입하여 **패킷 손실을 구조적으로 방지하는 무손실 전송, 1:1 논블로킹(Non-Blocking) Fat-Tree 대역폭 및 서브 마이크로초 통신 지연**을 달성할 필요

#### 한줄 요약
- 하드웨어 크레딧 기반 무손실 흐름 제어, 컷스루 스위칭, 중앙 서브넷 관리자를 통해 높은 클러스터 성능을 보장한다.

## Ⅱ. 특징

<details><summary>용어 설명</summary>

- **Credit-Based Flow Control**: 수신단 버퍼에 여유 공간이 있을 때만 송신단에 토큰(Credit)을 부여하여 하드웨어 레벨에서 패킷 드롭을 선제적으로 방지하는 흐름 제어.
...
분기 결과: 크레딧 확보 여부가 송신과 대기를 가르며, 송신 전 수신 여유를 확인하는 대가로 손실·재전송 비용을 선제적으로 방지하는 대신 링크 전송률은 수신 버퍼 용량에 묶인다.

#### 한줄 요약
- 크레딧 확보 여부가 전송과 대기를 가르며, 송신 전에 수신 여유를 확인하는 대가로 손실과 재전송 비용이 최소화된다.
...
#### 한줄 요약
- HDR 200G, NDR 400G, XDR 800G로 진화하며 고밀도 PAM4 변조와 FEC 기술을 통해 대역폭을 대폭 확장한다.
...
- **TOP500**: 전 세계에서 가장 성능이 뛰어난 슈퍼컴퓨터 500대의 순위를 매겨 매년 두 차례 발표하는 글로벌 프로젝트.
- **OpenSM**: 인피니밴드 서브넷의 노드 탐색, 경로 계산 및 LFT 배포를 담당하는 오픈소스 서브넷 관리자 소프트웨어.

</details>

- 하드웨어 레벨의 무손실 보장과 저지연을 바탕으로 전 세계 TOP500 슈퍼컴퓨터와 최선단 AI 연구 클러스터를 지탱하는 **초고성능 컴퓨팅 및 대규모 AI 분산 가속 인터커넥트의 핵심 패브릭 표준**으로 정립.
- 스위치 인네트워크 컴퓨팅(SHARP) 및 NDR(400G)/XDR(800G) 광 인터커넥트로 진화하는 가운데, 실무 인피니밴드 클러스터 구축 시에는 **서브넷 제어 마비를 방지하는 Active-Standby Master OpenSM 이중화 및 서브초 하트비트 페일오버**, **GPU 간 All-Reduce 트래픽 병목을 완화하는 1:1 풀 바이섹션(Non-Blocking) Fat-Tree 설계 및 SHARP 인스위치 집계 연산 활성화**, **플래핑 링크에 의한 테일 지연을 방지하는 실시간 비트 오류율(BER) 감시 및 불량 포트 자동 격리**를 결합하여 슈퍼컴퓨팅 패브릭의 안정성과 신뢰성을 확보해야 한다.

#### 한줄 요약
- InfiniBand는 크레딧 기반 무손실 제어와 Fat-Tree 토폴로지 및 SM 이중화를 통해 초고성능 AI 클러스터를 실현하는 표준 인터커넥트로 운용해야 한다.
