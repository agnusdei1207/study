...
- **Lossless Ethernet (무손실 이더넷)**: 패킷 드롭이 발생하면 성능이 급락하는 RDMA를 위해 L2 PFC와 L3 ECN으로 버퍼 오버플로우를 선제적으로 방지하는 네트워크.
...
분기 결과: **DCQCN** 감속 성공·**PFC** 발동이 갈리며, 감속 갈래는 혼잡 구간의 전송률만 낮춰 백프레셔 없이 정상 흐름을 계속하되 ECN 수렴 속도에 의존하고, PFC 갈래는 버퍼 고갈 전 PAUSE 프레임으로 유입을 일시 중단해 패킷 손실은 차단하는 대신 상호 대기 큐 간 데드락(PFC Deadlock)과 후속 지연을 부담하므로, DCQCN 조정이 늦을수록 무손실은 지켜도 지연·마비 위험 비용이 커진다.
...
- **PFC Deadlock**: 링 토폴로지나 버퍼 순환 구조에서 스위치들이 서로를 향해 PAUSE 프레임을 영구 전송하여 트래픽이 전면 마비되는 교착 현상.
...
- **Leaf-Spine 아키텍처**: 모든 리프(ToR) 스위치가 모든 스파인 스위치와 1:1로 직접 연결되어 서버 간 홉 수와 전송 지연을 일정하게 보장하는 2계층 데이터센터 패브릭.
- **All-Reduce**: 분산 딥러닝 훈련 시 모든 노드가 각자의 그라디언트(Gradient)를 취합하여 동일한 평균/합산 결과를 공유받는 집합 통신 연산.

</details>

- 고가의 독점 패브릭을 대체하여 개방형 표준 이더넷 생태계에서 대규모 분산 AI 인프라를 지탱하는 **글로벌 하이퍼스케일러 및 엔터프라이즈 AI 데이터센터의 사실상 표준(De-facto Standard) 네트워크 기술**로 확고히 안착.
- 차세대 무손실 이더넷 표준인 Ultra Ethernet Consortium(UEC)으로 진화하는 가운데, 실무 RoCEv2 패브릭 구축 시에는 **스위치 큐 간 상호 대기로 인한 전사 트래픽 마비를 방지하는 PFC 데드락 워치독(Deadlock Watchdog) 활성화**, **패킷 드롭 이전에 선제 감속을 유도하는 ECN 임계치 튜닝 및 DCQCN 혼잡 제어 연동**, **All-Reduce 트래픽의 스파인 링크 편중을 해소하는 5-Tuple UDP 소스 포트 무작위화 기반 ECMP 로드 밸런싱**을 결합하여 무손실 전송 성능과 신뢰성을 확보해야 한다.

#### 한줄 요약
- RoCEv2는 범용 이더넷 상에서 무손실 흐름 제어와 L3 라우팅을 지원하여 대규모 AI 분산 학습의 통신 병목을 해소하도록 최적화 구축해야 한다.
