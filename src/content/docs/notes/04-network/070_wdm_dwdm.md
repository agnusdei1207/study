...
- 고정 격자를 넘어 12.5GHz 단위로 파장을 가변 할당하는 Flex-Grid DWDM과 800G/1.6Tbps 코히어런트 광학(Coherent Optics)으로 진화하는 가운데, 실무 백본망 구축 시에는 **전광 스위칭을 가능케 하는 ROADM(WSS) 파장 라우팅**, **수천 km 무중계 전송을 위한 EDFA/Raman 복합 광증폭 및 광신호대잡음비(OSNR) 관리**, **초고속 전송 시 색분산(CD)과 편광모드분산(PMD)을 디지털 보상하는 코히어런트 DSP 및 연판정 FEC(SD-FEC)**를 결합하여 완벽한 초광대역 광 네트워크를 완성.
