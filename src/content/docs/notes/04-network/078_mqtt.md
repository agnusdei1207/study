...
#### 한줄 요약
- 최소 2바이트 고정 헤더와 발행-구독 모델 및 3단계 QoS를 통해 저전력 경량 IoT 통신을 구현해야 한다.
...
#### 한줄 요약
- 초경량 헤더, 비동기 디커플링, 3단계 QoS, **LWT** 단절 알림을 체계적으로 제공해야 한다.
...
#### 한줄 요약
- 지수 백오프, mTLS 상호 인증, 메시지 만료 제어, 공유 구독을 적용하여 브로커 안정성을 확보해야 한다.

## Ⅶ. 결론

<details><summary>용어 설명</summary>

- **RBAC (Role-Based Access Control)**: 사용자 역할에 따라 발행 및 구독 가능한 토픽 경로 권한을 통제하는 접근 제어 모델.
- **Matter**: 스마트홈 기기 간 상호운용성을 보장하기 위해 IP(IPv6/Thread/Wi-Fi) 위에서 구동되는 표준 스마트홈 통신 프로토콜.

</details>

- 스마트홈(Matter/Home Assistant), 커넥티드 카 텔레메트리, 스마트 팩토리 센서망 및 클라우드 IoT 플랫폼(AWS IoT Core, Azure IoT Hub)의 가장 지배적이고 사실상의 표준(De-facto Standard) 경량 메시징 프로토콜로 확고히 자리잡았다.
- MQTT 5.0의 사용자 속성(User Properties), 공유 구독(Shared Subscription), 메시지 만료 간격(Message Expiry) 기능을 통해 엔터프라이즈 확장성까지 확보한 가운데, 실무 브로커 클러스터(EMQX/HiveMQ) 운영 시에는 **대규모 단말 동시 재접속 시 브로커 마비를 방지하는 지수 백오프(Exponential Backoff) 및 지터(Jitter) 적용**, **단말 탈취 및 도청을 방지하는 mTLS 상호 인증과 토픽 수준 RBAC**, **백엔드 컨슈머 병목을 해소하는 MQTT 5.0 공유 구독 로드밸런싱**을 결합하여 안정적인 IoT 메시징 신뢰성을 확보해야 한다.

#### 한줄 요약
- 발행-구독 구조와 QoS 레벨 선택을 최적화하여 경량성과 전달 신뢰성의 균형을 확보해야 한다.
