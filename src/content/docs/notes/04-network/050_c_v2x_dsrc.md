...
```text
[V2X 안전 메시지 채널 접속 흐름] (진행 ①→③, 센싱에서 진입, ①→② 접속 절차, ③ BSM 송출, ① 표준에 따라 SPS·백오프 분기)
  │
  ├─ [C-V2X PC5 송신부] (① 서브채널 RSRP/RSSI를 1,000ms 주기 센싱해 빈 자원 블록(RB) 후보 선별)
  │
  ├─ [SPS 예약 스케줄러] (② **CBR** 혼잡도에 맞춰 전송 간격 조절 후 충돌 없는 주기 BSM 예약)
  │
  ├─ [802.11p 캐리어 센싱기] (① CCA로 채널 유휴 여부 즉각 판정)
  │
  ├─ [CSMA/CA 백오프 엔진] (② DIFS 대기 후 무작위 Contention Window 카운트다운)
  │
  └─ [브로드캐스트 송출부] (③ 예약 시점 또는 백오프 종료 시점에 안전 메시지 일제 송출)
```

분기 결과: ① 센싱 단계에서 두 표준이 갈라져 C-V2X는 1,000ms 관측으로 미래 충돌을 사전 회피하는 대가로 **CBR** 측정·예약 유지 비용을 치르고, DSRC는 즉시 송출 기회를 얻지만 밀집 구간에서 동시 송출 충돌·유실을 감수한다.
...
- 한국, 미국, 중국을 포함한 글로벌 주요국들이 C-ITS 국가 단일 표준으로 C-V2X 방식을 확정함에 따라 자율협력주행 및 커넥티드 카 생태계의 단일 지배 표준 기술로 자리매김.
