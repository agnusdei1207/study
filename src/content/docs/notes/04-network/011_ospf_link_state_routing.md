...
- 전체 네트워크 지도를 기반으로 최단 트리를 계산하여 **구조적 라우팅 루프 차단**
...
#### 한줄 요약
- SPF 루프 방지, 증분 LSA 고속 수렴, 다중 Area 계층 설계를 통해 확장성을 확보한다.
...
| 토폴로지와 주소 결합 | LSA 내에 **라우터 링크와 IPv4 주소가 결합** | **토폴로지(LSA 1/2)와 IPv6 주소(LSA 9)를 분리** |
...
| 라우터 장애 감지 지연(Dead Timer 40초)으로 인한 트래픽 유실 | **`BFD (Bidirectional Forwarding Detection)` 서브세컨드 연동** | 50ms 이내 고속 장애 감지 및 페일오버 |

#### 한줄 요약
- Virtual-Link 구제, MTU 일치, SPF 쓰로틀링, BFD 고속 연동으로 운영한다.

## Ⅶ. 결론

<details><summary>용어 설명</summary>

- **양방향 포워딩 감지(BFD, Bidirectional Forwarding Detection)**: 프로토콜에 독립적으로 데이터 평면의 링크 단절을 밀리초(ms) 단위로 고속 감지하여 라우팅 프로토콜에 알리는 기술.

</details>

- 기업 인트라넷, 데이터센터 패브릭 및 캠퍼스 백본망에서 가장 널리 활용되는 **표준 링크 상태 내부 게이트웨이 프로토콜**(**IGP**)의 사실상 표준으로 기능
- 실무 구축 시 **Area 0** 중심의 계층적 영역 분할과 **ABR 경로 요약**(Type 3 LSA), 브로드캐스트 망 내 **DR/BDR 선출 최적화**, 링크 플래핑 시 **CPU 보호 SPF Throttle 타이머 튜닝**, 50ms 이내 고속 장애 감지 **BFD**(Bidirectional Forwarding Detection)** 연동**, **IPv6 환경 OSPFv3 전환**을 결합하여 고신뢰 엔터프라이즈 코어 네트워크를 구축해야 함

#### 한줄 요약
- 다익스트라 **SPF** 알고리즘과 계층적 다중 **Area** 설계를 통한 루프 없는 고속 수렴 제공 핵심 내부 게이트웨이 라우팅 기술이다.
