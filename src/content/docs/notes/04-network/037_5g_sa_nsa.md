...
```text
[5G SA 음성 호 처리 흐름] (진행 ①→③, 발신에서 진입, ② 무선 품질 판정으로 분기, ③ 각 경로로 통화 연결)
  │
  ├─ [5G SA 단말] (① SIP INVITE 음성 호 발신)
  │
  ├─ [VoNR 판정기] (② NR 무선 채널 품질과 gNB 음성 지원 여부 판정)
  │
  ├─ [5G UPF 음성 경로] (③ 커버리지 양호 시 VoNR 호 수립으로 초고음질 무지연 통화)
  │
  └─ [EPS Fallback 지시기] (③ 음영·품질 미흡 시 **EPS Fallback** 리다이렉트로 4G LTE 셀 핸드오버·VoLTE 연결)
```

분기 결과: **EPS Fallback**이 ② 판정의 한 갈래를 구성하는데, VoNR 갈래는 5G 내 경로를 유지해 음질·지연을 얻는 대신 음영 지역 커버리지 리스크를 안고, Fallback 갈래는 통화 성공률을 확보하는 대신 4G 망 전환 지연과 음질 하락을 치른다.
...
- 글로벌 이동통신사들이 CAPEX/OPEX를 절감하며 5G 서비스를 조기 개시하기 위한 과도기적 NSA(Option 3x) 단계를 거쳐, B2B 특화망 및 지능형 융합 서비스를 위한 **순수 5G SA(Option 2) 아키텍처로의 전면적인 전환이 가속화**되고 있음.
