...
date: "2026-09-14T17:34:00+09:00"
tags:
  - "notes-software"
weight: 160
extra:
...
#### 한줄 요약
- 가로채기·암호화·라우팅·추적이 프록시 한 계층에 모여 앱의 구현 비용은 크게 줄지만, 그 대가로 모든 요청이 프록시 홉을 왕복하는 고정 지연을 새로 부담한다.
...
- 대규모 마이크로서비스 아키텍처(MSA) 및 제로 트러스트(Zero Trust) 네트워킹의 **서비스 메시의 핵심 표준 플랫폼**으로 자리 잡았다.
- 실무 구축 시에는 **서비스 간 자동 mTLS 암호화**(PeerAuthentication)**의 단계별**(PERMISSIVE $\to$ STRICT) **전환**, **사이드카 메모리 팽창을 방어하는 `Sidecar` CRD 네임스페이스 통신 스코프 제한**, **연쇄 장애를 차단하는 DestinationRule 서킷 브레이커**, **리소스 오버헤드를 대폭 절감하는 차세대 사이드카리스**(Sidecarless) **Ambient Mesh**(ztunnel)를 결합하여 보안 완결성과 운영 효율을 동시에 확보해야 한다.

#### 한줄 요약
- istiod 제어면과 Envoy 프록시를 통해 비즈니스 코드 변경 없이 mTLS 보안과 카나리 라우팅, 분산 추적을 일원화한다.
