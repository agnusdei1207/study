...
date: "2026-09-14T17:02:00+09:00"
tags:
  - "notes-software"
weight: 108
extra:
...
- **Apache Cassandra**: Facebook이 개발하고 아파치에서 오픈소스화한 분산 Wide-Column 데이터베이스로, P2P 마스터리스 링 아키텍처 지원.
...
- 마스터가 없어 단일 장애점(SPOF)이 없는 대등 노드 P2P 링 아키텍처
- CommitLog + MemTable + SSTable을 통한 순차 쓰기(Sequential I/O) 중심 구조
- 파티션 키(분산 저장)와 클러스터링 키(물리 정렬)를 결합한 복합 기본키(Primary Key) 구조

#### 한줄 요약
- 마스터리스 P2P 링과 순차 쓰기 기반 스토리지 엔진으로 대규모 시계열 및 로그 쓰기를 효율적으로 처리한다.
...
| 비교 항목 | 관계형 데이터베이스 (RDBMS) | Apache Cassandra (Wide-Column NoSQL) |
|:---|:---|:---|
| 아키텍처 모델 | 단일 Primary + 다수 Replica (마스터 의존) | 대등 노드 마스터리스 P2P 링 (SPOF 제거) |
| 데이터 모델링 | 3NF 정규화 모델링 (다중 테이블 Join 지원)| 쿼리 주도 비정규화 (1 Query = 1 Table, Join 불가) |
| 쓰기 성능 (Write) | 보통 (디스크 블록 랜덤 I/O 발생) | 우수 (CommitLog + MemTable 기반 순차 I/O) |
| 주요 제약사항 | 수평 확장의 물리적 한계 봉착 | 파티션 키 누락 시 전체 클러스터 풀스캔 부하 발생 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 파티션 키 없이 `SELECT` 조회 시 전체 클러스터 스캔 발생 | 조회 쿼리마다 파티션 키를 필수로 포함하는 테이블 전용 재설계 | 불필요한 노드 풀스캔 방지 및 $O(1)$ 라우팅 |
...
| 노드 간 데이터 불일치 누적 | Nodetool Repair 정기 실행 및 Read Repair 활성화 | 분산 복제본 간 데이터 정합성 유지 |
...
- 대규모 시계열 센서 데이터, 글로벌 메시징 및 분산 로깅 시스템의 **대표적인 Wide-Column NoSQL 분산 스토리지**로 자리 잡았다.
- 실무 구축 시에는 **조인을 배제하고 화면 조회 요건마다 1:1 전용 비정규화 테이블을 구성하는 쿼리 주도 모델링**(Query-Driven Modeling), **핫 파티션을 방지하는 시간 버킷 복합 파티션 키 설계**, **비즈니스 요건에 맞춘 Tunable Consistency($R+W>N$, Quorum) 설정**, **주기적인 Nodetool Repair/Tombstone 컴팩션 관리**를 결합하여 시스템 확장성과 데이터 신뢰성을 동시에 확보해야 한다.

#### 한줄 요약
- 마스터리스 P2P 링과 LSM-Tree 순차 쓰기를 바탕으로 쿼리 주도 비정규화와 일관성 수준 조절을 결합하여 무중단 쓰기 성능과 고가용성을 확보한다.
