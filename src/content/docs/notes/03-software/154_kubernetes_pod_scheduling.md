...
date: "2026-09-14T17:30:00+09:00"
tags:
  - "notes-software"
weight: 154
extra:
...
| 테인트/톨러레이션 | GPU 전용 노드 또는 컨트롤 플레인에 비인가 일반 파드가 배치되는 것을 차단 | NoSchedule, NoExecute |
...
| 특정 노드 1대에 동일 서비스 파드가 몰려 노드 다운 시 서비스 장애 | `topologySpreadConstraints` (maxSkew: 1) 전사 적용 | 가용 영역(AZ) 및 노드 간 균등 분산 |
| GPU 머신러닝 전용 고가 노드에 일반 웹 파드가 배치되어 자원 낭비 | GPU 노드에 `Taint: dedicated=gpu:NoSchedule` 설정 | 전용 특수 노드 자원 보호 |
...
- 엔터프라이즈 컨테이너 클러스터의 자원 균형 및 고가용성 아키텍처를 완성하는 **핵심적인 워크로드 배치 제어 엔진**으로 자리 잡았다.
- 실무 구축 시에는 **GPU 등 특수 리소스 보호를 위한 Taints & Tolerations**, **다중 가용 영역**(Multi-AZ) **간 파드 쏠림을 방어하는 `topologySpreadConstraints(maxSkew: 1)`**, **중요 워크로드의 배치 우선권을 보장하는 PriorityClass & Preemption**, **대규모 배치 지연을 해소하는 Just-in-Time 노드 프로비저너 Karpenter 연동**을 결합하여 자원 효율성과 인프라 가용성을 확보해야 한다.

#### 한줄 요약
- 2단계 노드 평가와 선언적 배치 제약을 결합하여 자원 균형 배치와 장애 영역 분산을 실현한다.
