...
- 쿠버네티스(K8s) 및 서비스 메시(Istio/Envoy) 아키텍처의 근간을 이루며 비즈니스 로직과 인프라 플랫폼 기능을 분리하는 **가장 핵심적인 클라우드 네이티브 컨테이너 디자인 패턴으로 확립**
...
#### 한줄 요약
- 파드 내 보조 컨테이너를 통해 비즈니스 코드 수정 없이 보안, 관측성, 트래픽 제어를 주입하는 클라우드 네이티브 아키텍처 체계 구축
