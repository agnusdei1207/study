...
date: "2026-09-14T17:03:00+09:00"
tags:
  - "notes-software"
weight: 109
extra:
...
- 그래프 전용 선언적 질의어인 Cypher 언어 및 ACID 트랜잭션 지원

#### 한줄 요약
- 무색인 인접성과 Cypher 언어로 복잡한 다단계 관계를 직관적이고 효율적으로 탐색한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 수백만 엣지가 연결된 **Super Node** 순회 시 메모리 폭증 | **Super Node** 서브 노드 분할 및 관계 방향/속성 기반 사전 필터링 | 순회 메모리 급증 방지 |
| 시작 노드 인덱스 누락으로 전체 그래프 풀스캔 발생 | 선택도 높은 Node Label 및 고유 식별자(ID)에 Schema Index 생성 | 출발점 탐색 시간 단축 |
| Cypher 쿼리에서 무제한 Hop 순회로 리소스 고갈 | 쿼리 작성 시 최대 순회 깊이 명시 (`-[:FRIEND*1..3]->`) 강제 | 쿼리 타임아웃 방지 |
...
- 엔터프라이즈 복합 관계망 분석 및 생성형 AI 지식 그래프(Graph RAG) 구축의 **대표적인 그래프 데이터베이스 엔진**으로 자리 잡았다.
- 실무 구축 시에는 **무제한 순회 폭주를 방지하는 최대 Hop 깊이 제한(`*1..3`)**, **수백만 엣지를 가진 Super Node 분할 및 관계 속성 사전 필터링**, **출발점 노드에 대한 스키마 인덱스 설계**, **대규모 분산 처리를 위한 Neo4j Fabric 및 Causal Clustering 결합**을 통해 안정적인 고성능 그래프 탐색을 실현해야 한다.

#### 한줄 요약
- 무색인 인접성과 Property Graph 모델을 바탕으로 최대 순회 깊이 제한과 슈퍼 노드 분할을 적용하여 복합 관계망을 효율적으로 탐색한다.
