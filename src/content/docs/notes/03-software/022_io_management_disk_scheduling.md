...
- 초고속 SSD 환경에서는 스케줄링 오버헤드를 배제하는 `none` 또는 **다중 큐 데드라인 스케줄러**(mq-deadline)를 선택하고, 대용량 스토리지에는 **예산 공정 큐잉**(BFQ)을 적용하는 미디어 맞춤형 스케줄러 선정 기준 확립

#### 한줄 요약
- NVMe 환경은 blk-mq 기반 무스케줄링(none)으로 오버헤드를 제거하고 대화형 HDD 환경은 BFQ 공정 제어로 병목을 해소하는 최적화 체계 구축
