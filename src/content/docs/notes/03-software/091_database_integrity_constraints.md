---
sidebar:
  order: 91
  label: "091. 데이터베이스 무결성 제약 조건"
  badge:
    text: "기출 · 70%"
    variant: note
title: "데이터베이스 무결성 제약 조건 (Database Integrity Constraints)"
date: "2026-09-14T16:48:00+09:00"
tags:
  - "notes-software"
weight: 91
extra:
  question_no: "091"
  source_status: "기출"
  source_history: "128회, 134회"
  priority: 70
  priority_note: "128•134회 반복, 무결성 제약 설계 핵심"
---

## Ⅰ. 개요

<details><summary>용어 설명</summary>

- **무결성 제약조건(Integrity Constraints)**: 데이터베이스 내 데이터의 정확성, 일관성, 유효성을 보장하기 위해 DBMS 엔진이 강제하는 규칙 체계.
- **4대 무결성 제약**: 개체 무결성(PK), 참조 무결성(FK), 도메인 무결성(Check), 사용자 정의 무결성(Trigger).

</details>

- 정의/개념: 데이터베이스에 저장되는 데이터의 유효성과 일관성을 위해 **개체(PK), 참조(FK), 도메인(Check), 사용자정의(Trigger)** 4대 규칙을 강제하는 스키마 제약 체계
- 배경/필요성: 애플리케이션 계층 검증 우회 시 발생하는 **불량 데이터 유입, 고아 레코드(Orphan Data) 양산 및 데이터 정합성 붕괴 위험 한계**

#### 한줄 요약
- 제약조건은 어떤 우회 경로에도 뚫리지 않는 대신 모든 쓰기가 검증 비용을 치르게 하므로, 대량 적재 구간에서는 제약을 일시 해제하고 검증을 사후로 미루는 판단이 필요해진다.

## Ⅱ. 특징

<details><summary>용어 설명</summary>

- **Declarative DDL**: 애플리케이션 코드가 아닌 `PRIMARY KEY`, `FOREIGN KEY`, `CHECK` 등 DDL 문법으로 데이터베이스 스키마 자체에 선언.
- **Orphan Data(고아 데이터)**: 부모 테이블의 레코드가 삭제되었는데, 자식 테이블의 외래키 참조 레코드가 그대로 남아 연결이 끊어진 불량 데이터.

</details>

- 모든 데이터 조작에 대해 DBMS 엔진 차원의 **선언적 DDL 기반 중앙집중식 규칙 강제**
- 런타임 제약조건 위반 감지 시 즉각적 에러 방출 및 **트랜잭션 자동 롤백(Fail-Fast)**
- 다중 마이크로서비스 및 배치 직접 접근 환경에서도 **우회 불가능한 최후의 데이터 안전망**

#### 한줄 요약
- 선언적 DDL과 엔진 차원의 중앙 통제로 어떤 우회 경로에서도 데이터 무결성을 엄격하게 방어한다.

## Ⅲ. 구조 및 구성요소

<details><summary>용어 설명</summary>

- **ON DELETE 옵션**: 부모 레코드 삭제 시 자식 데이터 처리 정책 (RESTRICT, CASCADE, SET NULL, NO ACTION).

</details>

```text
[데이터베이스 무결성 제약 조건 체계]
  │
  ├─ [개체 무결성 (Entity)]
  │     └─ [PRIMARY KEY] (유일성·NOT NULL 강제)
  │
  ├─ [참조 무결성 (Referential)]
  │     └─ [FOREIGN KEY] (부모-자식 참조 유지)
  │
  ├─ [도메인 무결성 (Domain)]
  │     └─ [CHECK / DEFAULT / NOT NULL] (값 범위)
  │
  └─ [사용자 정의 무결성 (User-Defined)]
        └─ [TRIGGER / 프로시저] (업무 규칙 강제)
```

- 선의 의미: 계층 구조 및 상하위 포함 관계를 나타낸다.

| 구성요소 | 책임 |
|:---|:---|
| 개체 무결성 | **기본키 유일성·Null 금지**로 행 식별 보장 |
| 참조 무결성 | **외래키**로 고아 데이터 발생 차단 |
| 도메인 무결성 | **타입·범위·기본값**으로 속성 유효성 검증 |
| 사용자 정의 무결성 | **트리거·프로시저**로 업무 규칙 검증 |

#### 한줄 요약
- 네 무결성은 행·관계·값·업무라는 다른 범위를 맡되 모두 쓰기 시점 검증이라는 같은 방식을 사용하므로, 검사 범위가 넓은 사용자 정의 무결성일수록 트리거 실행 비용이 가파르게 커진다.

## Ⅳ. 흐름도

<details><summary>용어 설명</summary>

- **RESTRICT vs CASCADE**: 자식이 있으면 부모 삭제를 금지하는 RESTRICT와 부모 삭제 시 자식까지 연쇄 삭제하는 CASCADE.

</details>

```text
[부모 레코드 삭제 경로] (진행 ①→⑤, ③~⑤ ON DELETE 정책 분기)
  │
  ├─ [FK 참조 탐색] (① DBMS 엔진이 자식 테이블(Orders) 외래키 인덱스를 B-Tree 탐색)
  │
  ├─ [삭제 실행 완료] (② 참조하는 자식 레코드 없음, 부모 레코드 정상 삭제 후 커밋)
  │
  ├─ [RESTRICT / NO ACTION] (③ 자식 존재 + 차단 정책, 삭제 차단·롤백 에러)
  │
  ├─ [CASCADE] (④ 자식 존재 + 연쇄 정책, 자식 주문도 연쇄 자동 삭제)
  │
  └─ [SET NULL] (⑤ 자식 존재 + 초기화 정책, 자식 외래키를 NULL로 변경)
```

분기 결과: 참조 탐색 비용은 정책과 무관하게 매 삭제마다 발생하므로 자식이 없는 삭제는 인덱스 룩업 1회로 끝나지만, 자식이 있는 삭제는 정책에 따라 실패·0건·N건·1건의 쓰기로 갈려 동일한 부모 삭제가 데이터 손실 위험과 영향 범위를 맞바꾼다

#### 한줄 요약
- RESTRICT는 삭제를 막아 정합성 판단을 응용에 되돌려 주고 CASCADE는 연쇄 삭제로 그 판단을 엔진이 대신하지만 영향 범위를 예측하기 어려우므로, 두 갈래는 데이터 손실 위험과 운영 편의를 맞바꾸는 선택이 된다.

## Ⅴ. 종류 및 비교

<details><summary>용어 설명</summary>

- **Application Validation vs DB Constraint**: 스프링 애플리케이션 레벨 검증(Java `@Valid`)과 DBMS 엔진 레벨 제약(DDL Constraint).

</details>

| 비교 항목 | 애플리케이션 검증 (Spring `@Valid`) | 데이터베이스 제약 (DB Constraints) |
|:---|:---|:---|
| 검증 주체 | 웹 서버 / 애플리케이션 메모리 | **DBMS 스토리지 엔진 (InnoDB 등)** |
| 장점 | 사용자 친화적 에러 메시지, 복잡한 비즈니스 로직 검증 | **직접 SQL/배치 접근에도 우회 불가능한 무결성 보장** |
| 단점 | DB 직접 쿼리나 타 서비스 연계 시 우회 가능 | 제약조건 변경 시 스키마 DDL 마이그레이션 오버헤드 |
| 최적 전략 | 1차 사용자 UX 및 폼 유효성 검증 | **최후의 데이터 무결성 보루(Defensive Barrier)** |

#### 한줄 요약
- 애플리케이션 검증은 사용자 경험을 개선하고, DB 무결성 제약은 최후의 데이터 안전망을 담당한다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>용어 설명</summary>

- **Unindexed FK(외래키 인덱스 누락)**: 자식 테이블의 FK 컬럼에 인덱스가 없을 경우, 부모 레코드 삭제/수정 시 자식 테이블 전체에 풀 테이블 락(Table Lock)이 걸리는 성능 저하.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| Foreign Key 인덱스 누락으로 부모 행 삭제 시 Table Lock 발생 | **모든 자식 테이블의 Foreign Key 컬럼에 B-Tree 인덱스 필수 생성** | 참조 검사 속도 향상 및 락 경합 방지 |
| `ON DELETE CASCADE` 남용으로 인한 대규모 데이터 연쇄 유실 | **실무에서는 `CASCADE` 금지, `RESTRICT` 또는 `Soft Delete` 적용** | 운영자의 실수로 인한 핵심 원장 삭제 방지 |
| 대용량 데이터 배치 적재(Bulk Insert) 시 제약조건 부하 | **적재 전 제약조건 일시 비활성화 $\to$ 적재 완료 후 일괄 검증 활성화** | 배치 처리 시간 80% 단축 |
| 분산 MSA 환경에서 DB 간 물리적 Foreign Key 설정 불가 | **카프카 CDC(Debezium) 기반 도메인 이벤트로 참조 정합성 유지** | 서비스 간 결합도 해제 및 최종 정합성 달성 |

#### 한줄 요약
- FK 인덱스 필수 생성, CASCADE 남용 금지, 배치 시 제약 일시 해제, MSA 이벤트 연계로 최적화한다.

## Ⅶ. 결론

<details><summary>용어 설명</summary>

- **심층 방어(Defense in Depth)**: 애플리케이션 유효성 검증과 데이터베이스 엔진의 선언적 제약조건을 다계층으로 배치하여 오류와 침해를 차단하는 다중 보호 전략.
- **소프트 딜리트(Soft Delete)**: 물리적 삭제(Hard Delete) 대신 삭제 여부 플래그나 삭제 시각 컬럼을 갱신하여 데이터의 이력과 참조 무결성을 보존하는 기법.

</details>

- **마이크로서비스 분산 환경 및 이벤트 주도 데이터 거버넌스 진화**: 단일 RDBMS 내부의 선언적 무결성 제약조건(PK, FK, Check, Unique)은 데이터베이스가 샤딩되거나 분산 서비스로 파편화됨에 따라, 아파치 카프카 및 이벤트 소싱 기반의 분산 데이터 정합성 검증과 데이터 카탈로그 거버넌스 도구로 확장 진화 추세
- **다계층 심층 방어 구축과 연쇄 삭제 통제 결단**: 사용자 입력 오류는 애플리케이션 검증(Bean Validation)으로 조기 차단하고 최후의 보루로 DB 선언적 제약조건을 두는 **심층 방어**(Defense in Depth)를 구현하되, 운영 중 대규모 데이터 유실을 방지하기 위해 `CASCADE`를 지양하고 **소프트 딜리트**(Soft Delete)와 FK 인덱싱을 강제하는 엔지니어링 결단

#### 한줄 요약
- 선언적 DDL 제약조건과 애플리케이션 검증을 결합한 다계층 심층 방어를 구축하고, FK 인덱싱 및 Soft Delete 정책을 통해 데이터 무결성과 시스템 안정성을 동시에 확보한다.
