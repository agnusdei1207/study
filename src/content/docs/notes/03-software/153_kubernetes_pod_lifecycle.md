...
date: "2026-09-14T17:29:00+09:00"
tags:
  - "notes-software"
weight: 153
extra:
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 배포 롤링 업데이트 시 클라이언트에 간헐적 502 에러 발생 | `preStop: sleep 10` 설정으로 Endpoint 제거 대기 시간 확보 | 무중단 롤링 배포 달성 |
| 초기 부팅이 1분 이상 걸리는 앱의 빈번한 조기 재시작 | `startupProbe` 도입 및 충분한 `failureThreshold` 부여 | 부팅 타임아웃 오류 방지 |
| 외부 DB 일시 장애 시 클러스터 전체 파드 무한 재시작 | Liveness는 내부 헬스만 검사하고 DB 연동은 Readiness로 분리 | 연쇄 재시작 장애 방지 |
...
- 마이크로서비스 애플리케이션의 무중단 배포 및 고가용성 서비스 연속성을 보장하는 **핵심적인 파드 런타임 제어 메커니즘**으로 자리 잡았다.
- 실무 운영 시에는 **부팅 타임아웃을 방어하는 StartupProbe**, **내부 데드락 복구 전용 LivenessProbe와 외부 의존성 장애 시 트래픽만 선별 격리하는 ReadinessProbe의 엄격한 역할 분리**, **엔드포인트 전파 지연을 보정하는 `preStop: sleep 10` 훅 및 `terminationGracePeriodSeconds` 유예 설정**을 결합하여 롤링 업데이트 시 다운타임과 커넥션 단절을 최소화해야 한다.

#### 한줄 요약
- 3대 프로브를 역할별로 분리하고 정상 종료 절차를 연계하여 서비스 무중단성과 자가 치유의 안정성을 확보한다.
