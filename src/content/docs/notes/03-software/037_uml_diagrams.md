---
sidebar:
  order: 37
  label: "037. UML 다이어그램 분류•표기법"
  badge:
    text: "기출 · 50%"
    variant: note
title: "UML 다이어그램 분류•표기법 (UML Diagrams)"
date: "2026-09-14T15:45:00+09:00"
tags:
  - "notes-software"
weight: 37
extra:
  question_no: "037"
  source_status: "기출"
  source_history: ""
  priority: 50
  priority_note: "구조•행위 다이어그램 체계 및 관계 표기법"
---

## Ⅰ. 개요

<details><summary>용어 설명</summary>

- **UML(Unified Modeling Language)**: 객체지향 소프트웨어의 산출물을 명세화, 시각화, 문서화하기 위해 OMG가 표준화한 모델링 언어.
- **OMG(Object Management Group)**: UML, CORBA, SysML 등의 개방형 객체지향 표준을 제정하고 관리하는 국제 컴퓨터 소프트웨어 컨소시엄.

</details>

- 정의/개념: 소프트웨어 시스템의 정적 구조(Structure)와 동적 행위(Behavior)를 시각화·명세화하는 **OMG 표준 모델링 언어**
- 배경/필요성: 비표준 도형 기반 설계서의 해석 모호성 및 **개발자 간 소통 왜곡에 따른 구현 단계의 설계 불일치 결함 한계**

#### 한줄 요약
- 정적 구조(7종)와 동적 행위(7종) 총 14개 다이어그램으로 시스템 설계를 표준 시각화한다.

## Ⅱ. 특징

<details><summary>용어 설명</summary>

- **구조 vs 행위 다이어그램**: 시스템의 정적 물리/논리 구성요소를 표현하는 구조 다이어그램과 시간 흐름에 따른 동적 동작을 표현하는 행위 다이어그램.
- **MDA(Model-Driven Architecture)**: 플랫폼 독립 모델(PIM)을 플랫폼 종속 모델(PSM) 및 코드로 자동 변환하는 모델 주도 개발 패러다임.

</details>

- 시스템의 시각화 관점에 따라 **정적 구조(7종)와 동적 행위(7종)의 14개 표준 다이어그램** 제공
- 객체 간 관계를 표현하는 **일반화, 실체화, 의존, 합성, 집합의 명확한 화살표 표기 규격**
- 도메인 분석부터 상세 설계, 자동 코드 생성(**MDA: Model-Driven Architecture**)까지 전주기 지원

#### 한줄 요약
- 표준 표기는 해석의 모호성을 줄이는 대신 도면 작성과 갱신 비용을 계속 요구하므로, 14종을 다 그리는 것이 아니라 필요한 관점만 고르는 것이 관건이 된다.

## Ⅲ. 구조 및 구성요소

<details><summary>용어 설명</summary>

- **클래스 다이어그램(Class Diagram)**: 시스템의 클래스, 속성, 메서드 및 클래스 간의 정적 관계를 표현하는 대표적 구조 다이어그램.
- **시퀀스 다이어그램(Sequence Diagram)**: 객체들이 주고받는 메시지의 시간적 순서(Sequence)를 생명선(Lifeline) 위에 표현하는 행위 다이어그램.

</details>

```text
[UML 2.x 다이어그램 14종 체계]
  │
  ├─ [구조 다이어그램] (정적 시스템 뼈대 7종)
  │     ├─ [클래스·객체·패키지] (논리적 모듈 구조)
  │     ├─ [컴포넌트·복합체] (물리적 컴포넌트)
  │     └─ [배치·프로파일] (하드웨어 배치·확장)
  │
  └─ [행위 다이어그램] (동적 실행 흐름 7종)
        ├─ [유스케이스] (사용자 관점 기능)
        ├─ [활동·상태 머신] (업무 흐름·상태 전이)
        └─ [상호작용 다이어그램] (객체 간 메시지 4종)
              ├─ [시퀀스·통신] (시간순·링크순 메시지)
              └─ [상호작용개요·타이밍] (흐름·시간 제약)
```

선의 의미: 계층 구조 및 상하위 포함 관계를 나타낸다.

| 구성요소 | 책임 |
|:---|:---|
| 정적 구조 (7종) | **클래스·컴포넌트·배치** 등 정적 관계 표현 |
| 동적 행위 (7종) | **유스케이스·활동·상태 머신** 등 행위 표현 |
| 상호작용 (4종) | **시퀀스·통신** 등 객체 간 메시지 교환 표현 |

#### 한줄 요약
- 구조 다이어그램은 한 시점의 정적 형상을, 행위 다이어그램은 시간에 따른 상호작용을 담당하므로, 두 계열을 함께 사용해야 설계가 하나의 모델로 닫힌다.

## Ⅳ. 흐름도

<details><summary>용어 설명</summary>

- **Lifeline & Message**: 시퀀스 다이어그램에서 객체의 생존 기간을 나타내는 수직 점선(Lifeline)과 객체 간 호출을 나타내는 수평 화살표(Message).

</details>

```text
[UML 설계 흐름] (진행 ①→④, 구조·행위 두 갈래가 관계 표기 합류 후 코드 매핑)
  │
  ├─ [관점 판정] (① 설계 대상이 정적 구조 표현인지 동적 런타임 상호작용인지 판별)
  │
  ├─ [정적 구조 설계] (② 도메인 엔티티·속성 도출 후 클래스 다이어그램 작성)
  │
  ├─ [동적 행위 설계] (② 시나리오별 참여 객체 Lifeline 배치 후 시퀀스 다이어그램 작성, 동기/비동기 메시지)
  │
  ├─ [관계 표기 적용] (③ OMG 표준 관계 표기법 적용, 일반화·실체화·의존·합성·집합을 실선·점선·삼각형·마름모 조합으로 구분)
  │
  └─ [검증·매핑] (④ 아키텍처 설계 검증 및 코드 구현 매핑)
```

분기 결과: 정적 구조 갈래는 클래스·패키지의 실체 간 관계를 그려 설계 오류를 미리 찾는 대신 도면 갱신 비용을 지속 요구하고, 동적 행위 갈래는 **시퀀스 다이어그램**의 메시지 순서 검증으로 순서 오류를 잡는 대신 생명선 수가 늘어날수록 작성·유지 비용이 급증하므로, 한 계열만 사용하면 남은 오류 유형이 구현 단계까지 살아남는다

#### 한줄 요약
- 정적 다이어그램은 구조의 오류를, 동적 다이어그램은 순서의 오류를 드러내므로, 한쪽만 그리면 남은 오류 유형이 구현 단계까지 살아남는다.

## Ⅴ. 종류 및 비교

<details><summary>용어 설명</summary>

- **합성 vs 집합(Composition vs Aggregation)**: 부모 객체 소멸 시 자식도 함께 소멸하는 강한 소유(합성: 검은 마름모)와 독립 생명주기를 갖는 약한 소유(집합: 빈 마름모).

</details>

| 관계 표기법 | 의미 및 결합도 | OMG 표준 화살표 표기법 | 코드 매핑 예시 |
|:---|:---|:---|:---|
| 일반화 (Generalization) | 부모 클래스를 상속받는 IS-A 관계 | 실선 + 빈 삼각형 화살표 (`──▷`) | `class Dog extends Animal` |
| 실체화 (Realization) | 인터페이스의 명세를 구현하는 관계 | 점선 + 빈 삼각형 화살표 (`--▷`) | `class ServiceImpl implements Service` |
| 의존 (Dependency) | 메서드 파라미터나 로컬 변수로 일시 참조 | 점선 + 열린 화살표 (`-->`) | `void send(Message msg)` |
| 합성 (Composition) | 생명주기를 공유하는 강한 전체-부분 관계 | 실선 + **채워진 마름모 (`──◆`)** | `class House { Room room = new Room(); }` |
| 집합 (Aggregation) | 독립적 생명주기를 갖는 약한 전체-부분 관계 | 실선 + 빈 마름모 (`──◇`) | `class Department { List<Employee> emps; }` |

#### 한줄 요약
- 일반화(상속), 실체화(구현), 의존(참조), 합성(강한소유), 집합(약한소유)으로 관계를 정밀 표현한다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>용어 설명</summary>

- **모델 드리프트(Model Drift)**: 코드 변경 시 설계를 동기화하지 않아 UML 다이어그램과 실제 소스 코드가 불일치하게 되는 부패 현상.
- **DaC(Diagram-as-Code)**: 마우스 GUI 도구 대신 PlantUML, Mermaid 등 텍스트 코드로 다이어그램을 작성하여 Git 버전 관리하는 방식.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 14종 다이어그램 과도한 작성으로 문서화 오버헤드 | **애자일 모델링**: 클래스, 시퀀스, 배치의 3대 핵심 다이어그램 집중 | 설계 소통 효율 향상 및 작성 비용 절감 |
| 코드 수정 후 다이어그램 미갱신으로 **모델 드리프트 발생** | **역공학(Reverse Engineering)** 도구 및 CI 문서 자동화 연동 | 소스 코드와 설계 다이어그램 간 100% 일치 보장 |
| GUI 도구 사용으로 인한 협업 및 버전 관리 곤란 | **Diagram-as-Code (PlantUML, Mermaid)** 표준화 | Git 기반 형상 관리 및 마크다운 문서 통합 |
| 객체 간 결합도 표기 모호성으로 인한 오구현 | **합성(`──◆`)과 집합(`──◇`) 표기 엄격 분리** | 메모리 누수 방지 및 명확한 수명주기 설계 |

#### 한줄 요약
- 다이어그램은 해석의 모호성을 줄이는 대신 작성·갱신 비용을 지속적으로 요구하므로, 핵심 도면만 남기고 나머지는 역공학과 Diagram-as-Code로 코드와 자동 동기화시킨다.

## Ⅶ. 결론

<details><summary>용어 설명</summary>

- **시스템 모델링 언어(Systems Modeling Language, SysML)**: 소프트웨어뿐 아니라 하드웨어, 데이터, 인력 등 복잡한 융합 시스템 엔지니어링을 위해 OMG에서 UML을 확장 표준화한 범용 모델링 언어.
- **역공학(Reverse Engineering)**: 구현된 소스 코드를 정적/동적으로 분석하여 클래스 다이어그램 및 시퀀스 흐름 등 상위 아키텍처 모델을 자동 추출하는 기법.
- **아키텍처 결정 레코드(Architecture Decision Record, ADR)**: 중요한 아키텍처 설계 의사결정의 맥락, 대안, 최종 결정 사항 및 파급 효과를 경량 마크다운 문서로 기록하여 Git 버전 관리하는 기법.

</details>

- **차세대 발전 전망**: 융합 복잡계 설계를 지원하는 **SysML**로의 표준 확장과 함께, AI 기반 소스 코드 정적 분석을 통한 **역공학**(Reverse Engineering) 및 실시간 모델 자동 갱신 파이프라인으로 진화
- **실무 공학적 통찰**: 14종 다이어그램의 전수 작성으로 인한 문서 부패(Model Drift)를 방지하기 위해, 핵심 구조 중심의 선별적 모델링과 **아키텍처 결정 레코드**(ADR)를 연계한 경량 형상 거버넌스 확립

#### 한줄 요약
- OMG 표준 다이어그램 분류와 관계 표기법의 시각화 원리를 바탕으로, 핵심 다이어그램 선별 모델링과 Diagram-as-Code 형상 관리를 적용하여 모델 드리프트를 방지하고 설계 정합성을 확보하도록 구축한다.
