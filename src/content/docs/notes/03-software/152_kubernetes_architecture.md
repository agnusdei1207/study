...
date: "2026-09-14T17:28:00+09:00"
tags:
  - "notes-software"
weight: 152
extra:
...
| 워커 노드 OOM 발생 시 핵심 데몬인 kubelet 프로세스 다운 | Kubelet 시스템 예약 메모리(`system-reserved`) 사전 격리 설정 | 워커 노드 안정성 보장 |
| 마스터 노드 장애 시 전체 클러스터 제어 불능 | Control Plane 다중화(Multi-Master HA) 및 리더 선출 체계 구축 | 마스터 SPOF 해소 |
...
- 클라우드 네이티브 컴퓨팅 및 현대 엔터프라이즈 인프라의 **분산 컨테이너 오케스트레이션의 사실상 표준**(De-facto Standard)으로 자리 잡았다.
- 실무 구축 시에는 **etcd 3/5대 홀수 쿼럼**(Quorum) **Multi-AZ 분산 및 고속 NVMe 스토리지 배치**, **kube-apiserver 수평 스케일아웃**, **워커 노드 OOM 시 에이전트 다운을 방지하는 kubelet `system-reserved` 자원 예약 격리**, **대규모 파드 스케줄링 병목을 해소하는 Karpenter 연동**을 결합하여 제어면 고가용성과 분산 탄력성을 확보해야 한다.

#### 한줄 요약
- 제어면과 작업 노드를 분리하고 선언적 상태 관리와 조정 루프를 통해 컨테이너 배포와 자가 치유를 자동화한다.
