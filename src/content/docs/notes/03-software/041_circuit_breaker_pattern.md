...
```text
[서킷 브레이커 상태 전이 흐름] (진행 ①→⑤, 실패율·시험 호출 결과에 따라 Closed ⇄ Open ⇄ Half-Open 순환)
  │
  ├─ [호출 인터셉터] (① 클라이언트 요청 가로채기, 현재 서킷 상태 판정)
  │
  ├─ [Closed 상태] (② 정상 시 원격 마이크로서비스 호출 실행, 슬라이딩 윈도우 실패율 50% 초과 시 Open 전이)
  │
  ├─ [Open 상태] (③ 차단 시 원격 호출 시도 없이 Fallback 즉시 실행(Fast-Fail), 쿨다운 10초 경과 시 Half-Open 전이)
  │
  ├─ [Half-Open 상태] (④ 복구 검증 시 제한된 N개 시험 호출 전송)
  │
  └─ [시험 호출 결과 판정] (⑤ 모두 성공 시 Closed 복귀, 실패 시 ③ Open 재진입)
```

분기 결과: 정상 갈래는 원격 왕복 지연을 그대로 치르는 대신 장애가 아닌 요청을 통과시키고, 차단 갈래는 왕복을 생략해 스레드 낭비를 0화하는 대신 정상일 수 있는 요청까지 실패시키며, 복구 판정은 시험 호출 표본 수에 따라 성급한 Closed 복귀(재차단 비용)와 장애 중 서비스로의 부하 재유입 사이 비용이 갈린다
...
#### 한줄 요약
- 3단계 유한 상태 기계와 슬라이딩 윈도우 집계 원리를 바탕으로, 격벽 격리와 정교한 Fallback을 적용하여 연쇄 장애를 차단하고 시스템 회복탄력성을 확보하도록 설계한다.
