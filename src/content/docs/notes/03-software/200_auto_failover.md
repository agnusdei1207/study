...
분기 결과: 펜싱 성공과 실패 갈래는 승격 진행 여부가 갈리며, 실패 시 온콜 수동 개입이라는 RTO 희생을 치르는 대신 이중 쓰기(스플릿 브레인)의 데이터 파괴 비용을 원천 차단한다.
...
- 클라우드 네이티브 및 분산 데이터베이스(Patroni, Orchestrator) 아키텍처에서 인적 개입 없는 무인 복구와 서비스 연속성을 달성하는 **가장 핵심적인 고가용성(HA) 자율 복구 기술**로 정립.
- 실무 구축 시에는 **'펜싱 선행 원칙(구 Primary 강제 차단 후 Standby 승격)'의 엄격한 준수**, **히스테리시스 임계치 기반 플래핑(Flapping) 오탐 방지**, **펜싱 실패 시 즉시 자동 승격을 중단하고 온콜 수동 개입으로 전환하는 안전 장치(Fail-Safe)**, **재접속 폭주를 방어하는 지수 백오프 및 지터(Jitter)**를 결합하여 복구 속도와 데이터 무결성을 완벽히 조화.
