...
date: "2026-09-14T17:27:00+09:00"
tags:
  - "notes-software"
weight: 151
extra:
...
- 하드웨어 가상화를 통한 독립된 Guest OS 및 커널 수준의 보안 격리(VM)
- 호스트 커널 공유를 통한 수 밀리초 단위 신속한 부팅 및 고밀도 집적(Container)
...
| 구성요소 | 책임 | 주요 특징 |
|:---|:---|:---|
| 가상머신 (VM) | 하이퍼바이저 기반으로 독립된 Guest OS 커널을 실행하여 하드웨어 격리 보장 | 강력한 커널 격리 |
...
```text
[워크로드 실행 환경 선택·기동] (진행 ①→⑤, 요청, 엔트리포인트 가동·트래픽 개시)
  │
  ├─ [요구 분석] (① 멀티테넌트 커널 격리가 필수인지, MSA 초고속 확장이 필요한지 분석)
  │
  ├─ [실행 모델 분기] (② 커널 분리가 필수이면 VM 경로, 경량 고밀도 배치이면 Container 경로)
  │
  ├─ [가상 하드웨어 할당] (③ VM 경로: vCPU·vRAM·vDisk 할당 후 Guest OS 커널 로딩, 수 분 소요)
  │
  ├─ [네임스페이스·cgroups 생성] (④ Container 경로: PID·NET 네임스페이스 및 메모리 상한 할당 후 OverlayFS 레이어 마운트, 수 밀리초)
  │
  └─ [애플리케이션 프로세스 가동] (⑤ 양 경로가 엔트리포인트 프로세스 가동 및 서비스 트래픽 개시로 수렴)
```

분기 결과: VM 경로는 독립 커널을 세우는 부팅 지연과 GB급 이미지 비용을 치르고 완전한 격리를 얻는 반면, Container 경로는 커널을 재사용해 밀리초 기동을 얻는 대신 공유 커널 취약점 위험을 치른다.
...
|:---|:---|:---|:---|
| 커널 격리 수준 | 독립 Guest OS 커널 격리| 호스트 OS 커널 공유 (공유 취약점)| 경량 독립 커널 탑재 (독립 커널 격리) |
...
#### 한줄 요약
- 엄격한 격리는 VM, 신속한 배포는 컨테이너, 서버리스 멀티테넌트는 Secure Container(MicroVM)를 선택한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 멀티테넌트 환경에서 컨테이너 탈출 해킹으로 타 테넌트 침해 | 테넌트 경계마다 VM 분리 또는 Kata Containers(MicroVM) 적용 | 커널 수준 격리 달성 |
| 수천 개 컨테이너 급증 시 호스트 커널 리소스 고갈 | Kubernetes Resource Request/Limit 및 cgroups 엄격 통제 | 단일 컨테이너의 호스트 자원 고갈 방지 |
| 리눅스 컨테이너 환경에서 윈도우 기반 레거시 앱 구동 불가 | Hyper-V 기반 Windows 가상머신(VM) 전용 인스턴스 구축 | OS 아키텍처 호환성 보장 |
...
- 하이브리드 클라우드 인프라 환경에서 **Kata Containers** 등 **경량 가상머신**(MicroVM) 및 **웹어셈블리**(Wasm) 차세대 런타임과의 융합을 통해 커널 보안 격리와 초경량 기동성을 동시에 달성하는 방향으로 진화하고 있다.
- 다중 테넌트 규제 준수가 요구되는 코어 원장에는 하드웨어 격리 기반 가상머신을 배치하고 무상태 MSA API 워크로드에는 OCI 컨테이너를 채택하는 계층적 분리 및 보안·성능 간 트레이드오프 통제 기준을 수립해야 한다.

#### 한줄 요약
- 워크로드의 보안 격리 요구와 확장 속도에 맞춰 VM과 컨테이너, 경량 가상머신을 계층적으로 조합하여 운영한다.
