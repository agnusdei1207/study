---
sidebar:
  order: 71
  label: "071. ATAM 아키텍처 트레이드오프 분석 방법"
  badge:
    text: "기출 · 50%"
    variant: note
title: "ATAM 아키텍처 트레이드오프 분석 방법 (Architecture Tradeoff Analysis Method)"
date: "2026-09-07T10:05:00+09:00"
tags:
  - "notes-software"
weight: 71
extra:
  question_no: "071"
  source_status: "기출"
  source_history: "131회"
  priority: 50
  priority_note: "131회 기출, 품질속성 절충 분석 절차"
---

## Ⅰ. 개요

<details><summary>용어 설명</summary>

- **ATAM(Architecture Tradeoff Analysis Method)**: 미국 SEI에서 개발한, 소프트웨어 아키텍처가 비즈니스 목표 및 품질 속성을 충족하는지 트레이드오프 관점에서 평가하는 방법론.
- **유틸리티 트리(Utility Tree)**: 전사 비즈니스 요구사항을 성능, 가용성, 보안 등 품질 속성별 세부 시나리오로 구조화한 트리.

</details>

- 정의/개념: 비즈니스 목표 달성을 위해 **유틸리티 트리(Utility Tree) 기반 품질 시나리오와 민감점·절충점(Trade-off)** 을 평가하는 아키텍처 검증 방법론
- 배경/필요성: 특정 품질 속성에 편향된 설계로 인한 **이질적 품질 속성 간 상충 저하 및 후반부 재구축 비용 폭증 한계**

#### 한줄 요약
- 유틸리티 트리와 시나리오를 바탕으로 아키텍처의 민감점, 절충점, 위험 요소를 체계적으로 분석한다.

## Ⅱ. 특징

<details><summary>용어 설명</summary>

- **Sensitivity vs Trade-off**: 단일 품질에 크게 영향을 주는 설계 결정(민감점)과 두 개 이상의 품질이 상충하는 설계 결정(절충점).
- **Risk vs Non-Risk**: 품질 목표 달성을 저해하는 잠재적 설계 결함(위험)과 긍정적으로 기여하는 안전한 설계 결정(비위험).

</details>

- 비즈니스 동인(Business Drivers)과 **유틸리티 트리 기반의 체계적 품질 분해**
- 아키텍처 4대 산출물(**Risk, Non-Risk, Sensitivity Point, Tradeoff Point**) 도출
- 아키텍트, 개발자, 고객 등 전 이해관계자가 참여하는 **합의 기반 시나리오 우선순위화**

#### 한줄 요약
- ATAM은 품질 간 충돌을 설계 확정 전에 드러내는 대신 이해관계자를 모으는 워크숍 비용을 요구하므로, 재구축 위험이 클수록 이득이 커진다.

## Ⅲ. 구조 및 구성요소

<details><summary>용어 설명</summary>

- **품질 시나리오 6요소**: 자극원(Source), 자극(Stimulus), 환경(Environment), 대상(Artifact), 응답(Response), 측정치(Measure).

</details>

```text
[ATAM 평가 분석 체계]
  │
  ├─ [비즈니스 동인] (Business Drivers)
  │
  ├─ [유틸리티 트리: Utility Tree]
  │     ├─ [성능] (응답시간·처리량 시나리오)
  │     ├─ [가용성] (장애 복구·Failover 시나리오)
  │     └─ [보안성] (데이터 암호화·접근 통제)
  │
  └─ [4대 평가 산출물]
        ├─ [민감점] (단일 속성 영향 아키텍처 결정)
        ├─ [절충점] (복수 품질 상충 의사결정)
        ├─ [위험점 Risk] (품질 저해 잠재적 결함)
        └─ [비위험점 Non-Risk] (품질 지원 안전 결정)
```
- 선의 의미: 계층 구조 및 상하위 포함 관계를 나타낸다.

| 구성요소 | 책임 |
|:---|:---|
| 유틸리티 트리 | 목표의 **품질 시나리오 구조화** |
| 민감점 | 단일 품질에 민감한 **설계 요소 식별** |
| 절충점 | 복수 품질의 **상충 결정 분석** |
| 위험·비위험 | 목표를 위협·지원하는 **설계 판정** |

#### 한줄 요약
- 유틸리티 트리가 추상적인 품질 목표를 검증 가능한 시나리오로 내려놓고, 그 시나리오를 통과시키는 설계 결정에서 민감점과 절충점이 드러난다.

## Ⅳ. 흐름도

<details><summary>용어 설명</summary>

- **ATAM 4단계 Phase**: Phase 0(준비 및 파트너십) $\to$ Phase 1(아키텍트 중심 1차 평가) $\to$ Phase 2(이해관계자 2차 평가) $\to$ Phase 3(결과 보고 및 후속 조치).

</details>

```text
[ATAM 평가 흐름] (진행 ①→⑤, 위험 테마 범주화·최종 개선 리포트 산출)
  │
  ├─ [도입 소개] (① 비즈니스 동인(Business Drivers)·후보 아키텍처 접근법 소개)
  │
  ├─ [유틸리티 트리 구성] (② 품질 시나리오 6요소 도출)
  │
  ├─ [우선순위 투표] (③ 중요도 x 난이도 High/Medium/Low 매트릭스 상위 선별)
  │
  ├─ [민감점·절충점 분석] (④ 상위 시나리오를 아키텍처 설계 결정에 대입·Trade-off 분석)
  │
  └─ [위험 테마 리포트] (⑤ 식별된 위험 요소를 Risk Themes로 범주화·개선 리포트 산출)
```

분기 결과: 시나리오 우선순위 투표가 갈래를 가르며, 상위 시나리오는 설계 대입 분석이라는 시간 비용을 치르고 민감점·절충점을 드러내지만, 하위 시나리오는 분석 비용을 아끼는 대신 미검증 품질 위험을 감수한다

#### 한줄 요약
- 시나리오 우선화에서 상위에 오른 것만 설계에 대입되므로, 우선순위 판정이 곧 어떤 절충점을 드러낼지를 미리 정하는 셈이다.

## Ⅴ. 종류 및 비교

<details><summary>용어 설명</summary>

- **SAAM vs ATAM vs CBAM**: 변경 용이성 중심의 SAAM, 다중 품질 절충 중심의 ATAM, 경제성/ROI 중심의 CBAM.

</details>

| 비교 항목 | SAAM (Software Architecture Analysis) | ATAM (Tradeoff Analysis) | CBAM (Cost-Benefit Analysis) |
|:---|:---|:---|:---|
| 핵심 평가 관점 | **모듈 변경 용이성 (Modifiability)** | **다중 품질 속성 간 상충 관계 절충** | **아키텍처 대안별 경제성 및 ROI** |
| 주요 산출물 | 변경 시나리오별 영향도 및 결합도 | **민감점, 절충점, 위험/비위험 요소** | **비용 대 편익 비율, 투자 우선순위** |
| 평가 기법 | 시나리오 기반 정성적 분석 | **유틸리티 트리 기반 정량/정성 절충** | 효용 점수화 및 경제적 가치 산정 |

#### 한줄 요약
- 단일 변경성은 SAAM, 다중 품질 절충은 ATAM, 경제성 투자는 CBAM을 선택한다.

## Ⅵ. 실무 고려사항 및 대책

<details><summary>용어 설명</summary>

- **Mini-ATAM**: 수 주가 걸리는 풀버전 대신, 핵심 시나리오 5~10개만 선별하여 1~2일 내에 끝내는 경량 아키텍처 평가.

</details>

| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 모호한 시나리오로 인한 주관적 평가 논쟁 | **6요소(자극원, 자극, 환경, 대상, 응답, 응답측정치) 표준 작성** | 수치화된 객관적 검증 기준 확립 |
| 특정 이해관계자의 발언권 독점으로 편향 발생 | **High/Medium/Low 브레인스토밍 및 스티커 투표(Dot Voting)** | 공정하고 중립적인 우선순위 도출 |
| 정규 ATAM 수행 기간(2~3주)의 일정 부담 | **핵심 비즈니스 시나리오 5개 중심의 Mini-ATAM 기법 적용** | 평가 기간 2일 이내 단축 및 신속 피드백 |
| 평가 후 도출된 위험 요소 방치 | **도출된 위험을 아키텍처 의사결정 기록(ADR) 및 백로그에 반영** | 후속 개발 시 위험 조기 완화 보장 |

#### 한줄 요약
- ATAM은 결정을 내려 주는 것이 아니라 절충점을 드러내는 데까지가 역할이므로, 평가 비용이 부담되면 Mini-ATAM으로 범위를 줄이되 도출된 절충점은 ADR로 남겨 판단 근거를 보존한다.

## Ⅶ. 결론

<details><summary>용어 설명</summary>

- **아키텍처 트레이드오프 분석 방법(Architecture Tradeoff Analysis Method, ATAM)**: 유틸리티 트리를 통해 상충하는 품질 속성 시나리오를 구체화하고 민감점과 절충점을 식별하는 아키텍처 평가 방법론.
- **비용 편익 분석 방법(Cost Benefit Analysis Method, CBAM)**: ATAM의 후속 단계로 아키텍처 전략별 구현 비용과 효용을 정량화하여 투자 대비 수익을 극대화하는 의사결정 모델.
- **아키텍처 의사결정 기록(Architecture Decision Record, ADR)**: 아키텍처 설계 과정에서 합의된 기술적 절충점과 결정 배경을 코드 저장소에 영구 기록하는 문서 체계.

</details>

- **경량 Mini-ATAM 도입 및 경제성 모델(CBAM) 결합 진화**: 전통적 중량급 평가였던 **아키텍처 트레이드오프 분석 방법(ATAM)**은 클라우드 네이티브 및 마이크로서비스 환경에서 스프린트 내 1~2일 내로 핵심 시나리오를 검증하는 경량 Mini-ATAM으로 발전하고 있으며, 경제적 투자 가치를 계량화하는 **비용 편익 분석 방법(CBAM)**과 연계되는 추세.
- **유틸리티 트리 기반 정량화와 ADR 자산화 결단**: 품질 속성 6요소(자극원·자극·환경·대상·응답·응답측정치)로 주관성을 배제하고, 평가를 통해 발굴된 민감점과 절충점을 **아키텍처 의사결정 기록(ADR)** 및 기술 부채 백로그에 명문화하여 설계 왜곡을 방지하는 아키텍처 거버넌스 확보 필요.

#### 한줄 요약
- ATAM은 Mini-ATAM과 CBAM 연계를 통해 경제성과 신속성을 겸비한 평가로 진화하고 있으며, 6요소 시나리오 정량화와 ADR 자산화가 핵심이다.
