...
- **순차 정렬 PK 확보와 Key Lookup 완전 제거 결단**: 무작위 UUID로 인한 디스크 단편화와 페이지 분할을 방지하기 위해 시간 정렬 식별자(TSID)를 클러스터드 기본키로 강제하고, 대량 빈발 트래픽 쿼리는 `SELECT *`를 배제한 채 INCLUDE 컬럼 기반 커버링 인덱스를 적용하여 무작위 **키 룩업**(Key Lookup)을 원천 차단하는 공학적 I/O 최적화 결단

#### 한줄 요약
- 인덱스 최적화는 컬럼형 하이브리드 엔진으로 진화하고 있으며, TSID 기반 클러스터링과 커버링 인덱스를 통한 Key Lookup 배제 체계 구축
