...
date: "2026-09-14T16:51:00+09:00"
tags:
  - "notes-software"
weight: 94
extra:
...
#### 한줄 요약
- 물리적 순차 I/O 활용과 2차 테이블 룩업 제거를 통해 대용량 조회 성능을 향상시킨다.
...
|:---|:---|:---|
| 클러스터드 인덱스 | 데이터 행을 키 순서대로 디스크에 정렬하여 범위 검색(BETWEEN, <, >) 속도 향상 | 리프 노드가 곧 실제 데이터 페이지 (테이블당 1개) |
...
[커버링 인덱스 판정 경로] (진행 ①→④, ③·④ 분기 후 결과 반환)
  ...
  │
  ├─ [컬럼 대조] (② SELECT·WHERE의 모든 컬럼이 인덱스 키에 포함 여부 판정)
  │
...
| 쿼리 최적화 효과 | 범위 검색, PK 기반 단건 조회 | 특정 조건 탐색 가속 | 초고빈도 대규모 조회 성능 향상 |
...
| `SELECT *` 남발로 인한 커버링 인덱스 무력화 | 필요한 컬럼만 명시적으로 SELECT하는 쿼리 작성 원칙 준수 | 커버링 인덱스 온전한 활용 |

#### 한줄 요약
- 순차 PK 설계, INCLUDE 컬럼 활용, `Using index` 모니터링, `SELECT *` 금지로 성능을 향상시킨다.
...
- **컬럼형(Columnar) 스토리지 및 하이브리드 엔진 결합 진화**: 전통적 행 기반 클러스터드 및 커버링 인덱스는 대규모 분석 워크로드를 흡수하는 열 지향 스토리지 엔진(DuckDB, ClickHouse) 및 PostgreSQL 하이퍼테이블과 융합되어, 디스크 읽기 범위를 대폭 줄이고 메모리 캐시 적중률을 높이는 형태로 진화 추세
- **순차 정렬 PK 확보와 Key Lookup 완전 제거 결단**: 무작위 UUID로 인한 디스크 단편화와 페이지 분할을 방지하기 위해 시간 정렬 식별자(TSID)를 클러스터드 기본키로 강제하고, 대량 빈발 트래픽 쿼리는 `SELECT *`를 배제한 채 INCLUDE 컬럼 기반 커버링 인덱스를 적용하여 무작위 **키 룩업**(Key Lookup)을 차단하는 공학적 I/O 최적화 결단

#### 한줄 요약
- 순차 정렬 키를 통한 클러스터드 인덱스와 질의 컬럼을 포괄하는 커버링 인덱스를 결합하여 Key Lookup 랜덤 I/O를 배제하고 질의 처리 성능을 최적화한다.
