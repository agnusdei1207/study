...
```text
[가상화 기술 선정 흐름] (진행 ①→④, 이종 OS·격리 필수 시 VM 계열·동일 커널 고밀도 우선 시 컨테이너 계열로 갈림)
  │
  ├─ [가상화 수단 판정] (① 이종 OS 구동 또는 완전한 하드웨어 격리 필수 여부 판정, 충족 시 VM 가상화 채택)
  │
  ├─ [게스트 커널 수정 판정] (② VM 진입 시 커널 수정 가능 여부 판정)
  │     ├─ [반가상화] (② 커널 수정 가능 시 하이퍼콜 직결 채택(Xen))
...
분기 결과: 이종 OS·완전 격리 갈래는 게스트 OS 커널을 통째로 복제해 부팅 시간과 메모리 비용을 치르고 그 안에서 커널 수정이 가능하면 **하이퍼콜** 직결로 I/O 성능을 얻는 대신 커널 유지보수 의존성을 지며, 동일 커널 갈래는 격리 깊이를 내주고 밀도·기동 속도를 얻되 멀티테넌트 신뢰 경계가 필요하면 **마이크로VM**으로 되돌려 밀도 대신 하드웨어 격리를 다시 사들인다
