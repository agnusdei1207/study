...
date: "2026-09-14T17:12:00+09:00"
tags:
  - "notes-software"
weight: 126
extra:
...
- 객체 스토리지 파일 기반의 ACID 트랜잭션 및 스냅샷 격리 보장
...
| 객체 스토리지 (Storage) | AWS S3 등에 불변(Immutable) Parquet 데이터 파일과 변경 로그 영구 저장 | 저비용 대규모 확장 |
...
| 트랜잭션 및 ACID| 엔진 내부 완전한 ACID 지원 | 트랜잭션 미지원 (일관성 없음) | 객체 스토리지 위에서 ACID 트랜잭션 보장|
| 스토리지 및 포맷| 전용 독점 포맷 (고비용) | 오픈 포맷 Parquet/JSON (저비용)| 오픈 테이블 포맷 (Delta/Iceberg/Parquet) |
| 쿼리 엔진 호환성| 단일 벤더 전용 엔진 종속 | 다양한 오픈소스 엔진 연동 | Spark, Trino, Python 등 다양한 엔진 직접 쿼리 |
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 자잘한 파티션 파일 누적으로 인한 쿼리 성능 급락 | `OPTIMIZE` (Bin-packing 컴팩션) 작업으로 512MB 파일 병합 | 메타데이터 조회 및 스캔 속도 개선 |
...
| 다중 엔진 동시 쓰기 시 커밋 충돌(Conflict) 발생 | 파티션 세분화 및 멱등 MERGE 연산으로 충돌 범위 격리 | 동시 쓰기 성공률 제고 |
...
- 현대 엔터프라이즈 데이터 엔지니어링 및 생성형 AI 인프라의 **차세대 통합 데이터 플랫폼 아키텍처**로 자리 잡았다.
- 실무 구축 시에는 **오픈 테이블 포맷**(Apache Iceberg/Delta Lake) 기반의 **단일 데이터 원천**(SSOT) 수립, **소형 파일 성능 저하 방지 정기 `OPTIMIZE` 컴팩션 및 과거 스냅샷 비용 통제 `VACUUM` 수명주기 거버넌스**, **동시 쓰기 충돌 방어 낙관적 동시성 제어**(`OCC`) 튜닝 결합을 통한 고성능과 비용 효율성을 동시에 확보해야 한다.

#### 한줄 요약
- 객체 스토리지의 경제성과 오픈 테이블 포맷의 ACID 정합성을 결합하고 컴팩션·진공 관리를 통해 분석 성능과 비용 효율성을 동시에 확보한다.
