...
date: "2026-09-14T17:31:00+09:00"
tags:
  - "notes-software"
weight: 155
extra:
...
| SSL/TLS 인증서 갱신 누락으로 인한 서비스 접속 불가 사고 | `cert-manager` 도입 및 Let's Encrypt 자동 갱신 파이프라인 구축 | 인증서 만료 장애 방지 |
| Ingress 갱신 시 iptables 수만 개 규칙으로 인한 성능 저하 | kube-proxy 모드를 `iptables`에서 `IPVS` 모드로 전환 운영 | 대규모 클러스터 패킷 처리 성능 향상 |
...
- 클라우드 네이티브 마이크로서비스(MSA) 네트워킹 및 외부 트래픽 유입 제어의 **핵심 표준 라우팅 아키텍처**로 자리 잡았다.
- 실무 구축 시에는 **불필요한 네트워크 홉**(Hop)**을 제거하는 AWS ALB `target-type: ip` 직접 라우팅**, **대규모 파드 환경에서 iptables 부하를 극복하는 IPVS/eBPF**(Cilium) **기반 kube-proxy 가속**, **Let's Encrypt 인증서 생명주기를 자동화하는 cert-manager 연동 및 차세대 Gateway API로의 점진적 진화**를 결합하여 고성능 통신과 보안 거버넌스를 확보해야 한다.

#### 한줄 요약
- L4 가상 IP 로드밸런싱과 L7 호스트·경로 라우팅을 결합하여 마이크로서비스 간 통신 추상화와 외부 진입점 제어를 일원화한다.
