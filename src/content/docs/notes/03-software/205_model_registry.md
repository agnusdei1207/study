...
date: "2026-09-14T18:07:00+09:00"
tags:
  - "notes-software"
weight: 205
extra:
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 단일 개발자가 검증되지 않은 모델을 운영에 임의 배포 | `RBAC` 역할 분리 및 **4-Eyes Principle**(2인 승인) 워크플로우 강제 | 무단 배포 사고 방지 |
| 데이터셋 출처가 누락되어 운영 모델 결함 원인 규명 불가 | `Artifact Hash + Git Commit + DVC 해시` 계보(Lineage) 필수 결속 | 모델 추적성 및 재현성 확보 |
...
- AI 윤리 및 금융/의료 등 엔터프라이즈 AI 규제 컴플라이언스를 충족하고 모델 배포 수명주기를 완벽히 통제하는 **가장 핵심적인 MLOps 모델 자산 관리 및 배포 거버넌스 허브**로 정립되었다.
- 실무 구축 시에는 **MLflow/SageMaker 기반 모델 아티팩트 불변 등록**, **Git 커밋/DVC 데이터셋/피처스토어를 1:1로 결속하는 계보 그래프**(Lineage Graph), **2인 승인**(4-Eyes Principle) **워크플로우를 통한 무단 배포 차단**, **`champion` 논리적 별칭을 활용한 즉각적인 롤백 및 카나리 서빙 연동**을 결합하여 안정적인 AI 서비스 딜리버리를 달성해야 한다.

#### 한줄 요약
- 불변 버전 관리, 계보 추적, 2인 승인 거버넌스를 통해 검증된 AI 모델만을 안전하게 배포하는 MLOps 핵심 인프라 체계를 구축한다.
