...
date: "2026-09-14T17:14:00+09:00"
tags:
  - "notes-software"
weight: 128
extra:
...
- 배경/필요성: 기존 Hive 메타스토어의 디렉터리 기반 파일 목록 조회(`LIST`) 의존으로 인한 **디렉터리 스캔 I/O 병목 및 파티션 변경 시 테이블 전체 재작성 한계**
...
- Spark, Trino, Flink, Snowflake, BigQuery 등 다양한 연산 엔진에 대한 높은 중립성

#### 한줄 요약
- 메타데이터 수준의 세밀한 통계와 엔진 중립성을 바탕으로 페타바이트급 테이블을 효율적으로 관리한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 빈번한 커밋으로 인한 수천 개의 Manifest File 누적 | `rewrite_manifests()` 연산으로 Manifest 파일 정기 병합 | 메타데이터 조회 성능 개선 |
...
- 엔터프라이즈 멀티 엔진 레이크하우스 및 클라우드 분석 플랫폼의 **표준적인 오픈 테이블 포맷**(Open Table Format)으로 자리 잡았다.
- 실무 운영 시에는 **매니페스트 탐색 지연 제거**(`rewrite_manifests()`), **소형 파일 I/O 최적화**(`rewrite_data_files()`), **스토리지 비용 절감 스냅샷 만료**(`expire_snapshots()`) 및 **고아 파일 삭제**(`remove_orphan_files()`) 정기 유지보수 자동화 결합을 통해 대규모 데이터의 쿼리 성능과 비용 효율성을 동시에 확보해야 한다.

#### 한줄 요약
- 3계층 매니페스트 트리와 숨겨진 파티셔닝을 바탕으로 다중 엔진 중립성을 보장하고 주기적 파일 재작성을 통해 대규모 데이터 레이크하우스의 쿼리 성능을 확보한다.
