...
date: "2026-09-14T16:44:00+09:00"
tags:
  - "notes-software"
weight: 85
extra:
...
[트랜잭션 생명주기] (진행 ①→④, ③·④ 분기 완료)
  ...
  │
  ├─ [Partially Committed] (② 마지막 SQL 연산 완료 후 무결성 제약조건·외래키 검증)
  │
  ├─ [Committed] (③ 제약 위반·데드락 없음 시 Redo Log 디스크 플러시(fsync) 후 영구 반영 확정)
  │
  └─ [Failed · Aborted] (④ 제약조건 위반·데드락 오류 발생 시 Undo Log 역순 실행으로 완전 롤백)
```

분기 결과: 정상 경로는 커밋마다 로그 플러시 1회라는 고정 비용을 치르는 반면 실패 경로는 변경량 전체를 역순으로 되감아야 하므로, 오류가 뒤늦게 발견될수록 롤백 비용이 커져 검증을 앞당기는 것 자체가 비용 절감이 된다
...
- **트랜잭션 격리 경계 최소화와 분산 정합성 트레이드오프 결단**: 외부 API 호출 및 대량 I/O를 트랜잭션 스코프 밖으로 엄격히 분리하여 커넥션 고갈을 방지하고, 마이크로서비스 간 분산 상태 전이는 2PC의 가용성 저하를 대체하는 **사가 패턴**(Saga Pattern)과 트랜잭셔널 아웃박스(Transactional Outbox)로 완화하는 아키텍처적 결단

#### 한줄 요약
- Undo/Redo 로깅과 격리성 제어 메커니즘을 바탕으로 로컬 트랜잭션 범위를 최소화하고, 분산 환경에서는 Saga 패턴과 결합하여 데이터의 신뢰성과 시스템 처리량을 균형 있게 확보한다.
