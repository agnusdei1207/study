...
date: "2026-09-14T17:07:00+09:00"
tags:
  - "notes-software"
weight: 116
extra:
...
#### 한줄 요약
- 대규모 1회성 배치는 MapReduce, 고속 반복 연산과 실시간 처리는 Spark를 선택한다.
...
- 빅데이터 분산 컴퓨팅의 효시이자 대규모 배치 아카이빙 및 ETL의 **기초 프레임워크**로 자리 잡았다.
- 현대 데이터 엔지니어링 실무에서는 **단순 반복 디스크 배치는 점진적으로 인메모리 Spark로 전환**, **HDFS/오브젝트 스토리지 기반의 저비용 대용량 영속성 계층 활용**, **Shuffle 네트워크 병목을 완화하는 Combiner 및 Key Salting 설계**, **NameNode 메모리 고갈을 방지하는 Small File 병합**(HAR/Parquet)을 결합하여 배치 시스템의 처리 효율을 확보해야 한다.

#### 한줄 요약
- HDFS 블록 복제와 MapReduce 데이터 지역성 연산을 결합하고 셔플 병목을 제어하여 대용량 배치 처리의 안정성을 확보한다.
