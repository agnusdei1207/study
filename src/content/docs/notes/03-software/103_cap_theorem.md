...
date: "2026-09-14T16:58:00+09:00"
tags:
  - "notes-software"
weight: 103
extra:
...
- 배경/필요성: 네트워크 지연과 패킷 유실이 필연적인 분산 환경에서 **일관성(C)·가용성(A)·분할내성(P) 동시 만족의 수학적·물리적 한계**
...
[분산 노드 간 네트워크 분할 감지] (진행 ①→④, CP·AP로 갈린 뒤 복구·병합 경로로 수렴)
  ...
  │
  ├─ [CP 시스템 경로] (② 과반수 미달, 요청 거부(Fail-Fast)로 데이터 불일치 방지)
  │
...
- **근본적 아키텍처 제약 원칙 확립**: 글로벌 분산 컴퓨팅 및 클라우드 네이티브 데이터베이스 설계를 위한 아키텍처 제약 원칙 정립
- **상황별 최적 데이터 신뢰성 확보**: 정합성 중심 도메인 CP 시스템 및 고가용성 중심 도메인 AP 시스템 배치와 PACELC 기반 Tunable Consistency 및 Local Quorum 최적화 결합

#### 한줄 요약
- 네트워크 분할 내성(P)을 전제로 비즈니스 요구에 따라 CP와 AP 모델을 선별하고, PACELC 사상에 기반한 정족수(Quorum) 조정을 통해 일관성과 가용성의 균형을 확보한다.
