...
date: "2026-09-14T16:43:00+09:00"
tags:
  - "notes-software"
weight: 83
extra:
...
- 배경/필요성: 팀별 개별 인프라 구축에 따른 **아키텍처 파편화, 보안 설정 누락 및 초기 환경 구성(Scaffolding)의 시간 지연 한계**
...
- **Opt-in(자발적 채택)**: 강제로 표준을 강요하지 않고, 편의성이 검증된 개발자 경험(DX)을 제공하여 자발적으로 골든 패스를 선택하게 만듦.
- **Escape Hatch(탈출구)**: 머신러닝/빅데이터 등 골든 패스로 수용하기 힘든 특수 아키텍처에 대해 안전하게 로우레벨 제어를 허용하는 예외 경로.

</details>

- 강제 통제가 아닌 우수한 개발자 경험(DX)을 통한 **자발적 채택(Opt-in) 유도**
...
[골든 패스 첫 배포 경로] (진행 ①→⑤, 3분 내 첫 배포 완료)
  ...
  │
  └─ [즉시 배포] (⑤ K8s 개발 네임스페이스로 3분 내 첫 배포 완료, TTFC 대폭 단축)
```
...
|:---|:---|:---|
| 채택 방식 | **자발적 채택 (Opt-in, 높은 편의성 기반 유도)**| 무조건적 강제 (Enforced, 예외 불허) |
...
| 템플릿 생성 후 설정 드리프트(Drift) 발생 | **GitOps(ArgoCD) 기반 소스코드와 인프라의 단일 진실 공급원 유지** | 형상 불일치 해소 |
...
- **자발적 채택(Opt-in) 유도와 템플릿 지속 진화 거버넌스 결단**: 강제적 규제(Paved Road)가 아닌 실질적인 생산성 혜택을 제공하여 자발적 진입을 유도하되, 템플릿 노후화(Path Stagnation)를 방지하기 위한 지속적 패치 파이프라인과 탈출구(Escape Hatch) 피드백을 수렴하는 플랫폼 수명주기 거버넌스 확보

#### 한줄 요약
- 검증된 표준 템플릿과 가드레일을 결합한 골든 패스를 통해 자발적 채택을 유도하고, 탈출구(Escape Hatch) 환류 체계를 운영하여 개발 생산성과 인프라 표준화를 조화롭게 달성한다.
