...
date: "2026-09-14T16:16:00+09:00"
tags:
  - "notes-software"
weight: 54
extra:
...
- **소규모 배치(Small Batch)** 단위의 고빈도 배포를 통한 장애 파급 범위(Blast Radius) 최소화
...
```text
[지속적 배포 카나리 흐름] (진행 ①→⑥, 가드레일 지표 충족 여부가 승격·롤백을 갈림)
  │
  ├─ [커밋 푸시] (① 개발자 Git 커밋이 CI 파이프라인·품질 게이트 통과)
  │
  ├─ [자동 배포] (② ArgoCD가 프로덕션 K8s 클러스터에 신규 버전 컨테이너 자동 배포)
  │
  ├─ [카나리 라우팅] (③ Argo Rollouts가 전체 트래픽의 5%를 신규 카나리 Pod로 라우팅)
  │
  ├─ [지표 수집] (④ Prometheus가 5분간 카나리 Pod의 에러율 및 지연시간 수집)
  │
  ├─ [가드레일 판정] (⑤ 에러율이 임계치(0.1% 이하) 충족 시 승격, 급증 시 Auto-Rollback 실행)
  │
  └─ [승격·복구] (⑥ 충족 갈래는 트래픽을 20%에서 100%로 점진 확대 후 승격 완료, 미충족 갈래는 신규 Pod 즉시 제거 후 구버전 100% 복구)
```

분기 결과: 가드레일 판정에서 갈리므로, 임계치(0.1%) 충족 갈래는 단계 승격으로 수동 승인 없는 리드타임을 얻는 반면, 미충족 갈래는 즉각 Auto-Rollback으로 장애 파급 비용을 수 초 내에 차단한다
...
- **차세대 발전 전망**: 서비스 메시 기반 정밀 트래픽 제어 및 **오류 예산**(Error Budget) 소진율 연동 잔여 예산 기반 프로덕션 릴리즈 승격 자동 결정 AI 자율형 무인 배포 체계 진화
- **실무 공학적 통찰**: **서비스 수준 목표**(SLO) 기반 실시간 가드레일 지표 계측과 데이터베이스 하위 호환성 보장 **확장-축소**(Expand-Contract) 마이그레이션 거버넌스 확립을 통한 무인 자동 배포 신뢰성 확보

#### 한줄 요약
- 실시간 SLO 가드레일과 Expand-Contract 데이터베이스 마이그레이션을 연계하여, 무인 자동 배포 환경의 서비스 안정성을 확보한다.
