...
date: "2026-09-14T16:05:00+09:00"
tags:
  - "notes-software"
weight: 49
extra:
...
#### 한줄 요약
- DDD의 핵심은 패턴 목록이 아니라 모델과 코드가 같은 언어를 사용하게 하는 규율이므로, 용어 번역이 필요해지는 순간 이미 모델이 어긋난 것으로 본다.
...
- **전술적 설계(Aggregate Root, Entity, VO)** 를 통한 도메인 불변식(Invariant)의 정밀한 캡슐화
...
```text
[DDD 설계 흐름] (진행 ①→⑤, 탐색·언어·경계·전술 설계 순으로 확정)
  │
  ├─ [이벤트 스토밍] (① 도메인 이벤트를 시간순으로 나열하는 워크숍 수행)
  │
...
분기 결과: 이벤트 스토밍이 먼저 오는지가 흐름의 갈림으로, 언어·경계가 조직도나 기술 구조를 따라 그어지면 후속 전술 설계 전체를 다시 그어야 하는 재작업 비용이 발생한다
...
| Aggregate 크기가 너무 커서 락(Lock) 경합 발생 | Aggregate 최소화 및 타 Aggregate는 ID로만 참조 | 동시성 처리량 향상 및 트랜잭션 범위 최소화 |
...
- **차세대 발전 전망**: 마이크로서비스 및 이벤트 주도 아키텍처(EDA)의 도메인 분할 표준으로 안착하며, LLM 기반 이벤트 스토밍 시나리오 분석 및 보편적 언어 사전 자동화와 결합되어 모델링 생산성 증대
- **실무 공학적 통찰**: 모든 영역에 전술적 패턴을 강제하는 오버엔지니어링을 지양하고, 비즈니스 차별화를 좌우하는 **핵심 도메인**(Core Domain)에 역량을 집중하며 외부 레거시 연동 시 **부패 방지 계층**(ACL)을 엄격히 배치하는 전략적 기준 확립

#### 한줄 요약
- 보편적 언어와 바운디드 컨텍스트의 경계 분리 원리를 바탕으로, 애그리게이트 루트 캡슐화와 부패 방지 계층(ACL)을 적용하여 모델 오염을 방지하고 도메인 불변식을 확보하도록 설계한다.
