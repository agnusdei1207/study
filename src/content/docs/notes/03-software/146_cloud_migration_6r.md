...
date: "2026-09-14T17:23:00+09:00"
tags:
  - "notes-software"
weight: 146
extra:
...
|:---|:---|:---|:---|
| 코드 수정 범위 | 무수정 (코드 변경 없음) | 부분 수정 (DB/연동 설정) | 전면 재설계 (MSA/컨테이너) |
| 이관 소요 기간 | 수일 ~ 수주 (가장 빠름) | 수주 ~ 수개월 (중간) | 수개월 ~ 수년 (장기화) |
| 클라우드 이점 활용| 최소 (IaaS 비용 절감 위주)| 중간 (관리형 PaaS 운영 절감) | 최대 (오토스케일링/탄력성 확보)|
| 최적 적용 대상 | 데이터센터 임대 만료 긴급 이전| DB 운영 부담 해소가 필요한 앱 | 핵심 고객 서비스, 트래픽 폭증 앱|

#### 한줄 요약
- 신속성은 Rehost, 운영 효율은 Replatform, 확장성 극대화는 Refactor를 선택한다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 모든 시스템을 무리하게 MSA로 전면 재개발하려다 예산 초과 | Rehost로 일단 신속 이관 후 2단계로 Refactor 분할 추진 | 프로젝트 리스크 분산 및 성공률 제고 |
| 파악되지 않은 레거시 통신 의존성 누락으로 이관 후 장애 | Discovery Service 기반 정밀 네트워크 의존성 맵 사전 분석 | 숨은 의존성 결함 방지 |
...
- 엔터프라이즈 디지털 전환 및 대규모 데이터센터 철수(DC Exit) 프로젝트의 **체계적인 마이그레이션 의사결정 프레임워크**로 자리 잡았다.
- 실무 구축 시에는 **우선 Rehost로 신속히 탈출 후 클라우드 상에서 점진적 Refactor/Modernization을 추진하는 2단계**(Phased) **전략**, **네트워크 숨은 의존성을 규명하는 자동 Discovery 실사**, **데이터베이스 무중단 컷오버를 보장하는 CDC**(AWS DMS) **실시간 동기화**, **이관 후 비용 낭비를 차단하는 FinOps Right-Sizing**을 결합하여 무중단 전환과 클라우드 네이티브 현대화를 달성해야 한다.

#### 한줄 요약
- 비즈니스 가치와 기술 복잡도를 평가하여 6R 전략을 선별하고 Wave 단위 CDC 컷오버를 적용하여 마이그레이션 리스크와 다운타임을 최소화한다.
