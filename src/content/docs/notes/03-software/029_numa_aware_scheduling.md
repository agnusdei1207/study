...
#### 한줄 요약
- 프로세서와 메모리 노드의 물리 토폴로지를 일치시켜 로컬 메모리 접근 비율을 높인다.
...
| 주 적용처 | 범용 애플리케이션 | 저지연 주식 거래, 전용 DB 인스턴스 | 대규모 공유 메모리 DBMS, 빅데이터 연산 |
...
- 실무 고성능 환경에서는 **DBMS/저지연 애플리케이션의 `numactl`(--cpunodebind/--membind) 물리 고정, 가상화 환경의 vNUMA 토폴로지 노출, 대규모 분산 메모리 워크로드의 `numactl --interleave=all` 및 `vm.zone_reclaim_mode=0` 튜닝**을 결합한 메모리 대역폭 향상 체계 구축

#### 한줄 요약
- First-Touch 정책과 메모리 지역성 원리를 바탕으로, numactl 바인딩과 vNUMA 토폴로지 노출을 적용하여 원격 인터커넥트 병목을 방지하고 메모리 접근 처리량을 향상하도록 구축한다.
