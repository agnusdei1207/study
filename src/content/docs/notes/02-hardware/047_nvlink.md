...
sidebar:
  ...
  badge:
    ...
    variant: note
title: "NVLink"
date: "2026-09-14T14:52:00+09:00"
tags:
  - "notes-hardware"
weight: 47
extra:
...
- **NVLink**: NVIDIA가 개발한 GPU-to-GPU 및 GPU-to-CPU 고속 전용 양방향 직렬 인터커넥트 기술로, PCIe의 대역폭 한계와 지연시간을 극복.
- **NVSwitch**: 단일 서버 내부 또는 다중 노드에 탑재된 수십~수백 개의 GPU를 풀메시(Full-Mesh) 비차단 크로스바로 연결하는 전용 고속 스위칭 ASIC.

</details>

- 정의/개념: GPU 간 직접 메모리 접근(P2P)과 대규모 데이터 전송을 위해 PCIe 버스 병목을 극복한 **고대역폭·저지연 독자 인터커넥트 기술**
- 배경/필요성: 표준 PCIe 버스의 대역폭 한계 및 호스트 CPU 경유 통신으로 인한 **GPU 간 텐서 동기화 지연과 분산 학습 확장성 병목 한계 극복 및 GPU 간 직접 메모리 공유 체계 확립 필요성 증대**

#### 한줄 요약
- NVLink는 GPU 간 직접 메모리 접근(P2P)과 초당 수 테라바이트 대역폭을 제공하여 단일 노드 내 다중 GPU를 거대 단일 GPU처럼 묶는 고속 인터커넥트이다.
...
- 고대역폭 및 저지연: 세대별 발전을 통해 GPU당 수백 GB/s~수 TB/s(NVLink 5세대 기준 **1.8 TB/s**) 대역폭과 수십 나노초대 지연시간 달성
...
#### 한줄 요약
- 1.8 TB/s 고대역폭과 NVSwitch 풀메시 크로스바를 통해 GPU 메모리를 통합하고 집단 통신을 하드웨어로 가속한다.

## Ⅲ. 구조 및 구성요소

<details><summary>용어 설명</summary>

- **SerDes(Serializer/Deserializer)**: 병렬 데이터를 고속 직렬 비트 스트림으로 변환하여 물리 케이블 및 기판 패턴으로 고속 전송하는 물리 계층 송수신기.
- **NCCL(NVIDIA Collective Communications Library)**: NVLink 및 NVSwitch 하드웨어 토폴로지를 자동 인식하여 All-Reduce 집단 통신 성능을 제고하는 소프트웨어 라이브러리.
...
[NVLink 노드 스케일업 아키텍처]
  ...
  │     ├─ [고속 SerDes PHY] (PAM4 직렬 고주파 인터페이스)
  │     └─ [SHARP 연산 가속기] (All-Reduce 인네트워크 연산)
  │
  ├─ [NVLink 직렬 연결망] (고속 P2P 양방향 데이터 경로)
  │
...
```text
[NVLink 집단 통신(All-Reduce) 경로] (진행 ①→⑤, 스캔·전송·라우팅·브리징·동기화)
  │
  ├─ [NCCL 런타임] (① 물리 토폴로지 스캔 및 링·트리 집단 통신 경로 산출)
  │
  ├─ [GPU NVLink P2P 컨트롤러] (② CPU 우회 및 상대 GPU HBM으로 그래디언트 직결 전송)
  │
  ├─ [NVSwitch (SHARP 가속기)] (③ 비차단 크로스바 라우팅 및 스위치 내 부분합 집계)
  │
  ├─ [InfiniBand RoCE RDMA HCA] (④ 멀티 노드 구간 스케일아웃 패브릭 변환 브리징)
  │
  └─ [전체 참여 GPU] (⑤ 그래디언트 동기화 완료 후 가중치 갱신 및 순전파 재개)
```

분기 결과: 노드 내부 GPU 간 통신은 NVLink와 NVSwitch를 통해 고속으로 완료되며, 노드 간 통신은 **InfiniBand RDMA**와 연계되어 스케일아웃 확장된다.
...
| 핵심 특징 | GPU당 **1.8 TB/s 고대역폭**, NVSwitch 비차단 풀메시 크로스바, SHARP 인스위치 집단 연산 | 개방형 표준 인터페이스, 범용 장치 호환성, 단방향 64~128 GB/s 대역폭 | PCIe 물리 계층 공유, 바이트 단위 캐시 일관성 지원, 메모리 용량 동적 확장 | 멀티 노드 스케일아웃 패브릭, 커널 바이패스 RDMA 지원, 저지연 패킷 스위칭 |
...
#### 한줄 요약
- 단일 노드 내 고속 GPU 스케일업에는 NVLink가 널리 쓰이며, 노드 간 대규모 클러스터 스케일아웃 네트워킹에는 InfiniBand가 표준이다.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| All-Reduce 통신 대기로 연산 유휴 | **통신-연산 오버랩** 및 텐서 버킷팅 | 지연시간 **은닉** |
| 8개 GPU 고밀도 집적으로 발열 급증 | **칩 직접 수랭식(Liquid Cooling)** 구축 | 열 스로틀링 방지 및 **클록 유지** |
| 멀티 노드 확장 시 대역폭 불균형 | **노드 내 텐서 병렬화** 및 노드 간 파이프라인 분할 | 클러스터 **확장 효율 90% 이상 유지** |
...
- **랙 스케일 대규모 패브릭 진화**: 단일 서버 노드를 넘어 랙 전체 72개 가속기를 액체 냉각 구리선으로 직결하는 **엔비디아 NVL72**(NVLink 72) 기반 랙 스케일 단일 도메인 확장
- **실무 계층적 토폴로지 설계 통찰**: 고빈도 통신이 요구되는 **텐서 병렬화**(Tensor Parallelism) 축을 NVLink 도메인 내부에 격리 배치하고 노드 간 스케일아웃 네트워크를 결합하는 하이브리드 분산 토폴로지 설계 체계 구축

#### 한줄 요약
- NVL72 기반 랙 스케일 가속 패브릭 진화 및 텐서 병렬화 축 격리 계층적 분산 토폴로지 구축
