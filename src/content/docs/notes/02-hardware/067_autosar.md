...
#### 한줄 요약
- AUTOSAR는 하드웨어와 응용 소프트웨어를 계층적으로 분리하여 전장 소프트웨어의 재사용성과 부품사 간 상호운용성을 확보한다.
...
- 모델 주도 개발(MDD): 표준 **ARXML** 명세서를 기반으로 RTE 및 BSW 소스 코드를 자동 생성하여 개발 생산성 향상

#### 한줄 요약
- VFB 추상화로 하드웨어 종속성을 완화하고, Classic/Adaptive 이원화와 ARXML 표준 명세로 전장 개발을 표준화한다.
...
- **복합 디바이스 드라이버(Complex Device Driver, CDD)**: 복잡한 센서나 엄격한 저지연 타이밍 제어가 필요하여 표준 BSW/MCAL 계층을 우회하고 MCU 하드웨어 레지스터를 직접 다루는 특수 드라이버.
...
분기 결과: 모델링 변경 시 ARXML 수정과 코드 재생성만으로 소프트웨어가 갱신되며, SWC 비즈니스 로직은 타깃 MCU 교체 시에도 재사용된다.
...
- **전장 소프트웨어 진화**: 실시간 분산 제어 중심의 클래식 플랫폼과 SDV 고성능 자율주행을 위한 어댑티브 플랫폼의 공존 체계를 바탕으로, 영역 기반 E/E 아키텍처 및 SOME/IP 서비스 지향 통신, 클라우드 연계 OTA와의 통합 가속화 추세
- **시스템 엔지니어링 고려사항**: 하드웨어 독립성을 보장하는 VFB 포트 모델링과 함께, ISO 26262 ASIL-D 수준의 E2E 데이터 무결성 검증 및 BSW 메모리 최적화 거버넌스 확보 필수
