...
date: "2026-09-14T09:32:00+09:00"
tags:
  - "notes-hardware"
weight: 86
extra:
...
- **I/O 가상화 격리 진화**: PCIe 장치 식별자(BDF) 기반 도메인 격리와 IOTLB 캐시 가속을 바탕으로 I/O 패스스루 및 제로 트러스트 보안 표준으로 안착되었으며, CXL 메모리 풀링 및 DPU/IPU 가속 환경의 복합 이종 메모리 격리 장치로 진화
- **시스템 엔지니어링 고려사항**: 워크로드 보안 등급에 따른 엄격 모드와 지연 무효화 모드의 선별적 적용과 함께, 대용량 페이지 기반 IOTLB 적중률 튜닝을 병행하는 정밀한 공학적 절충 필수

#### 한줄 요약
- PCIe 도메인 격리와 IOTLB 캐싱으로 악성 DMA를 차단하고 워크로드별 모드 선별 및 대용량 페이지 튜닝으로 보안과 I/O 처리율을 동시 달성해야 함
