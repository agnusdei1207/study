...
- ResNet: **잔차 연결(Residual Connection)** 기반 심층(100+ 계층) 신경망의 기울기 소실 해결
...
| MBConv 블록(EfficientNet) | 깊이별 분리 합성곱과 채널 어텐션으로 **연산 효율 대폭 향상** |
...
|:---|:---|:---|:---|
| 적용 기준 | **구조 단순성** 및 직관적 특징 추출 요구 시 | **심층 신경망 구성** 및 범용 백본 네트워크 필요 시 | **자원 제약 환경(모바일/엣지)** 및 최고 연산 효율 시 |
...
- **온디바이스 제약 및 추론 효율 절충 통찰**: 온디바이스 인공지능(On-Device AI) 단말의 메모리 대역폭과 추론 지연시간 제약을 극복하기 위해 경량 합성곱 계열과 트랜스포머 간의 최적 하드웨어 정렬 체계 구축

#### 한줄 요약

- 잔차 연결과 복합 스케일링 기반의 합성곱 원리를 바탕으로, 기울기 소실을 방지하고 에지 단말의 자원 제약에 부합하는 분류 아키텍처를 설계한다.

## 2~4교시 확장

**예상 문항**
> 딥러닝 기반 이미지 분류 모델에서 신경망의 깊이 심화에 따른 기울기 소실 문제 해결 방안과 연산 효율성을 높이기 위한 아키텍처 설계 전략을 논하시오. (25점)
