...
|---|---|---|
| **최소분산 불편추정량 (MVUE)** | 모든 불편추정량 중 분산이 최소이며, 정칙성 조건과 하한 달성 가능한 추정량 존재 시 CRLB 도달 여부를 효율성 판단에 활용 | Ⅱ 4대 조건 |
...
| **대표 사례** | 외생성 등 Gauss-Markov 가정 아래 OLS 회귀계수, 표본평균, $n-1$ 표본분산 | Ridge 회귀계수, 분모가 $n$인 모분산 MLE, Lasso |

## Ⅴ. 편향-분산 트레이드오프와 머신러닝에서의 시사점

> 총 예측오차(MSE)를 최소화하기 위해 불편성을 포기하고 의도적 규제(Regularization)를 부여함.

| 모델 복잡도 | Bias | Variance | 선택 기준 |
|---|---|---|---|
| 낮음 | 큼 | 작음 | 과소적합 점검 |
| 중간 | 절충 | 절충 | 검증 MSE 최소점 |
| 높음 | 작음 | 큼 | 과대적합 점검 |

- MSE 분해: $E[(\hat\theta-\theta)^2]=Var(\hat\theta)+Bias(\hat\theta)^2$
- 선형모형의 외생성·선형성 등 가정 아래 OLS는 불편성을 가지며, 고차원 예측에서는 L2 규제 등을 적용해 편향을 감수하고 분산과 전체 MSE를 낮출 수 있음.
...
### 2. 핵심 메커니즘 / 체계
<div class="itpe-flow-map" role="img" aria-label="불편추정량 판정과 보정"><div class="itpe-flow-node"><strong>추정량 평가</strong><span>기준: 불편성 · 효율성 · 일치성 · 충분성</span></div><div class="itpe-flow-arrow">↓</div><div class="itpe-flow-node"><strong>Bessel's Correction</strong><span>처리: 자유도 $n-1$ 적용</span></div><div class="itpe-flow-arrow">↓</div><div class="itpe-flow-node is-current"><strong>판정</strong><span>산출: $E(s^2)=\sigma^2$</span></div></div>
- 표본평균 사용으로 잃어버린 1자유도를 보정하기 위해 $n-1$로 나눔.

| 선택 기준 | 불편추정량 | 편향 허용 추정량 |
|---|---|---|
| 우선 목표 | 모수 해석 | 예측 MSE(Mean Squared Error) |
