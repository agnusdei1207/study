...
- [ ] 데이터 복제의 3대 구현 계층(스토리지 블록, DBMS 엔진, 애플리케이션 CDC)의 장단점을 비교할 수 있는가?
- [ ] 동기(Sync), 비동기(Async), 반동기(Semi-Sync) 복제 방식의 RPO 및 지연시간 트레이드오프를 설명할 수 있는가?
- [ ] Active-Active 다중 리전 환경에서 스플릿 브레인 방지 및 데이터 충돌 해결 전략을 제시할 수 있는가?
