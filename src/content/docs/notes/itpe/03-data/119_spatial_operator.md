...
- 본질: **공간 연산자는 기하 객체 사이의 위상·거리·측정·변환을 계산하는 함수이며, 공간 인덱스를 쓰는 질의는 후보 선별 뒤 정확한 기하 판정을 수행할 수 있음**
...
| **3. 2단계 처리 메커니즘** | - **후보 선별(Filter)**: 공간 인덱스의 경계 상자로 후보 집합 구성<br/>- **정확 판정(Refine)**: 실제 기하를 대상으로 연산 조건 평가 |
...
2. **1단계: MBR 필터링 단계 (Filter Phase)**:
   - 질의 대상 영역의 MBR과 데이터 테이블의 공간 인덱스(R-Tree)를 비교하여 MBR이 겹치는 후보 레코드만 고속 추출
    - 후보 집합 크기는 공간 분포·경계 상자 겹침·질의 조건에 따라 달라짐
...
| **대규모 폴리곤 연산 비용** | 복잡한 기하의 정밀 판정과 조인 후보 증가 | 유효성·정확도 요구를 확인하고, 경계 단순화나 분할·인덱스 사용을 시험해 평가 |
