...
- [ ] 사실 테이블(Fact)과 차원 테이블(Dimension)의 역할 및 키(대리키 vs 외래키) 구조를 설명할 수 있는가?
- [ ] 킴볼의 4단계 차원 모델링 절차(프로세스, 입도, 차원, 사실)를 순서대로 기술할 수 있는가?
- [ ] 스타 스키마와 스노우플레이크 스키마의 성능 및 스토리지 트레이드오프, 그리고 SCD Type 2의 이력 관리 방식을 비교할 수 있는가?
