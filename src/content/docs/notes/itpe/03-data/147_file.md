...
- [ ] 파일의 3대 구조(순차 파일, 직접 파일, ISAM)의 물리 배치 메커니즘과 장단점을 비교할 수 있는가?
- [ ] ISAM의 3대 영역(인덱스, 기본 데이터, 오버플로우) 동작 방식과 주기적 재구성 필요성을 설명할 수 있는가?
- [ ] 파일 시스템의 4대 한계(종속성, 중복성, 동시성 부재, 장애 회복 부재)와 DBMS의 해결책을 설명할 수 있는가?
- [ ] 리눅스 VFS의 4대 핵심 구조체(Superblock, Inode, Dentry, File)와 Page Cache의 역할을 제시할 수 있는가?
