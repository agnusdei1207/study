...
- [ ] 프로세스 마이닝 이벤트 로그의 3대 필수 속성(Case ID, Activity, Timestamp)의 역할을 설명할 수 있는가?
- [ ] 3대 분석 유형인 프로세스 발견, 적합도 검사, 프로세스 향상의 입력과 출력 메커니즘을 비교할 수 있는가?
- [ ] Alpha Miner 대비 Inductive Miner가 가진 수학적 건전성(Soundness)의 강점을 설명할 수 있는가?
- [ ] 스파게티 프로세스(Spaghetti Process) 극복을 위한 해피 패스(Happy Path) 필터링 기법과 RPA 연계 전략을 제시할 수 있는가?
