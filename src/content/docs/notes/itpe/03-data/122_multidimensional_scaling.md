...
|:---|:---:|:---|
| **Stress 값** | **해석** | **유의점** |
| 낮을수록 | 원래 비유사도와 저차원 거리 간 불일치가 작음 | 차원·거리척도·Stress 정의·자료에 따라 달라져 보편 합격선으로 단정하지 않음 |
...
| Stress가 상대적으로 큼 | 선택 차원에서 거리 구조를 충분히 보존하지 못할 가능성 | 차원·거리 정의·자료를 재검토하고 결과의 안정성과 해석 가능성을 비교 |
