...
- 본질: **분산 데이터베이스는 서로 연결된 여러 노드에 데이터를 나누거나 복제해 저장하고, 질의·갱신을 조정하는 데이터 관리 시스템으로, 분산 구조의 세부를 감추는 수준과 트랜잭션 보장은 구현에 따라 달라짐**
- 암기: `위-분-복-병-장` (5대 투명성: 위치, 분할, 복제, 병행, 장애) / `준-실` (2PC: Prepare, Commit)
- 판단축:
  - **집중형 DB**: 단일 노드 수직 확장(Scale-up), 트랜잭션 단순, 단일 장애점(SPOF) 위험.
  - **분산 DB**: 수평 확장과 데이터 배치를 지원하지만, 투명성의 범위·일관성·트랜잭션 보장은 제품 및 구성에 따라 다름.
...
> 분산 데이터베이스의 정의와 목적, 데이터 분할·복제 및 분산 질의 구조를 설명하고, 분산 트랜잭션의 2단계 커밋 절차와 장애 시 한계 및 보완 방안을 논하시오. (예상)
