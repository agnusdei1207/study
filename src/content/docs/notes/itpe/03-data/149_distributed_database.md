...
- [ ] 분산 데이터베이스의 5대 투명성(위치, 분할, 복제, 병행, 장애)의 정의와 은닉 대상을 제시할 수 있는가?
- [ ] 2단계 커밋(2PC)의 Prepare 단계와 Commit 단계의 시퀀스를 도식화하고 블로킹 발생 시나리오를 설명할 수 있는가?
- [ ] 2PC 만장일치 방식과 Raft/Paxos 과반수 합의(Quorum) 방식의 장애 내성 차이를 비교할 수 있는가?
- [ ] MSA 환경에서 2PC를 대체하는 사가(Saga) 패턴의 오케스트레이션과 보상 트랜잭션 메커니즘을 설명할 수 있는가?
