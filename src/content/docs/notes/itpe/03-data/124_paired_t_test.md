...
<svg viewBox="0 0 520 260" width="100%" height="260" xmlns="http://www.w3.org/2000/svg" style="font-family: sans-serif; background: var(--sl-color-bg-inline-code, #f8fafc); border-radius: 8px; border: 1px solid var(--sl-color-gray-5, #e2e8f0);">
  ...
  <text x="45" y="147" font-size="10" font-family="monospace" fill="#059669">짝 안의 공통 변동을 차이 계산으로 줄일 수 있음</text>
...
- 본질: **대응 표본 t-검정은 짝지어진 관측값의 차이($D$)를 한 표본으로 보고 그 평균이 기준값과 다른지 검정하는 방법**
...
> 대응 표본 t-검정(Paired t-test)의 차이값(D) 분석과 사전·사후 효과 검증의 정의와 목적, 핵심 구조와 작동 원리를 설명하시오. (예상)
...
| **3. 독립 t-검정과의 차이** | 독립 t-검정이 두 독립 집단의 평균($df = n_1+n_2-2$)을 비교하는 반면, 대응 t-검정은 짝 안의 공통 변동을 차이 계산으로 줄일 수 있음 |
...
| **짝짓기 효과** | 짝 안의 공통 변동을 차이 계산으로 줄일 수 있음 | 서로 다른 집단의 개별 편차를 짝으로 제거하지 않음 |
| **주요 가정** | 쌍 간 독립성, 차이값 분포의 정규성 근사 | 집단 간 독립성. 등분산을 가정하지 않는 Welch 검정 선택 가능 |
...
### Ⅲ. 핵심 전제 조건: 차이값의 분포를 그래프와 진단 방법으로 검토하고 표본 크기·이상치·추론의 강건성을 함께 고려
