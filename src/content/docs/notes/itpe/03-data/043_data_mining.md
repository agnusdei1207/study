...
- [ ] 데이터마이닝의 정의(비명시적·사전미인지·잠재적 유용 지식 발굴)를 설명할 수 있는가
- [ ] KDD 5단계와 CRISP-DM 6단계 순환 모델의 차이를 도식화할 수 있는가
- [ ] 지도학습(분류·예측)과 비지도학습(군집·연관) 4대 기법을 구분할 수 있는가
- [ ] 연관분석 3대 평가 척도(지지도, 신뢰도, 향상도)의 수식과 의미를 암기하고 있는가
- [ ] 데이터마이닝(가설 발견)과 OLAP(가설 검증)의 패러다임 차이를 비교할 수 있는가
- [ ] Ⅶ 결론에서 XAI 설명력 확보 및 실시간 MLOps 연계 방안을 제시할 수 있는가
