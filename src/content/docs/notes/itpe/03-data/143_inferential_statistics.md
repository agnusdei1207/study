...
- [ ] 중심극한정리(CLT)의 조건과 표본평균의 표본분포 공식을 유도하고 설명할 수 있는가?
- [ ] 바람직한 점추정량의 4대 조건(불편성, 일치성, 효율성, 충분성)을 정의할 수 있는가?
- [ ] 1종 오류($\alpha$)와 2종 오류($\beta$)의 상충(Trade-off) 관계 및 검정력($1-\beta$)을 도식화할 수 있는가?
- [ ] 빅데이터 환경에서 발생하는 p-value 역설의 수학적 원인과 효과 크기(Effect Size)를 활용한 해결책을 제시할 수 있는가?
