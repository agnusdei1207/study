...
- [ ] 빅데이터 분석 도구 선정을 위한 5대 평가 원칙(목적, 성능, 역량, 비용, 생태계)을 설명할 수 있는가?
- [ ] 지연시간(Latency)과 데이터 규모(Volume)를 축으로 하는 도구 선정 4사분면 매트릭스를 도식화할 수 있는가?
- [ ] 소규모 데이터셋에 분산 엔진을 적용할 때 발생하는 오버엔지니어링 문제와 DuckDB 등의 경량 대안을 제시할 수 있는가?
