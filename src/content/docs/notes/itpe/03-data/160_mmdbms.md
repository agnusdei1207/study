...
- 판단축:
  - **디스크 중심 DBMS**: 영속 데이터는 디스크에 두고 메모리를 버퍼로 활용.
...
| 항목 | 핵심 서술 내용 |
|:---|:---|
| **정의** | 주 데이터를 주기억장치(RAM)에 두고 처리하여 디스크 입출력을 줄이는 DBMS |
| **목적** | 메모리 상주 데이터의 접근 지연을 줄이되 영속성·복구 요구도 함께 충족 |
| **핵심 구조** | 주 데이터를 RAM에 두고 질의·갱신을 처리하며 영속 저장소와 회복 경로를 함께 구성 |
| **회복 기법** | 체크포인트와 트랜잭션 로그를 이용해 영속 데이터를 복구; 내구성 수준은 설정에 따라 다름 |
| **실무 제언** | 성능 요구·데이터 크기·비용·내구성 요구를 측정한 뒤 메모리 상주 범위와 영속성 설정을 결정 |
---
...
| **응답 지연시간** | 저장 매체·부하·캐시 상태에 따라 달라짐 | 워크로드·제품·구성에 따라 달라짐 | 명령·설정·부하에 따라 달라짐 |
| **트랜잭션 지원** | 제품·격리 수준·설정에 따름 | 제품·격리 수준·설정에 따름 | 제품별 명령과 트랜잭션 기능에 따름 |
...
- **핵심 가치**:
  - 메모리에 주요 데이터를 두어 디스크 읽기 의존도를 낮출 수 있음.
  - 실제 지연시간과 처리량은 영속성 설정, 데이터 크기, 질의, 동시성, 하드웨어에 따라 측정.
...
```mermaid
flowchart TD
    A[현재 노드: 최소 키~최대 키 범위] --> B[왼쪽 자식 서브트리]
    A --> C[오른쪽 자식 서브트리]
    B --> D[현재 노드보다 작은 키 범위]
    C --> E[현재 노드보다 큰 키 범위]
```
...
- **T-Tree**:
  - 메인 메모리 DB 연구에서 사용된 균형 탐색 트리 계열의 인덱스 구조. 실제 DB 제품은 다양한 인덱스 구조를 사용하며 T-Tree가 보편적 필수 구성은 아님.
  - 구체 노드 구성·탐색 성능은 구현에 따름. 디스크 기반 B-Tree와 비교할 때 워크로드와 메모리 계층을 고려.
...
```mermaid
flowchart TD
    A[트랜잭션 변경] --> B[메모리 내 데이터 상태 변경]
    A --> C[로그 기록]
    C --> D[설정된 내구성 시점에 영속 저장]
    B --> E[체크포인트로 데이터 상태 기록]
    D --> F[장애 후 로그와 체크포인트로 복구]
    E --> F
```

1. **로그 기록과 그룹 커밋 (Group Commit)**:
   - 제품은 커밋 전 로그를 영속 저장할 수 있으며, 여러 동시 트랜잭션의 로그를 묶어 기록하는 그룹 커밋을 제공할 수 있음. 지연 내구성 설정은 응답을 앞당기는 대신 장애 시 손실 위험을 남김.
...
3. **비휘발성 메모리 (NVRAM / NVDIMM)**:
   - 배터리 백업 메모리나 비휘발성 메모리를 사용할 수 있으나, 영속성은 하드웨어·로그·복제·제품 설정을 포함한 전체 구성으로 확인.
...
|:---|:---|:---|:---|
| **주 저장소** | 보조기억장치 (디스크/SSD) | **주기억장치 (RAM)** | **주기억장치 (RAM)** |
| **응답 지연시간** | 저장 구성·워크로드에 따라 달라짐 | 메모리 상주 이점이 있으나 구현·워크로드에 따라 달라짐 | 명령·지속성 설정·배포 구성에 따라 달라짐 |
| **트랜잭션 지원** | ACID 범위는 제품·설정에 따름 | 트랜잭션 범위와 내구성은 제품·설정에 따름 | 원자성 및 트랜잭션 범위는 명령·버전에 따라 다름 |
| **질의 언어** | 표준 SQL (복합 조인 지원) | **표준 SQL (복합 조인 지원)** | Key-Value 커맨드, 전용 API |
| **주요 인덱스** | B-Tree, B+Tree | **T-Tree, Hash Index, Bw-Tree** | SkipList, Dict(Hash) |
| **데이터 모델** | 관계형 테이블 스키마 | 관계형 테이블 스키마 | Strings, Hashes, Lists, Sets |
| **대표 제품** | Oracle, PostgreSQL, MySQL | **ALTIBASE, TimesTen, SAP HANA** | **Redis, Memcached, Dragonfly** |
...
- **재기동 시 대용량 메모리 로딩 지연 (RTO)**:
  - 대규모 메모리 DB는 재기동 시 영속 저장소에서 메모리로 복원하는 시간이 복구 목표를 넘을 수 있음
...
| 한계 | 우선 제안 |
|:---|:---|
| 빠른 응답을 위해 지연 내구성을 허용하면 응답 후 장애에서 최근 커밋이 유실될 가능성 | 업무별 RPO를 먼저 정하고 커밋 응답 조건을 선택, 장애 주입으로 실제 손실 범위 검증 |
| RAM 상주 용량을 과도하게 잡으면 비용과 복구 시간이 증가 | 데이터 온도·접근 패턴을 측정해 상주 범위를 정하고, 복제·체크포인트·복원 절차를 함께 시험 |
...
- **검증 출처**:
  ...
  - [SAP HANA Platform, Data and Log Volumes](https://help.sap.com/docs/SAP_HANA_PLATFORM/6b94445c94ae495c83a19646e7c3fd56/4a9a3970fb634a319f32bb76e1c59dd5.html)
