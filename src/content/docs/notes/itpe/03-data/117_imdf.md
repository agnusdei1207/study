...
- 본질: **IMDF(Indoor Mapping Data Format)는 실내 공간의 위치·시설 정보를 표현하고 교환하기 위한 GeoJSON 기반 OGC 커뮤니티 표준**
- 암기: `베-빌-풋-레-유` (5대 공간 계층: Venue, Building, Footprint, Level, Unit) / `오-앵-어-릴` (4대 보조 피처: Opening, Anchor, Amenity, Relationship)
- 판단축:
  - **IMDF (GeoJSON)**: 실내 공간 정보를 모바일·웹 지도에서 표현하고 교환하는 표준
...
| **2. 주요 피처** | Venue, Building, Level, Unit 등 장소·건물·층·공간 표현 객체 |
| **3. 보조 피처** | Opening, Anchor, Amenity, Relationship 등 출입구·기준점·시설·객체 관계 표현 |
...
- **주요 연결 및 보조 피처**:
  ...
  - **Anchor**: 실내 지도 좌표와 외부 위치 참조를 연결하는 기준점 객체
  - **Amenity**: 화장실, 엘리베이터, ATM, 심장충격기(AED) 등 편의시설 아이콘 표시 객체
  - **Relationship**: IMDF 피처 사이의 지정된 관계를 표현하며, 경로 계산 그래프 자체를 모두 정의하는 것은 아님
