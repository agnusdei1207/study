...
- [ ] 로지컬 DW(LDW)가 물리적 ETL 복제 없이 실시간 분석을 제공하는 가상화 원리를 설명할 수 있는가?
- [ ] 쿼리 푸시다운(술어, 프로젝션, 집계)이 시스템 성능 및 네트워크 대역폭에 미치는 영향을 기술할 수 있는가?
- [ ] 전통적 물리 DW와 로지컬 DW의 장단점을 5대 축(이동방식, 지연, TCO, 유연성, 원천부하)으로 비교할 수 있는가?
