...
- [ ] 음성데이터 마이닝의 5단계 처리 파이프라인(전처리-화자분리-STT-감정분석-텍스트마이닝)을 도식화할 수 있는가?
- [ ] 단순 텍스트 마이닝 대비 음향 신호(MFCC, Pitch, dB) 결합이 감성 분석 정확도에 미치는 영향을 설명할 수 있는가?
- [ ] AICC 실무 구축 시 Barge-in(동시 발화) 문제와 개인정보(PII) 실시간 마스킹 해결책을 제시할 수 있는가?
