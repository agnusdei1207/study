...
author: "Antigravity"
date: "2026-09-21T18:25:00+09:00"
tags:
  - "notes-data"
weight: 148
extra:
...
- 본질: **최댓값 또는 최솟값을 $O(1)$의 시간복잡도로 추출하기 위해 부모와 자식 간의 대소 관계를 유지하는 완전 이진 트리(CBT, Complete Binary Tree) 기반의 반정렬(Semi-sorted) 자료구조로, 포인터 오버헤드 없이 1차원 연속 배열에 매핑하여 캐시 지역성(Cache Locality)과 우선순위 큐(PQ)를 구현하는 핵심 기술**
...
| **물리 매핑** | 1차원 배열 사상: 부모 = `i >> 1`, 좌측 자식 = `i << 1`, 우측 자식 = `(i << 1) | 1` (포인터 없는 캐시 최적화) |
| **핵심 연산** | ① 삽입(Up-Heap, $O(\log N)$): 맨 끝 리프 추가 후 부모와 비교 승격 / ② 삭제(Down-Heap, $O(\log N)$): 루트 반환 후 마지막 원소를 루트로 이동하여 강등 |
| **힙 vs BST** | 힙은 CBT 기반 극값 추출($O(1)$) 및 PQ 구현 특화 / BST는 좌우 전정렬 기반 임의 키 검색($O(\log N)$) 특화 |
| **실무 제언** | 대규모 스트리밍 Top-K 집계 시 크기 $K$의 Min Heap을 유지하여 연산 복잡도를 $O(N \log K)$, 메모리를 $O(K)$로 고정 절감 |
...
- [ ] 완전 이진 트리(CBT)의 1차원 배열 인덱스 매핑 공식(부모, 좌/우 자식)을 비트 연산과 함께 제시할 수 있는가?
- [ ] Up-Heap과 Down-Heap의 동작 절차를 단계별로 도식화하고 시간복잡도를 설명할 수 있는가?
- [ ] 하향식(Bottom-Up) 방식의 Heapify가 $O(N)$을 달성하는 수학적 유도 과정을 설명할 수 있는가?
- [ ] 실시간 스트리밍 환경에서 크기 $K$의 최소 힙을 활용한 Top-K 추출 아키텍처를 제시할 수 있는가?
