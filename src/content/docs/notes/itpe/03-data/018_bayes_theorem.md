...
tags:
  ...
  - "나이브베이즈"
  - "기저율오류"
date: "2026-09-20T23:07:00+09:00"
author: "기술사 수험생"
extra:
...
```text
  ...
  ┌───────────────────┴───────────────────┐
  ▼                                       ▼
[가능도 P(E|H)] × [사전확률 P(H)]      [증거확률 P(E)]
(가설 하 증거 출현 확률) (기존 배경 지식) (전확률 공식에 의한 정규화 상수)
```
...
```text
[나이브 베이즈 분류 수식]
  P(Class | Word1, Word2, ..., WordN) ∝ P(Class) × ∏ P(Word_i | Class)
  * 가정: 클래스가 주어졌을 때 모든 입력 특성(단어)들은 상호 조건부 독립이다!
```
...
```text
[조건] 인구 10,000명 중 희귀병 환자 10명 (기저율 P(H) = 0.001)
       검사 민감도 P(E|H) = 0.99, 위양성률 P(E|~H) = 0.01

[실제 계산]
1. 실제 환자 중 양성자 (True Positive)  = 10명 × 0.99 = 9.9명
2. 건강한 사람 중 위양성 (False Positive) = 9,990명 × 0.01 = 99.9명
3. 양성 판정을 받았을 때 진짜 환자일 확률:
   P(H|E) = 9.9 / (9.9 + 99.9) ≈ 9.0% !
```
...
### 학습자 통찰 메모 — 답안 밖
- `[핵심 통찰]`: 기저율 오류는 수학 문제가 아니라 실무 경영진을 설득할 때 가장 자주 터지는 의사결정 함정임. 아무리 정확한 AI라도 사기 발생률 자체가 0.01%라면 오경보가 대다수를 차지하게 됨.
- `나라면`: FDS 시스템 설계 시 단순 임계치 판정을 배제하고, 베이지안 갱신 엔진을 통해 사용자의 추가 행동 증거가 유입될 때마다 실시간으로 사후 사기 확률을 동적 재평가하는 순차 의사결정 파이프라인을 구축하겠음.
...
기저율 무시 (오경보)   베이지안 기저율 보정    Brier Score 최소화      위양성 오경보 70% 감축
배치 재학습 병목       사후확률 순차 갱신     Calibration 곡선 일치   실시간 스트리밍 AI 달성
```
...
- 공식: P(H|E) = [P(E|H) × P(H)] / P(E)  (Posterior ∝ Likelihood × Prior)

2. 4대 핵심 구성요소
┌───────────────┬─────────────────────────────────────────────┐
│ 요소          │ 개념 및 의미                                │
├───────────────┼─────────────────────────────────────────────┤
│ 사전확률 P(H) │ 증거 관측 전 가설에 대한 초기 믿음 (기저율) │
│ 가능도 P(E|H) │ 가설이 참일 때 증거가 관측될 확률           │
│ 증거확률 P(E) │ 전체 표본공간에서 증거가 나타날 전확률      │
│ 사후확률 P(H|E)│ 증거가 주어진 후 갱신된 가설의 최종 확률   │
└───────────────┴─────────────────────────────────────────────┘

3. 실무 제언: 기저율 오류(Base Rate Fallacy) 극복
- 희귀 질환/사기 탐지 시 낮은 기저율을 무시하면 위양성이 폭증하므로, 반드시 실제 도메인 기저율을 Prior로 반영해야 함.
```
...
- [ ] [Ⅰ 개요]: 베이즈 정리의 정의와 사후확률 비례식($Posterior \propto Likelihood \times Prior$)을 기술하였는가?
- [ ] [Ⅱ 요소]: 사전확률, 가능도, 증거확률, 사후확률 4대 요소의 의미를 설명하였는가?
- [ ] [Ⅲ 비교]: 모수를 바라보는 관점 등 빈도주의와 베이지안의 철학적 차이를 비교하였는가?
- [ ] [Ⅴ 사례]: 기저율 오류의 수치적 예시와 머신러닝 불균형 데이터 적용 시의 시사점을 논하였는가?
