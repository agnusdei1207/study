...
- 주의: 시스템 이용률($\rho$)이 1에 가까워지면 대기시간 분모($(1-\rho)$)가 0으로 수렴하면서 대기열이 무한대로 폭발하므로, 실제 운영 시스템의 CPU/처리 이용률은 **70~80% 이내**로 상한 통제 필수
...
- **목적**: 시스템 이용률($\rho$), 평균 대기시간($W_q$), 평균 큐 길이($L_q$)를 산출하여 SLA(응답시간 보장)를 만족하는 최소 서버 대수($c$) 산정
...
1. **리틀의 법칙 (Little's Law)**:
   - 시스템이 정상 상태(Steady-state)에 있을 때, 도착 분포나 서비스 분포의 형태와 무관하게 항상 성립하는 정리
...
> **[핵심 통찰]**
> 비기술 관리자들은 종종 "서버 CPU가 50%밖에 안 도니 인스턴스를 줄여서 비용을 아끼라"고 말하지만, 대기행렬이론은 **"이용률 100% 가동은 재앙을 부르는 설계"**임을 수학적으로 증명한다.
> 대기시간 공식 $W_q = \frac{\rho}{\mu(1-\rho)}$에서 보듯, 이용률 70%에서 90%로 가는 20%의 증가는 대기시간을 무려 3.5배 이상 폭발시킨다.
> 엔지니어의 역할은 "비용 최적화"와 "지연시간(SLA) 폭발 방지" 사이의 완벽한 균형점(골디락스 존, 이용률 60~75%)을 대기행렬 수식으로 증명하여 경영진을 설득하는 것이다.
>
> **[나라면 이렇게 쓴다]**
> 결론부에서 "클라우드 네이티브 쿠버네티스 HPA(Horizontal Pod Autoscaler) 환경에서의 대기행렬 응용"을 제시하겠다. 기존의 단순 CPU/Memory 임계치 기반 오토스케일링을 지양하고, 리틀의 법칙에 기반하여 `현재 큐 체류 요청 수(L)`와 `도착률(λ)`을 모니터링하여 목표 응답시간($W$)을 초과하기 전에 선제적으로 Pod를 스케일아웃하는 예측형 오토스케일링 아키텍처를 강조하겠다.
...
| **4. 시스템 사이징** | 이용률 $\rho = \lambda / \mu$가 1에 근접할수록 대기시간이 $1/(1-\rho)$에 의해 폭증하므로 서버 목표 이용률을 70~80%로 통제 |
...
- [ ] 대기행렬 시스템의 4대 요소와 켄달 표기법 6개 슬롯의 의미를 기술할 수 있는가?
- [ ] 리틀의 법칙($L = \lambda W$)의 전제 조건과 수식적 의미를 설명할 수 있는가?
- [ ] M/M/1 모형에서 시스템 이용률 $\rho$의 증가에 따른 대기시간 $W_q$의 비선형 폭증 그래프를 도식화할 수 있는가?
