...
- **정의**: C++로 작성된 크로스 플랫폼 문서 지향 NoSQL로, 동적 스키마(Schema-less) 특성과 WiredTiger 스토리지 엔진을 기반으로 고성능 읽기/쓰기를 제공하는 데이터베이스
- **데이터 모델링 매핑 구조**:
...
> **[핵심 통찰]**
> MongoDB는 과거 "빠르지만 데이터가 유실될 수 있는 비정형 캐시형 DB"라는 오명을 벗고, WiredTiger 엔진 도입, Raft 기반 Replica Set, 그리고 4.0 이후 **분산 다중 문서 ACID 트랜잭션**을 완성하며 엔터프라이즈 코어 DB로 완전히 진화했다.
> 최근에는 단순 문서 저장을 넘어 **Atlas Vector Search**를 통해 LLM의 임베딩 벡터와 메타데이터 필터링을 단일 엔진에서 처리함으로써, 별도의 전용 Vector DB(Pinecone 등)를 도입하는 아키텍처 복잡성을 제거하는 핵심 플랫폼으로 자리 잡았다.
>
> **[나라면 이렇게 쓴다]**
> 결론부에서 RDBMS와의 무조건적인 대체 관계가 아닌 "폴리글랏 퍼시스턴스(Polyglot Persistence)" 관점을 제시하겠다. 계정계 코어 트랜잭션은 RDBMS가 담당하고, 채널계 카탈로그 및 AI RAG 파이프라인의 지식 베이스는 MongoDB Atlas(BSON + Vector Search)가 담당하는 멀티모달 통합 데이터 아키텍처를 제시하겠다.
