...
- **CAP 정리 (CAP Theorem)** : 네트워크 분할 중 일관성과 가용성을 동시에 완전 보장할 수 없다는 분산 시스템의 절충 설명.
- **일관성 (Consistency)** : CAP 맥락에서 읽기가 선형화 가능성 등 선택한 일관성 보장을 충족하는 속성.
- **가용성 (Availability)** : 분할 상태에서도 각 요청이 유효한 응답을 받는 보장.
- **네트워크 분할 (Partition)** : 노드 간 통신 단절로 그룹 사이 메시지 전달이 제한되는 상태.
- **PACELC** : 분할 시(P) 가용성(A)과 일관성(C), 정상 상태(E)에서는 지연(L)과 일관성(C)의 절충을 나타내는 모형.
- **정족수 (Quorum)** : 읽기·쓰기 또는 합의에 필요한 노드의 최소 응답 집합.
