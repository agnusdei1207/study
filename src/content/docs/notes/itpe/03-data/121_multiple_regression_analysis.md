...
- [ ] 가우스-마르코프 정리에 따른 OLS 잔차의 4대 기본 가정과 검정 방법을 서술할 수 있는가?
- [ ] 일반 결정계수($R^2$) 대신 수정된 결정계수(Adjusted $R^2$)를 사용하는 이유와 수식을 설명할 수 있는가?
- [ ] 다중공선성의 발생 메커니즘, VIF 수식 및 판별 기준, Lasso/Ridge 정규화의 차이를 비교할 수 있는가?
