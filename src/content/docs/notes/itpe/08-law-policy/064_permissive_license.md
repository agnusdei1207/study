...
| 문제 | 해결 방안 |
|---|---|
| 코드 변경이나 재배포 때 고지·NOTICE 자료가 빠질 수 있음 | 제안: 빌드 시 SBOM과 라이선스 고지 파일을 산출하고, 릴리스 검토에서 바이너리 포함 구성요소와 대조한다. |
| Apache-2.0의 특허 허락 범위를 프로젝트 전체의 특허 보증으로 오해할 수 있음 | 제안: 핵심 구성요소의 특허 출처·기여자·변경 이력을 기록하고, 특허 쟁점은 배포 전 별도 법률 검토한다. |
