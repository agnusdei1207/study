...
```mermaid
flowchart TD
    A[제품·서비스의 의도된 사용과 실제 사용 확인] --> B[법률상 판단 기준·적용 분야 대조]
    B --> C[사업자가 해당 여부를 사전 검토하고 근거 기록]
    C --> D{장관 확인이 필요한가?}
    D -->|아니오| E[사업자가 검토 결과에 따라 의무 이행]
    D -->|예| F[과학기술정보통신부장관에게 확인 요청]
    F --> G[전문위원회 자문 등을 거쳐 장관이 확인]
    G --> E
```

이 도해는 인공지능기본법 제32조의 사전 검토와 장관 확인 요청 경로를 나타낸다. 모든 사업자가 반드시 확인 요청을 거치는 것은 아니다.
