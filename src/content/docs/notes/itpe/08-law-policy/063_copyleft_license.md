...
| 문제 | 해결 방안 |
|---|---|
| 이미 설계에 편입된 라이선스가 제품의 소스 공개 가능 범위와 맞지 않으면 뒤늦은 교체 비용이 커짐 | 제안: 의존성을 처음 승인할 때 제품의 배포 형태와 소스 공개 허용 수준을 먼저 정하고, 이를 만족하지 않는 GPL·AGPL 구성요소는 법무·개발 검토 전 도입하지 않는 선택 기준을 둔다. |
