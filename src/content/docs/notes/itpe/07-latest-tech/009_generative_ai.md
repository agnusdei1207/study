...
- 본질: 생성형 AI는 학습한 데이터의 패턴을 바탕으로 새로운 콘텐츠를 생성하는 모델이다.
- 메커니즘: 사전학습 → 지시 조정·정렬 → 사용자 입력 조건화 → 결과 생성·검증

<details>
<summary>핵심 용어</summary>

- **확산 모델(Diffusion Model)**: 데이터에 노이즈를 더하는 과정을 학습한 뒤 역과정을 수행해 새 표본을 생성하는 모델
- **모델 정렬(Alignment)**: 모델의 응답이 정해 둔 사람의 의도·안전 기준에 더 잘 맞도록 학습·조정하는 과정
- **환각(Hallucination)**: 생성 모델이 근거가 부족하거나 사실과 다른 내용을 그럴듯하게 생성하는 현상
</details>
