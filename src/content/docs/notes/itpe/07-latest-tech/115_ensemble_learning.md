...
| 문제 | 해결 방안 |
|---|---|
| 모델이 비슷한 오류를 반복해 결합 효과가 낮음 | 데이터·알고리즘·특성의 차이가 실제 오류 다양성으로 이어지는지 검증 |
| 스태킹 평가에 학습 정보가 새어 성능이 과대평가됨 | OOF 예측으로 메타 모델을 학습하고 최종 테스트 자료는 평가에만 사용 |
| 복잡한 앙상블이 지연·운영 비용을 높임 | 단일 모델과 비교해 정확도·지연·비용의 균형으로 채택 여부 결정 |
