...
- **Self-RAG(Self-Reflective Retrieval-Augmented Generation):** 모델이 검색 필요성·검색 자료·생성 답변을 반성 토큰으로 평가하며 생성하도록 학습한 RAG 방식이다.
...
## 1교시 예상문제 (10점)
> Self-RAG의 개념과 반성 토큰을 이용한 검색·생성 구조를 설명하시오. (예상)
...
| 점검 대상 | 판단 기준 |
|---|---|
| 검색 필요 | 답변 문맥만으로 근거를 만들 수 없는가 |
| 검색 자료 | 질문과 관련되고 출처가 확인되는가 |
| 생성 답변 | 근거를 반영하며 질문에 유용한가 |
