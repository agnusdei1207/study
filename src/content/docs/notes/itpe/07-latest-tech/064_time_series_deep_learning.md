...
| 문제 | 해결 방안 |
|---|---|
| 학습·검증 분할에 미래 정보가 섞이면 성능이 과대평가됨 | 시간순 분할과 입력 시점 기준의 자료 가용성을 점검 |
| 분포가 변하면 과거에 맞춘 예측이 저하될 수 있음 | 실제 예측 오차를 감시하고 자료·모델 갱신 시 재평가 |
| 복잡한 모델의 운영비가 품질 이득보다 클 수 있음 | 단순 기준모델과 같은 조건으로 품질·비용을 함께 비교 |
