...
- **Agentic AI Foundation (AAIF):** Linux Foundation이 2025년 12월 설립을 발표한 에이전트 관련 오픈 프로젝트를 지원하는 중립 재단이다.
...
## 1교시 예상문제 (10점)
> 자율 AI 에이전트 환경에서 AGENTS.md의 역할과 AAIF의 출범 취지, 운영 시 보안상 유의점을 설명하시오. (예상)

---

## 1교시 10점 답안
### 1. 정의·목적
정의: AGENTS.md는 코딩 에이전트가 프로젝트 작업 규칙과 맥락을 참고하도록 제공하는 열린 Markdown 파일이다.
목적: 반복 설명을 줄이고 여러 에이전트가 프로젝트 지침을 일관되게 참조하도록 돕는다.

### 2. 지침과 실행 권한의 구분
```mermaid
flowchart TD
    R[저장소 AGENTS.md] -->|프로젝트 지침을 제공| A[에이전트 작업 판단]
    A -->|도구 사용 요청| G[런타임 권한·샌드박스 검사]
    G -->|허용된 작업만 수행| T[대상 시스템 변경]
```
...
| 문제 | 해결 방안 |
|---|---|
| 프로젝트 지침과 실제 시스템 접근권한이 한 문서에 섞이면 텍스트만으로 보안이 확보된다는 오해가 생김 | 지침은 작업 맥락으로, 접근통제는 런타임 정책으로 분리하고 CI·샌드박스 시험으로 통제 작동을 검증한다. |
