...
- 본질: 프롬프트 엔지니어링은 입력 지시와 문맥을 설계해 모델 응답을 과업에 맞게 유도하는 방법이다.
- 메커니즘: 과업·성공 기준 정의 → 지시·예시·출력 형식 작성 → 결과 평가 → 수정

<details>
<summary>핵심 용어</summary>

- **Few-shot prompting**: 프롬프트에 소수의 입력·출력 예시를 넣어 원하는 응답 패턴을 안내하는 방법
- **ReAct(Reasoning and Acting)**: 모델의 판단과 도구 실행을 번갈아 수행하고 관찰 결과를 다음 판단에 반영하는 패턴
- **하네스 엔지니어링(Harness Engineering)**: 프롬프트뿐 아니라 도구·상태·권한·평가를 포함해 모델 실행을 둘러싼 환경을 설계하는 일
</details>
