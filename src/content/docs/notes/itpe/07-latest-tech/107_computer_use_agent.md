...
- **컴퓨터 유즈 에이전트**: Computer-Use Agent. 화면을 관측하고 클릭·입력 등 컴퓨터 도구로 작업을 수행하는 에이전트
- **GUI(Graphic User Interface)**: 화면 요소를 통해 사용자와 응용 프로그램이 상호작용하는 인터페이스다.
...
## 1교시 예상문제 (10점)
> 컴퓨터 유즈 에이전트의 개념과 화면 관측·입력 실행 흐름을 설명하시오. (예상)
...
| 구분 | 핵심 |
|---|---|
| 정의 | **컴퓨터 유즈 에이전트**는 GUI를 관측하고 입력 도구로 작업을 수행하는 AI 에이전트 |
| 목적 | 화면 기반 반복 작업의 수행과 실행 결과 확인 |
...
| 한계 | 대응 |
|---|---|
| GUI 에이전트 도입 범위가 읽기·검색에서 계정 변경·결제까지 한 번에 넓어질 수 있음 | 화면 조회와 비파괴 입력부터 시작하고, 계정 변경·결제 등 외부 효과가 있는 동작은 별도 승인 단계가 검증된 뒤에만 추가한다. |
