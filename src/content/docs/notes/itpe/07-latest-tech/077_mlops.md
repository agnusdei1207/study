...
```mermaid
flowchart TD
    D[버전이 관리되는 데이터·코드] -->|학습 파이프라인 입력| T[학습·검증]
    T -->|승인된 모델 등록| R[모델 레지스트리]
    R -->|배포 승인·전달| S[모델 서비스]
    S -->|예측·품질 정보 수집| M[운영 모니터링]
    M -->|필요 시 검토 후 재학습| T
```
...
|---|---|
| 데이터·코드 | 버전, 검증, 실행 환경 |
| 모델 | 학습 이력, 평가 결과, 승인·배포 상태 |
| 서비스 | 지연·오류·품질 지표, 변경·장애 기록 |
...
| 문제 | 해결 방안 |
|---|---|
| 자동화가 검증되지 않은 모델을 빠르게 운영에 반영할 위험이 있음 | 데이터·모델 검증, 승인 기준, 점진 배포·복구 경로를 파이프라인에 포함하고 운영 결과에 따라 재학습 여부를 판단한다. |
