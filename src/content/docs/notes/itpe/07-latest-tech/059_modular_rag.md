...
| 문제 | 해결 방안 |
|---|---|
| 모듈 수가 늘며 인터페이스와 운영 복잡도가 커짐 | 과업에 필요한 모듈만 두고 공통 입력·출력 계약과 독립 평가를 마련 |
| 검색 오류가 응답까지 전달될 수 있음 | 근거 부족·불일치 시 추가 검색 또는 답변 보류 경로를 설계 |
| 모듈 교체 후 품질 저하를 놓칠 수 있음 | 대표 질의 세트로 모듈별·종단 간 평가를 배포 전에 수행 |
