...
```mermaid
flowchart TD
    A[점의 Eps 이웃 탐색] --> B{이웃 수가 MinPts 이상?}
    B -->|예| C[핵심점에서 밀도 연결 이웃으로 군집 확장]
    B -->|아니오| D[경계점 또는 노이즈로 분류]
```
...
| 문제 | 해결 |
|---|---|
| 군집마다 밀도가 크게 다르면 하나의 Eps로 모든 군집을 찾기 어려움 | 핵심 거리 분포를 살펴 Eps 민감도를 확인하고, 가변 밀도 자료에서는 OPTICS 등 대안을 같은 자료에서 비교한다. |
