...
- **합의 알고리즘(Consensus Algorithm):** 분산 노드가 정해진 신뢰·장애 가정 아래 공유 상태를 결정하는 절차다.
- **작업 증명(Proof of Work, PoW):** 계산 퍼즐을 풀어 블록 후보를 제안하는 합의 방식.
- **지분 증명(Proof of Stake, PoS):** 지분 기반 검증자 선정·참여 규칙으로 블록을 검증하는 합의 방식.
- **위임 지분 증명(Delegated Proof of Stake, DPoS):** 참여자가 검증자를 선출·위임하는 지분 기반 방식.
- **실용적 비잔틴 장애 허용(Practical Byzantine Fault Tolerance, PBFT):** 정해진 비잔틴 장애 가정 아래 복수 단계 메시지 교환으로 합의하는 프로토콜.
