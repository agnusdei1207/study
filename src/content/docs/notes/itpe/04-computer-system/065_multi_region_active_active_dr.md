...
컴퓨터 시스템 및 네트워크 → 핵심 개념 → Multi-Region Active-Active DR

## 30초 인출

- 본질: 주 센터와 대기 센터로 나누는 Active-Standby는 재해 시 대기 장비 승격과 DNS 전파에 수십 분이 걸리고 대기 자원이 낭비됨
- 메커니즘: 지리적으로 수백 km 떨어진 복수 리전에 상시 가동 시스템을 구축하고 트래픽을 양방향 동시 수용
...
> Multi-Region Active-Active DR의 개념과 핵심 구조 또는 동작을 설명하시오. (예상)
...
- 정의: Multi-Region Active-Active DR은/는 지리적으로 수백 km 떨어진 복수 리전에 상시 가동 시스템을 구축하고 트래픽을 양방향 동시 수용 방식이다.
- 목적: 재해 발생 시 전환 시간 없는 제로 RTO 달성에 기여한다.
...
> Multi-Region Active-Active DR의 구조와 동작을 설명하고, 주요 비교 또는 적용 시 문제와 대응책을 제시하시오. (예상)
