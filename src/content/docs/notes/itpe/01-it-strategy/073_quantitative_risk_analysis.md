...
- 본질: 선별된 위험이 프로젝트 전체 비용·일정 목표에 미치는 영향을 확률·금액으로 계량화하는 분석 기법이다.
...
- **EMV(Expected Monetary Value)** : 위험의 발생 확률(P)과 영향 금액(I)을 곱해 미래 손익의 통계적 기댓값을 산출하는 기법이다.
- **Monte Carlo Simulation** : 불확실한 입력 변수에 확률분포를 할당하고 수천 번의 난수 연산을 통해 결과 분포를 도출하는 전산 시뮬레이션이다.
- **Tornado Diagram** : 민감도 분석 결과를 변동 폭이 큰 순서대로 수평 막대로 배치하여 핵심 리스크를 식별하는 다이어그램이다.
- **Decision Tree** : 의사결정 노드와 확률 노드의 분기별 조건부 확률·성과를 모델링하여 대안을 비교하는 기법이다.
- **Contingency Reserve(비상예비비)** : 대응 전략이 있는 식별된 위험에 대해 일정 기준선에는 시간, 원가 기준선에는 자금을 배정한 예비분이다.
- **S-Curve(누적 확률 곡선)** : 몬테카를로 시뮬레이션 결과로 특정 예산 또는 일정 내에 프로젝트를 완공할 누적 확률을 나타낸 곡선이다.
