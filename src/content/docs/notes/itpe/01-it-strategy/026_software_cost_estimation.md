...
- 본질: SW사업 대가산정은 사업 유형과 작업 규모에 맞춰 예산·계약 대가를 계산하는 방법이다. 개발 기능 규모와 운영 업무의 성격을 구분해 산정한다.
...
- 문제: 공공 SW 사업에서 모호한 요구사항으로 인한 무상 과업 변경 강요와 헤드카운트(M/M) 중심 임의 삭감으로 수주 기업의 적자 및 품질 저하가 악순환됨.
- 해결 방안: 소프트웨어 진흥법 제46조에 따른 FP(기능점수) 단가 체계를 철저히 적용하고, 개발 단계 과업 변경 발생 시 공공 SW 과업심의위원회를 의무 개최하여 증분 기능점수에 대한 정당한 계약금액 및 납기 연장을 제도화함.

```mermaid
flowchart TD
    subgraph Baseline["1. 적정 예산 수립 및 계약 Baseline 확정"]
        B1["ISMP 단계 상세 요구사항 명세 도출"]
        B2["기능점수(FP) 간이법/정규법 기반 개발비 산정"]
        B3["투입공수(M/M) 방식 배제 및 SW 대가기준 준수 계약"]
        B1 --> B2 --> B3
    end
    subgraph Control["2. 사업 수행 중 과업 변경 통제"]
        C1["발주기관의 신규·수정 요구사항 발생"]
        C2["추가 기능점수(FP) 정량 실측 및 비용 산출"]
        C3["공공 SW 과업심의위원회 개최 (과업 적정성 심의)"]
        C1 --> C2 --> C3
    end
    subgraph Adjustment["3. 계약 조정 및 공정 대가 지급"]
        A1["적격 판정: 계약금액 증액 및 사업 기간 합리적 연장"]
        A2["반려 판정: 불합리한 무상 과업 요구 배제"]
        C3 -->|승인| A1
        C3 -->|기각| A2
    end
