...
- 메커니즘: 전략을 계획·시뮬레이션(BPS)에 반영하고 성과(CPM)를 모니터링하며, VBM·BSC·ABM 관점으로 판단과 실행을 조정한다.
...
| 구분 | 핵심 |
|---|---|
| 정의 | **SEM(Strategic Enterprise Management, 전략적 기업경영)**은 기업 전략을 계획·성과관리·의사결정과 연결하는 경영 접근이다. |
| 목적 | 전략–실행 정렬 · 목표–실적 환류 · 의사결정 근거 확보 |

### Ⅱ. 전략경영 관점의 연결

**VBM(Value Based Management)**는 가치, **BSC(Balanced Scorecard)**는 전략 실행지표, **ABC(Activity Based Costing)**는 활동원가, **ABM(Activity Based Management)**은 활동 개선 관점을 제공한다.
...
- **관리 지표 연계**: **EVA(Economic Value Added)** 재무성과를 BSC 실행지표·ABC 원가정보와 함께 해석한다.
...
> SAP Help의 2025 FPS01 문서에 나오는 구성요소는 계획·성과·연결·정보수집·이해관계자 소통 기능을 나눠 맡음.
...
| 문제 | 해결 방안 |
|---|---|
| SEM 관점을 전사에 한꺼번에 확장하면 지표·원가·가치 사이의 연계를 검증하기 어렵다. | 하나의 전략목표를 선정해 BSC 실행지표, ABC/ABM 활동원가, VBM 가치지표를 연결한 관리 사례를 시범 운영한다. 가정·책임자·실적을 검증한 뒤 적용 범위를 넓힌다. |
