...
- **판정 기준 (Trigger)**: ERP 운영 실적 집계와 경영진 KPI 보고 수치 간 오차율 > 목표 기준 초과, 또는 환율·유가 등 거시 변수 변동 시 연간 사업계획 재시뮬레이션 소요시간 > 1주일 지연 시 관리 결함 판정.
...
- **기대 효과 (Impact)**: 단순 과거 실적 사후보고 탈피, 선제적 경영 의사결정 속도 목표 기준 단축, 부서별 이기주의를 배제한 전사 가치 극대화 자원 배분을 달성함.

```mermaid
flowchart TD
    P["ERP 운영 실적 집계와 경영진 KPI 보고 수치 간 오차"] --> A["SEM-BPS 기반의 What-If 시나리오 플래너를 구축"] --> V["분기별 EVA(경제적부가가치) 실적치 ↔ BSC 전략목표 "] --> E["단순 과거 실적 사후보고 탈피, 선제적 경영 의사결정 속도"]
    V --> P
```
