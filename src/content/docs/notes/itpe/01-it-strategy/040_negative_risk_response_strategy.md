...
- 본질: 부정적 위험 대응은 프로젝트 목표에 해를 줄 수 있는 사건에 미리 대처하는 일이다. 위험의 가능성과 영향을 보고 회피·완화·전가·수용 등을 고른다.
...
- **회피(Avoid)** : 계획·범위·기술 스택을 변경하여 위험 발생 원인을 근원적으로 제거하는 위협 대응 전략
- **전가(Transfer)** : 외주 계약·보험·보증 등을 통해 위험 발생 시의 재정적·운영적 책임을 제3자에게 이전하는 전략
- **완화(Mitigate)** : 사전 프로토타이핑·테스트 강화·이중화 설계를 통해 위험 발생 확률이나 부정적 영향을 임계치 이하로 축소하는 전략
- **수용(Accept)** : 대응 비용이 손실보다 크거나 통제 불가능할 때 우발사태 예비비(Contingency Reserve)를 확보하고 수용하는 전략
- **상위보고(Escalate)** : PM의 권한과 프로젝트 범위를 벗어나는 전사적·조직적 위험을 상위 프로그램이나 포트폴리오 경영진에게 이관하는 전략
- **잔여위험·2차위험** : 위험 대응 조치 실행 후에도 통제 범위 내에 잔존하는 위험(잔여위험) 및 대응 실행 과정에서 파생된 새로운 위험(2차위험)
...
> 부정적 위험의 대응 전략과 잔여위험 관리 방식을 설명하시오. (예상·10점)
