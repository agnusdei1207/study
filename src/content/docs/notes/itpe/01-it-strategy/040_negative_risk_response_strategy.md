...
- **회피(Avoid)**: 범위·접근법·일정 등을 변경해 위협의 원인을 제거하거나 목표를 보호하는 전략. 이는 해당 용어의 역할과 작동을 설명한다.
- **전가(Transfer)**: 보험·보증·계약 등을 통해 위협의 재무적·수행 책임을 제3자에게 이전하는 전략. 이는 해당 용어의 역할과 작동을 설명한다.
- **완화(Mitigate)**: 위협의 발생가능성 또는 영향을 허용 가능한 수준으로 낮추는 전략. 이는 해당 용어의 역할과 작동을 설명한다.
- **수용(Accept)**: 적극 조치를 취하지 않거나 예비비·예비계획을 준비한 채 위험을 인정하는 전략. 이는 해당 용어의 역할과 작동을 설명한다.
- **상위보고(Escalate)**: 프로젝트 범위·권한 밖의 위협을 적절한 상위 조직에 이관하는 전략. 이는 해당 용어의 역할과 작동을 설명한다.
- **잔여위험·2차위험**: 대응 후 남은 위험과 대응 조치로 새로 발생한 위험. 이는 해당 용어의 역할과 작동을 설명한다.
...
위협별 트리거와 대응 책임자를 등록하고 실행 후 잔여·2차위험을 재평가해 예비계획을 갱신한다.

```mermaid
flowchart LR
 A[위협 트리거] --> B[대응전략·책임자 지정] --> C[예비계획 실행] --> D[잔여·2차위험 평가] --> E[위험등록부 갱신] --> A
```
