...
| 구분 | 핵심 |
|---|---|
| 정의 | **FinOps** 는 엔지니어링·재무·사업 부서가 클라우드 사용량과 비용·가치를 함께 살펴 개선하는 운영 방식이다. |
| 목적 | 클라우드 비용을 사용량과 사업 가치에 맞춰 관리한다. |
...
> 제안: 비용 총액만 줄이는 대신 대표 서비스의 **거래 1건당 비용** 과 응답시간을 함께 보고, 절감 조치가 사용자 경험을 해치지 않는지 확인한다.

| 판단 대상 | 함께 확인할 값 | 결정 |
|---|---|---|
| 최적화 전·후 | 거래량, 거래당 비용, 응답시간 | 비용만 개선되고 성능이 악화되면 조치 재검토 |

Ⅴ절은 비용 귀속과 조치 책임의 일반적인 문제를 비교했다. 이 제안은 **한 서비스에서 어떤 값으로 최적화의 채택 여부를 결정할지** 정한다. 거래당 비용이 적절하지 않은 서비스라면 해당 서비스의 산출 단위를 대신 사용한다.
