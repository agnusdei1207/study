---
title: "EVM"
author: "Codex"
date: "2026-09-22T23:35:00+09:00"
tags:
  - "notes-it-strategy"
sidebar:
  badge:
    text: "B"
extra:
  keyword_grade: "B"
  model: "Gemini 3.8 Flash"
---

## 지식 로드맵 내 현재 위치

지식 위치: IT 전략·관리 → 프로젝트 통제·원가 관리 → **EVM**

## 30초 인출

- 본질: EVM은 수행한 작업의 가치를 계획·실제 비용과 비교해 일정·원가 상태를 확인하는 관리 기법이다.
- 메커니즘: **PMB** 수립 → **PV** · **EV** · **AC** 측정 → 편차·성과지수 분석 → **EAC** 예측 → 시정조치 순으로 통제한다.
- 산출물: 성과측정기준선 · 상태일 성과보고서 · 완료예상원가 · 변경기록이다.

<details>
<summary>핵심 용어</summary>

- **PMB(Performance Measurement Baseline)** : 승인된 범위·일정·원가를 통합하여 프로젝트 성과 측정의 기준이 되는 성과측정 기준선
- **PV(Planned Value)** : 특정 기준 시점까지 완료하기로 계획된 작업에 배정된 승인 예산(계획 가치)
- **EV(Earned Value)** : 특정 기준 시점까지 실제로 완료된 작업에 배정되었던 승인 예산(획득 가치)
- **AC(Actual Cost)** : 특정 기준 시점까지 완료된 작업을 수행하기 위해 실제로 지출된 총원가(실제 원가)
- **BAC(Budget at Completion)** : 프로젝트 전체 작업을 완수하기 위해 승인된 총예산(완료 시점 총예산)
- **EAC(Estimate at Completion)** : 현재까지의 원가·일정 성과 추세를 반영하여 최종 완료 시점에 소요될 것으로 예상되는 총비용(완료 시점 추정치)
- **ES(Earned Schedule)** : 획득 가치(EV)를 시간 단위로 환산하여 프로젝트 후반부 SPI 왜곡(1.0 수렴)을 보완하는 일정 성과 관리 기법

</details>

---

## 1교시 예상문제 (10점)

> EVM의 핵심 지표와 일정·원가 성과 판정 방식을 설명하시오. (예상·10점)

---

## 1교시 10점 답안

### 1. 정의·목적

- 정의: **EVM(Earned Value Management)** 은 PV·EV·AC로 범위·일정·원가 성과를 통합 측정하는 기법
- 목적: 일정 지연·예산 초과 조기 식별

### 2. 핵심 구조 및 매커니즘

- 정의: **EVM(Earned Value Management)** 은 PV·EV·AC로 범위·일정·원가 성과를 통합 측정하는 기법
- 목적: 일정 지연·예산 초과 조기 식별

```mermaid
flowchart TD
    A["WBS 작업패키지"] --> B["PMB 수립"]
    B --> C["성과 측정"]
    C --> D["분석·예측"]
    D --> E["시정·변경"]
```

| 지표 | 산식 | 판정 |
|---|---|---|
| **CV** | `EV - AC` | 음수: 원가 초과 |
| **SV** | `EV - PV` | 음수: 일정 지연 |
| **CPI** | `EV / AC` | 1 미만: 원가 비효율 |
| **SPI** | `EV / PV` | 1 미만: 일정 지연 |

---

## 2~4교시 예상문제 (25점)

> EVM의 개념과 PV·EV·AC·BAC를 설명하고, CV·SV·CPI·SPI 산식과 EAC 예측모델 및 한계 대응방안을 제시하시오.

---

## 2~4교시 25점 답안

## Ⅰ. 범위·일정·원가 통합 관리, EVM의 개요

> EVM은 지출액이 아니라 완료된 작업가치를 기준선과 비교하며, 예측력은 EV 승인근거와 AC 적시성에서 결정됨.

- 정의: **PV** · **EV** · **AC** 를 비교하여 프로젝트 성과를 정량 측정하고 미래 원가를 예측하는 통합 관리 기법
- 목적: 일정 지연·예산 초과 조기 식별 · 선제적 공정 통제

## Ⅱ. EVM 성과 통제 프로세스

> 작업패키지별 기준선을 세우고 상태일마다 성과를 측정해야 편차와 예측이 같은 범위를 가리킴.

```mermaid
flowchart TD
    A["WBS 작업패키지"] --> B["PMB 수립"]
    B --> C["성과 측정"]
    C --> D["분석·예측"]
    D --> E["시정·변경"]
```

## Ⅲ. 핵심 산식과 EAC 선택

> 산식은 같아도 EAC는 미래 효율에 대한 가정이 다르므로 원인을 확인한 뒤 모델을 선택해야 함.

| 지표 | 산식 | 판정 |
|---|---|---|
| **CV** | `EV - AC` | 음수: 원가 초과 |
| **SV** | `EV - PV` | 음수: 일정 지연 |
| **CPI** | `EV / AC` | 1 미만: 원가 비효율 |
| **SPI** | `EV / PV` | 1 미만: 일정 지연 |

| 가정 | EAC | 적용 |
|---|---|---|
| 편차 일회성 | `AC + (BAC - EV)` | 원인이 제거됨 |
| CPI 지속 | `BAC / CPI` | 원가 효율이 지속됨 |
| CPI·SPI 지속 | `AC + (BAC - EV) / (CPI × SPI)` | 일정이 원가에도 영향 |
| 재추정 | `AC + Bottom-up ETC` | 기준선 가정이 무너짐 |

## Ⅳ. EVM의 문제점·대응책

> 주관적 진척·AC 지연·후반부 SPI 수렴을 통제하지 않으면 정밀한 산식도 잘못된 경보를 냄.

| 위험 | 대책 | 효과 |
|---|---|---|
| 주관적 진척 | **0/100 규칙** ·검수 산출물 기준 EV 승인 | 진척 부풀림 억제 |
| AC 반영 지연 | 발생주의 원가 연계 | CPI 시차 축소 |
| SPI 후반 수렴 | **ES(Earned Schedule)** ·주공정 병행 | 일정 지연 가시성 유지 |

## Ⅴ. 조기 경보 중심의 결론

> 임계값 자체보다 지표의 원인과 EAC 가정을 기록하고 그에 맞는 시정조치를 실행하는 것이 핵심임.

### 실전 답안용 기술사적 제언

- 문제: 수행사의 주관적 진척 부풀림으로 인해 EV가 왜곡되고, 사업 후반부로 갈수록 일정 지연에도 불구하고 SPI가 1.0에 수렴하는 착시 현상으로 위기 대응 시기를 실기함.
- 해결 방안: 0/100 규칙 및 공식 산출물 승인 기준의 객관적 EV 측정 체계를 확립하고, 일정 성과 측정 시 시간 단위를 직접 반영하는 ES(Earned Schedule) 기법을 병행 도입하여 완료 시점(EAC(t))과 최종 원가(EAC)를 정밀 예측함.

```mermaid
flowchart TD
    subgraph Data["1. 성과 측정 기준선(PMB) 및 실측 데이터"]
        D1["WBS 작업패키지별 예산 할당 (PV)"]
        D2["산출물 승인 기반 실제 완성 가치 실측 (EV)"]
        D3["실제 투입 원가 집계 (AC)"]
        D1 & D2 & D3 --> D4["편차 및 지수 산출 (CV, SV, CPI, SPI)"]
    end
    subgraph ES_Method["2. Earned Schedule(ES) 병행 분석"]
        S1["ES(획득일정) 산출: 시간축 상의 진척 측정"]
        S2["SPI(t) = ES / AT (후반부 1.0 수렴 왜곡 제거)"]
        D4 --> S1 --> S2
    end
    subgraph Forecast["3. 미래 예측 및 통제 조치"]
        F1["최종 원가 예측: EAC = AC + (BAC - EV) / CPI"]
        F2["최종 일정 예측: EAC(t) = SAC / SPI(t)"]
        F3["임계치 초과 시 즉각 CCB 소집 및 공정 만회 대책 수립"]
        D4 --> F1
        S2 --> F2
        F1 & F2 --> F3
    end
```

## 출제 이력과 검증 출처

- 공식 문제지 원문으로 확인한 직접 기출 없음
- [PMI: The Standard for Earned Value Management](https://www.pmi.org/standards/earned-value-management)
- [U.S. Department of Energy: EVMS Gold Card](https://www.energy.gov/em/earned-value-management-system-evms)

## 연결 토픽

- 이전 토픽: [CRM](./031_crm.md)
- 연관 토픽: [PMO](./004_pmo.md), [WBS](./007_wbs.md), [프로젝트 위험관리](./009_project_risk_management_negative.md), [CCPM](./112_critical_chain_toc.md)
- 다음 토픽: [IT 아웃소싱](./033_it_outsourcing.md)
