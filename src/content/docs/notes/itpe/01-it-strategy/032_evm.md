...
title: "EVM"
author: "Codex"
date: "2026-09-21T23:02:00+09:00"
...
- 본질: **EVM(Earned Value Management)**은 범위·일정·원가를 화폐가치로 통합해 현재 성과와 완료예상원가를 함께 판단하는 관리 기법
- 메커니즘: PMB 수립 → PV·EV·AC 측정 → CV·SV·CPI·SPI 분석 → EAC 예측 → 시정조치
- 산출물: 성과측정기준선 · 상태일 성과보고서 · 완료예상원가 · 변경기록

<details>
<summary>핵심 용어</summary>

- **PMB(Performance Measurement Baseline)**: 승인된 범위·일정·원가를 결합한 성과 측정 기준선
- **PV(Planned Value)**: 상태일까지 계획한 작업의 예산가치
- **EV(Earned Value)**: 상태일까지 실제 완료한 작업의 예산가치
- **AC(Actual Cost)**: 상태일까지 실제 발생한 원가
- **BAC(Budget at Completion)**: 전체 작업의 승인예산
- **EAC(Estimate at Completion)**: 현재 성과와 가정을 반영한 완료예상원가
- **ES(Earned Schedule)**: EV를 시간축에 대응시켜 일정 성과를 보완하는 기법
...
```mermaid
flowchart TD
    A["WBS<br/>활동: 작업패키지 정의<br/>산출: WBS 사전"] --> B["PMB<br/>활동: 일정·예산 배분<br/>산출: S-Curve"]
    B --> C["성과 측정<br/>활동: PV·EV·AC 집계<br/>산출: 상태일 데이터"]
    C --> D["분석·예측<br/>활동: 편차·지수·EAC 계산<br/>산출: 성과보고서"]
    D --> E["시정·변경<br/>활동: 원인조치·승인<br/>산출: 조치·변경기록"]
```
...
```mermaid
flowchart TD
    A["문제: 주관적 EV·AC 지연·SPI 수렴"] --> B["대안: 산출물 EV·ES·발생원가"]
    B --> C["판정: 승인근거·가정·주공정 편차"]
    C --> D["효과: 조기경보·완료예측 개선"]
```
...
- 산식: `CV=EV-AC`, `SV=EV-PV`, `CPI=EV/AC`, `SPI=EV/PV`
