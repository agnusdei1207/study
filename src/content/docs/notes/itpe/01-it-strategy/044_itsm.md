...
- 본질: ITSM은 고객에게 제공하는 IT 서비스를 기준으로 장애·변경·배포와 개선을 관리하는 방식이다.
...
| 구분 | 핵심 |
|---|---|
| 정의 | **ITSM**은 IT 서비스의 설계·전환·제공·개선을 관리하는 체계다. |
| 목적 | 서비스 요구를 충족하면서 고객과 조직에 가치를 제공한다. |
...
| 문제 | 해결 방안 |
|---|---|
| 티켓 처리량이나 개별 시스템 가동률만 보면 서비스 이용자의 중단 영향과 반복 장애의 비용이 드러나지 않을 수 있다. | 서비스별 책임자를 정하고 장애·변경·사용자 영향 자료를 함께 검토해 개선 우선순위와 투자 결정을 주기적으로 갱신한다. |

이는 ITSM의 운영 요구사항을 조직 성과관리로 연결하기 위한 제안이며 특정 지표나 조직구조를 표준 의무로 보는 뜻은 아니다.
