...
- [핵심 통찰]: 투자 심사역이나 경영진이 불신하는 사업계획서는 거대 시장에 임의의 점유율을 곱해 매출을 단정하는 논리임. TAM은 성장 가능 범위를 보여주는 지표일 뿐이며, 사업성은 바텀업으로 검증된 SOM과 고객 단위 수익성으로 판단함.
...
```mermaid
flowchart TD
    P["거시 보고서 단순 인용 여부 판정, 유닛 이코노믹스($LT"] --> A["Top-down(성장 한계선)과 Bottom-up(실행 영"] --> V["시장 경계 정의, 경쟁사 윈백 비용 반영, 분기별 실제 영"] --> E["시장 규모 과대 추정에 따른 데스밸리(Death Valle"]
    V --> P
```
