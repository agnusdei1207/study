...
- **PLM(Product Lifecycle Management)**: 제품 전 생애주기의 사람·데이터·프로세스·시스템을 잇는 정보 중추이다.
- **BOM(Bill of Materials)**: 제품을 이루는 부품·수량·상하위 관계의 구조이다.
- **EBOM(Engineering Bill of Materials)**: 설계 관점의 제품 구조, 제조 관점 구조와 대응 관계 유지가 핵심이다.
- **MBOM(Manufacturing Bill of Materials)**: 생산·조립 관점의 제품 구조, 작업과 자재 집행의 기준이다.
- **Digital Thread**: 요구사항부터 현장 이력까지 제품 정보를 추적 가능하게 연결하는 흐름이다.
- **Baseline**: 승인 이후 변경 절차 없이 임의 수정할 수 없는 제품 정의의 기준 시점이다.
...
- **판정 기준 (Trigger)**: 설계변경(ECO) 발생 시 EBOM-MBOM 간 미매핑 부품 존재율 > 목표 기준, 또는 현장 생산라인에서 구버전 도면/BOM 작업 적발 시 즉시 배포 중단.
...
- **기대 효과 (Impact)**: 불량 설계로 인한 폐기·재작업 비용 목표 기준 절감, 신제품 개발(Time-to-Market) 목표 기준 단축, 제조물 책임(PL) 소송 방어를 달성함.

```mermaid
flowchart TD
    P["설계변경(ECO) 발생 시 EBOM-MBOM 간 미매핑 부"] --> A["PLM-ERP-MES 간 변경 배포 시 자동 유효성 검증 "] --> V["Digital Thread 기반으로 부품 단위 요구사항 ↔"] --> E["불량 설계로 인한 폐기·재작업 비용 목표 기준 절감, 신제"]
    V --> P
```
