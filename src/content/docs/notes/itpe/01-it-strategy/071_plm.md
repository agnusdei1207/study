...
- 본질: 제품 기획부터 설계·생산·서비스·폐기까지 제품 정의와 변경 이력을 통합 관리하는 시스템
- 메커니즘: 요구사항/CAD(EBOM) → 형상·변경승인(ECO) → 제조전개(MBOM) → ERP/MES 실행 → 품질/서비스 환류
- 판정 기준: EBOM-MBOM 간 미매핑 부품 0건 및 현장 구버전 도면 작업 0건 통제
...
```mermaid
flowchart TD
    S1["① 제품 정의 (기획·설계)<br/>요구사항 · 3D CAD · 형상 구조 연결<br/>(산출: EBOM · 도면 · 사양서)"]
    S2["② 형상·변경 통제 (ECO)<br/>영향도 분석 · CCB 심의 · 승인 및 버전 확정<br/>(산출: Baseline · 변경 이력)"]
    S3["③ 제조 전개 (생산준비)<br/>설계 구조와 공정·라우팅·소모자재 대응<br/>(산출: MBOM · 작업 표준)"]
    S4["④ 생산·서비스 환류<br/>ERP/MES 생산실적 및 품질·고장 이력 연결<br/>(산출: Digital Thread · 설계 개선요구)"]
...
```mermaid
flowchart TD
    G1["변경 패키지 (BOM · 도면 · 적용시점 · 영향시스템)"] --> G2{"변경 배포 게이트<br/>승인형상 ↔ EBOM-MBOM ↔ 배포대상 일치?"}
    G2 -->|통과| G3["ERP · MES 즉시 배포 및 적용 이력 기록"]
    G2 -->|미통과| G4["배포 보류 · 누락 매핑 보완 및 설계 재검토"]
```
...
```mermaid
flowchart TD
    P1["현행 한계<br/>분산 문서 · 수작업 BOM 전환 오류 · 현장 구버전 도면 혼용"] --> P2["개선 대안<br/>대표 제품군 파일럿 · EBOM-MBOM 자동매핑 · 배포 게이트"]
    P2 --> P3{"검증 판정<br/>EBOM-MBOM 미매핑 부품 0건 & 구버전 작업 0건?"}
    P3 -->|달성| P4["실행 효과<br/>폐기/재작업비 40% 절감 · Time-to-Market 20% 단축"]
    P3 -->|미달| P5["보완 조치<br/>Digital Thread 양방향 추적성 전수 재감사 및 작업 중단"]
```
