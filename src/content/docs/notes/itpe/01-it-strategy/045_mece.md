...
| 구분 | 핵심 |
|---|---|
| 정의 | **MECE**는 하나의 분할 기준으로 대상을 중복 없이 빠짐없이 구조화하는 원칙이다. |
| 목적 | 복잡한 문제를 분석 가능한 항목으로 나누고 중복·누락을 확인한다. |
...
| 문제 | 해결 방안 |
|---|---|
| 문제 구조를 만든 사람이 임의의 분할축을 고르면 겉으로는 완전해 보여도 항목 간 중복이나 실제 업무 누락이 남을 수 있다. | 분석에 착수하기 전에 범위·분할축·항목 포함 기준을 기록하고, 업무 담당자와 독립 검토자가 반례와 미분류 사례를 확인하도록 한다. |

이는 MECE를 실제 의사결정에 쓰기 위한 검토 절차 제안이며 어떤 한 가지 분할축이 모든 문제에 적합하다는 뜻은 아니다.
