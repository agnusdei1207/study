...
| 구분 | 핵심 |
|---|---|
| 정의 | **시스템 운영·유지보수 감리**는 독립된 감리자가 운영·유지보수 기준 준수와 통제 실효성을 증적으로 확인하는 활동이다. |
| 목적 | 서비스 안정성과 계약 이행을 확인하고 개선점을 찾는다. |
...
| 문제 | 해결 방안 |
|---|---|
| 모든 운영서비스를 같은 주기와 깊이로 점검하면 핵심서비스의 중대한 변화나 취약 증적에 감리역량을 집중하기 어렵다. | 서비스 중요도·변경빈도·사고이력으로 감리 우선순위를 정하고, 고위험 서비스는 표본 확대와 이행확인을 강화한다. 주기와 표본은 감리계획에서 근거와 함께 정한다. |
