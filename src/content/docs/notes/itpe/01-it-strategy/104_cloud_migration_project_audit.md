...
- **판정 기준**: 업무별 **6R** 전략 적합성, CSP **공유책임** RACI 매핑, 데이터 대사 결과 및 **FinOps** 태깅 준수 여부..
- **대응 방안**: 4단계 감리 게이트 운영, IaC 템플릿 정적 분석을 통한 보안 가드레일 점검, CSP 종속 대비 Exit Plan 수립.
- **검증 체계**: 모의이행(Dry Run) 수행 증적과 원천·목표 데이터의 건수·금액 대사 로그를 검증한다..
- **기대 효과**: 이전 실패 위험과 비용 통제 누락을 줄이고 공공 서비스의 안전한 이행을 확인한다..

```mermaid
flowchart TD
    STRAT["6R 전략·공유책임 점검"] --> IAC["IaC·권한 설정 감리"]
    IAC --> DRY["모의이행·데이터 대사"]
    DRY --> GATE["전환 품질 게이트"]
    GATE -->|잔여위험| PLAN["시정계획·재감리"]
    PLAN --> IAC
```
