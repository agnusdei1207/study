...
- **Risk Register(위험 등록부)**: 식별한 위험의 원인·책임자·분석 결과·대응·상태를 기록하고 추적하는 문서다.
- **Residual Risk(잔여 위험)**: 대응을 실행한 뒤에도 남아 계속 감시해야 하는 위험이다.
- **Secondary Risk(2차 위험)**: 위험 대응을 실행한 결과 새로 발생하는 위험이다.
- **Risk Owner(위험 책임자)**: 위험 상태를 감시하고 대응전략과 조치의 실행을 관리하는 책임자다.
- **Trigger(트리거)**: 대응을 시작하거나 위험 상태를 재평가하게 하는 관측 가능한 조건이다.
...
- 정의: 프로젝트 목표에 영향을 줄 수 있는 **불확실성**을 **위험 등록부(Risk Register)**에 식별하고 분석·대응·감시하는 **위험관리** 활동
- 목적: 위협의 발생가능성과 영향을 낮추고 **위험 소유자(Risk Owner)**가 기회를 실현하도록 의사결정을 지원하는 것
위험은 미래의 불확실성이고, 이슈는 이미 발생해 해결이 필요한 현재 문제이므로 두 대상을 구분해 관리한다.
