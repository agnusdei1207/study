...
- **대응 방안**: 상용SW 직접구매 대상 여부 검토, 과업심의위원회를 통한 과업 확정 및 인터페이스 기반 분리
- **검증 체계**: 요구사항추적표(RTM) 기반 과업-계약-검수 추적성 검증, 하도급지킴이 직불 내역 정기 모니터링
- **기대 효과**: 다단계 마진 누수 감소, 개발자 처우 개선 및 공공 SW 사업의 예산 낭비·품질 결함 예방

```mermaid
flowchart TD
    R["업무 요구사항"] --> C{"표준 기능으로 충족 가능한가"}
    C -->|가능| S["상용SW 적용·직접구매 대상 검토"]
    C -->|불가| D["차별 업무 맞춤개발 범위 확정"]
    S --> I["인터페이스·검수 기준 합의"]
    D --> I
    I --> T["요구사항·계약·검수 결과 대조"]
    T -->|변경 필요| R
```
