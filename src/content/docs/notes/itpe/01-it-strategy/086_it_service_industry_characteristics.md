...
- 본질: SW의 **무형성**·주문생산성으로 인해 정보 비대칭과 다단계 하도급, **헤드카운팅** 관행이 발생하는 국내 **IT서비스**의 고유 속성이다.
...
- **IT서비스(SI/SM)**: 고객 요구를 분석해 정보시스템을 기획·구축(SI)하거나 운영·유지보수(SM)하는 서비스를 제공하는 산업이다.
- **Intangibility(무형성)**: 소프트웨어 개발 완료 전까지 물리적 실체와 동작 상태를 눈으로 확인할 수 없는 특성이다.
- **Headcounting(헤드카운팅)**: 결과물의 기능 가치가 아닌 투입된 엔지니어의 인원수와 투입 기간(M/M)을 기준으로 대가를 산정하는 방식이다.
- **과업심의위원회**: 공공 SW 사업에서 발주자의 과업 내용 변경 및 사업 기간 연장 요구를 객관적으로 심의하여 계약금액을 조정하는 법정 기구이다.
- **하도급 제한**: 원수급자의 과도한 하도급과 재하도급을 제한하여 사업 책임과 품질 관리가 유지되도록 하는 규정이다.
- **Composable IT**: 검증된 SaaS와 재사용 모듈을 API로 조합해 업무 요구에 맞는 시스템을 빠르게 구성하는 방식이다.
- **하도급대금 직불제**: 원수급자를 거치지 않고 발주자가 하수급인에게 하도급대금을 직접 지급하는 제도이다.
- **상용SW 직접구매**: 맞춤 개발 대신 상용 SW를 발주처가 직접 구매하도록 분리발주하는 조달 방식이다.
