...
- 본질: **EA·ITA**는 조직의 업무·데이터·응용·기술 구조를 전사 관점에서 정리하고 변화에 맞춰 관리하는 아키텍처다.
- 메커니즘: 현행·목표 구조를 비교해 차이를 찾고, 이행과제와 관리기준을 정한다.
...
> 범정부 EA 참조모형과 업무·데이터·응용·기술 뷰의 연계를 설명하시오. (예상·10점)
...
| 구분 | 핵심 |
|---|---|
| 정의 | **EA·ITA**는 조직의 업무·데이터·응용·기술 구조를 전사 관점에서 정리하고 변화에 맞춰 관리하는 아키텍처다. |
| 목적 | 중복 투자를 줄이고 정보자원 간 일관성과 상호운용성을 높인다. |

### Ⅱ. 참조모형과 아키텍처 뷰

```mermaid
flowchart TD
    subgraph VIEWS["기관의 아키텍처 뷰"]
      BA["업무"]
      DA["데이터"]
      AA["응용"]
      TA["기술"]
...
제언: 주요 투자·사업 심의 때 EA 원칙과 예외 사유를 함께 검토하도록 한다.
...
참조모형은 기관의 아키텍처를 일정한 관점에서 분류·비교하도록 돕는다. PRM은 성과 관점이며, 나머지 모형과 일대일 대응한다고 보지 않는다.
...
| 문제 | 해결 방안 |
|---|---|
| 아키텍처 현행화가 담당자의 기억에 의존하면 투자 심의 때 기준정보가 오래될 수 있음 | 시스템 변경·사업 종료 절차에 아키텍처 갱신 책임과 확인 시점을 포함 |
