...
- **BIA(Business Impact Analysis)** : 업무 중단의 시간대별 영향을 분석해 복구 우선순위와 목표를 정하는 활동이다.
- **BCP(Business Continuity Plan)** : 재난 시 핵심 업무를 지속·복구하기 위한 조직·절차·자원 계획이다.
- **DRS(Disaster Recovery System)** : 주 시스템 장애 시 대체 환경에서 서비스를 복구하도록 구성한 시스템이다.
- **Failover(장애 조치)** : 장애가 난 주 시스템의 역할을 대기 시스템으로 전환하는 동작이다.
- **Failback(복귀)** : 주 시스템 복구 후 서비스와 데이터를 원래 환경으로 되돌리는 동작이다.
