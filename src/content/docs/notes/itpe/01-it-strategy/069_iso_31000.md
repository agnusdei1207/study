...
```mermaid
flowchart TD
    S1["① 범위·상황·기준 설정<br/>조직 목표·이해관계자·평가 기준 확정<br/>(산출: Context & Criteria)"]
    S2["② 위험 식별·분석·평가<br/>위험요인 도출 및 발생가능성/영향 분석<br/>(산출: Risk Register · 우선순위)"]
    S3["③ 위험 처리 (Treatment)<br/>대응전략(회피/전가/완화/수용) 실행<br/>(산출: Treatment Plan)"]
    S4{"④ 잔여 리스크 판정<br/>조직 허용 한계(Risk Appetite) 만족?"}

    S1 --> S2 --> S3 --> S4
    S4 -->|만족/통과| P_PASS["수용 및 상시 모니터링·보고<br/>(기록/보고 및 KRI 추적)"]
    S4 -->|미달/미통과| P_FAIL["추가 처리 수립 또는 기준 재검토<br/>(Risk Owner 비상 에스컬레이션)"]
    P_FAIL -.-> S3
```
...
```mermaid
flowchart TD
    P1["현행 한계<br/>목록 나열식 페이퍼 관리 · 정적 연례 평가 · 잔여 위험 방치"] --> P2["개선 대안<br/>목표별 Risk Owner 지정 · KRI 모니터링 단축 · 잔여 리스크 승인"]
    P2 --> P3{"검증 판정<br/>잔여 리스크 수준 <= 조직 수용 한계(Tolerance)?"}
    P3 -->|달성| P4["실행 효과<br/>불확실성 선제 회복력(Resilience) · 거버넌스 투명성 확보"]
    P3 -->|미달| P5["보완 조치<br/>C-Level 책임자 공식 승인 및 완충 예산(Contingency) 배정"]
```
