...
- 본질: 목표에 미치는 불확실성의 영향을 관리하기 위한 원칙·프레임워크·프로세스의 범용 국제표준이다.
...
- **Risk** : 불확실성이 목표에 미치는 영향으로, 기대보다 나은 결과와 나쁜 결과를 모두 포함하는 개념이다.
- **Risk Assessment** : 리스크를 식별하고 발생 가능성과 영향을 분석한 뒤 처리 우선순위를 평가하는 과정이다.
- **Risk Treatment** : 리스크를 회피·감소·공유·수용할 대안을 선택하고 실행한 뒤 잔여 리스크를 다시 평가하는 과정이다.
- **Risk Owner** : 특정 리스크의 처리 권한과 관리 책임을 맡아 대응 승인과 상태 모니터링을 수행하는 주체이다.
