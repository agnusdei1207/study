...
- 본질: BSC(Balanced Scorecard, 균형성과표)는 재무·고객·내부 프로세스·학습과 성장 관점으로 전략 실행 성과를 관리하는 체계
- 메커니즘: 학습·성장 역량 → 내부 프로세스 → 고객·재무 성과의 인과관계 검증
...
| 구분 | 핵심 |
|---|---|
| 정의 | **BSC** 는 전략 목표를 재무·고객·내부 프로세스·학습과 성장의 네 관점으로 연결해 관리하는 성과관리 체계 |
| 목적 | 재무·비재무 성과의 균형 관리와 전략 실행 지원 |
...
**제언:** 전략목표와 연결되지 않는 지표의 재검토와 결과의 실행과제 반영

---

## 2~4교시 예상문제 (25점)

> BSC(Balanced Scorecard)의 개념과 네 관점·전략 목표의 관계를 설명하고, 성과관리 적용 시 고려사항을 제시하시오. (예상·25점)
...
> 역량·인프라(학습·성장) → 프로세스 혁신 → 고객 만족 → 재무 성과로 이어지는 인과 사슬
...
> 각 관점의 공통 전개 구조인 전략 목표 → **KPI(Key Performance Indicator)**·목표치·실행과제, 인과 검증을 위한 측정 단계의 **선행지표(Leading)**·**후행지표(Lagging)** 쌍 배치

선행지표 개선만으로 후행 성과가 자동 개선되지 않는 점. 교육 이수 후 업무 처리시간·고객 만족도의 동반 변화 확인을 통한 전략지도 인과 가설 검증
...
> **IT-BSC**의 IT 조직 역할 재정의: 단순한 **비용 센터(Cost Center)**에서 전략적 **가치 창출자(Value Enabler)**로의 관점 전환
...
> 지표 난립·인과 단절에 따른 평가 무용화 방지: 선행지표 변화의 후행지표 연결 여부를 먼저 판정하고 미충족 지표를 관점에서 제외
...
| 위험 | 대책 | 효과 |
|---|---|---|
| **관점 간 인과관계 단절** | 전략 체계도(Strategy Map)를 활용해 목표 사이의 인과 가정을 검토 | 관점 간 연결 누락·약한 가정 점검 |
...
> 제안: 전략체계도의 모든 화살표를 인과관계로 단정하지 않고, 중요한 **선행지표·후행지표 한 쌍**의 기간별 변화를 함께 검토
...
Ⅴ절의 지표 운영 문제와 구분되는 **전략체계도 연결의 검증 방법**. 동반 변화만으로 인과가 입증되지 않으므로 외부 요인·업무 맥락의 함께 검토
