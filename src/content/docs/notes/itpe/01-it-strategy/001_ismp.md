...
> ISMP의 정의와 5단계 수행 흐름, ISP와의 차이를 설명하시오. (예상·10점)
...
- 정의: 특정 정보시스템의 요구사항·구조·규모·비용을 구체화해 발주 기준을 정하는 계획
...
| 구분 | 핵심 |
|---|---|
| 정의 | 특정 정보시스템의 요구사항·구조·규모·비용을 발주 가능한 수준으로 구체화해 구축 기준을 정하는 정보화 계획 |
...
> 제안: 발주 단위를 확정할 때 요구사항 중 설계·규모·검수조건 중 하나라도 미결인 항목은 먼저 따로 표시하고, 영향이 큰 항목부터 확정 여부를 결정한다.
...
Ⅳ절이 여러 발주 위험과 대책을 비교했다면, 이 제안은 **미결 요구를 발주 범위에서 어떻게 다룰지** 정하는 판단 절차다. 미결 항목을 무조건 제외하거나 자동 승인하지 않는다.
