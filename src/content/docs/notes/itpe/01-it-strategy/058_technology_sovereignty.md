...
- **판정 기준 (Trigger)**: 핵심 국가 기간망 및 주요 금융·공공 서비스의 단일 외산 SW/인프라 의존도(Herfindahl 지수)가 목표 기준를 초과하거나 대체 전환시간(MTTS) > 7일일 때 공급망 주권 위기로 판정.
...
```mermaid
flowchart TD
    P["핵심 국가 기간망 및 주요 금융·공공 서비스의 단일 외산 "] --> A["공공 조달 시 개방형 표준(Open Standard) 및 "] --> V["연 1회 모의 공급망 단절(Vendor Blackout) "] --> E["지정학적 기술 수출 통제 및 벤더 라이선스 정책 급변에 대"]
    V --> P
```
