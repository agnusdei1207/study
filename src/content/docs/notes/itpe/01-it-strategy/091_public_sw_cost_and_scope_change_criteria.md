...
- 본질: 무리한 납기 방지를 위한 발주 전 '**적정 사업기간**' 산정과 수행 중 무상 과업 추가 방지를 위한 '**과업심의**' 통제 제도이다.
...
- **SW(Software)**: 요구사항을 처리하고 업무 기능을 제공하도록 개발·운영되는 소프트웨어 산출물이다.
- **FP(Function Point)**: 사용자가 요구하는 논리적 기능의 규모를 수치로 나타내는 측정 단위이다.
- **RTM(Requirements Traceability Matrix)**: 요구사항이 설계·구현·시험·계약 산출물에 어떻게 반영됐는지 연결해 기록하는 표이다.
- **Baseline**: 범위·일정·원가를 승인해 이후 변경통제의 비교 기준으로 삼는 기준 상태이다.
- **과업심의위원회**: 공공 SW 사업의 과업을 확정하고 변경에 따른 비용·기간 조정 필요성을 심의하는 기구이다.
...
- [ ] Ⅰ: **적정 사업기간**과 **과업심의**의 목적을 설명할 수 있는가?
...
- [ ] Ⅵ: 심의 결과를 계약 **Baseline**까지 연결하는 통제안을 제시할 수 있는가?
