...
- [핵심 통찰]: **과업심의**위원회 의결서는 변경 범위와 대가·기간 조정의 근거가 된다. 발주자의 구두 요구에 따른 무상 과업 확대를 통제하고 정당한 대가(FP)와 기간 협의를 뒷받침한다.
- 나라면: 변경요청서 접수 즉시 범위·비용·일정·품질 4차원 영향평가서를 첨부하고, 심의 의결 즉시 변경계약서 체결 및 RTM-일정 베이스라인을 동기화하겠음.

### 실전 답안용 기술사적 제언

- **판정 기준**: 변경요청이 범위·비용·일정·품질과 계약조건에 미치는 영향 및 계약금액·기간 조정 필요성 판정.
- **대응 방안**: 소프트웨어진흥법 제50조 기반 **과업심의**위원회 가동, 4차원(범위·비용·일정·품질) 통합 영향분석 실시.
- **검증 체계**: 심의 의결서와 변경 계약서 간 추적성 검증, RTM(요구사항추적표) 및 WBS 일정 베이스라인 동시 갱신.
- **기대 효과**: 구두 과업 추가와 계약 분쟁을 줄이고 승인된 범위·대가·일정에 따른 수행 책임 확보.

```mermaid
flowchart TD
    CR["과업 변경요청"] --> IA["범위·비용·일정·품질 영향분석"]
    IA --> COM["과업심의위원회"]
    COM --> CON["변경계약 체결"]
    CON --> BASE["RTM·WBS Baseline 갱신"]
    BASE -->|이행 차이·추가 변경| IA
```
