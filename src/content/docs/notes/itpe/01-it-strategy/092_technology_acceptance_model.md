...
- 본질: 기술수용모델은 기술이 유용하고 사용하기 쉽다는 인식으로 사용 의도를 설명한다.
- 메커니즘: 사용 용이성은 유용성에도 영향을 주며, 두 인식은 사용 의도와 실제 사용에 연결된다.

<details>
<summary>핵심 용어</summary>

- **기술수용모델(TAM, Technology Acceptance Model)** : 지각된 유용성과 사용 용이성을 중심으로 기술 사용 의도를 설명하는 모델이다.
- **PU(Perceived Usefulness)** : 기술이 업무 성과에 도움이 된다고 사용자가 믿는 정도다.
- **PEOU(Perceived Ease of Use)** : 기술을 사용하는 데 드는 노력이 적다고 사용자가 믿는 정도다.
- **BI(Behavioral Intention)** : 기술을 사용할 의향이다.
- **UTAUT(Unified Theory of Acceptance and Use of Technology)** : 성과·노력 기대 등 여러 요인을 함께 다루는 기술수용 통합 이론이다.
...
| 구분 | 핵심 |
|---|---|
| 정의 | **기술수용모델(TAM)** 은 지각된 유용성과 사용 용이성으로 기술 사용 의도를 설명하는 모델이다. |
| 목적 | 사용자가 기술을 받아들이는 이유를 파악해 도입·개선 방향을 정한다. |
...
**제언:** 설문상 수용 의도와 실제 사용을 구분해 도입 효과를 확인한다.
...
기본 구조는 **PEOU**가 **PU**에 영향을 주고, 두 인식이 사용 의도를 설명한다. 연구에 따라 태도 등 매개 변수를 포함하는 모형도 있으므로, 모든 도입 사례에 같은 경로와 효과 크기를 단정하지 않는다.
...
설문 문항은 인식과 의도를 측정한다. 이용 로그는 행동을 보여주지만 사용 이유까지 설명하지 않으므로 인터뷰와 함께 본다.
...
수용 지표가 개선되어도 업무 성과가 변하지 않으면 기능 적합성과 업무 절차를 다시 검토한다.
