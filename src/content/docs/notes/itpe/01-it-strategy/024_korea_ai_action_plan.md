...
- **국가인공지능전략위원회**: 대통령 소속으로 AI 주요 정책을 심의·의결하는 위원회. 이는 해당 용어의 역할과 작동을 설명한다.
- **인공지능 기본계획**: 과학기술정보통신부장관이 3년마다 수립하고 위원회 심의·의결을 거치는 법정 계획. 이는 해당 용어의 역할과 작동을 설명한다.
- **AX(AI Transformation)**: 전 산업 및 공공 업무 전반에 인공지능을 내재화하여 생산성을 혁신하는 전환. 이는 해당 용어의 역할과 작동을 설명한다.
- **인공지능기본법**: AI 산업 육성 지원과 고영향 AI 안전·신뢰성 확보 의무를 법제화한 기본 법령. 이는 해당 용어의 역할과 작동을 설명한다.
- **고영향 AI(High-impact AI)**: 사람의 생명·신체·기본권에 중대한 영향을 미쳐 강화된 안전성 검증이 요구되는 AI. 이는 해당 용어의 역할과 작동을 설명한다.
- **NPU(Neural Processing Unit)**: 딥러닝 연산에 최적화된 저전력·고효율 인공지능 전용 반도체. 이는 해당 용어의 역할과 작동을 설명한다.
- **KPI(Key Performance Indicator)**: 정책 및 세부 사업의 목표 달성도를 정량 측정하는 핵심성과지표. 이는 해당 용어의 역할과 작동을 설명한다.
- **소버린 AI(Sovereign AI)**: 외산 빅테크에 종속되지 않고 자국의 데이터와 인프라로 자체 AI 역량을 보유하는 전략. 이는 해당 용어의 역할과 작동을 설명한다.
