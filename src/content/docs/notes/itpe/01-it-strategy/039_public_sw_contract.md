...
- **RFP(Request for Proposal)**: 사업 목적, 범위, 요구사항, 제안·평가기준, 계약조건을 제시하는 제안요청서이다.
- **기능점수(Function Point)**: 사용자 관점의 기능 규모를 측정해 비용·기간 산정에 활용하는 방식이다.
- **과업내용**: 계약상 수행해야 할 기능·업무·산출물·조건의 범위이다.
- **협상에 의한 계약**: 제안서 평가 후 우선협상대상자와 기술·가격·계약조건을 협상하는 방식이다.
- **RTM(Requirements Traceability Matrix)**: 요구사항을 설계·구현·시험·인수 증거와 연결하는 추적표이다.
- **계약 기준선(Baseline)**: 범위·일정·비용·품질·인수조건의 승인된 기준이다.
...
```mermaid
flowchart TD
    subgraph PreProcure["1. 발주 준비 및 심의 통제"]
        P1["요구사항 상세화 및 기능점수(FP) 기반 적정 대가 산정"]
        P2["과업심의위원회 개최 (과업 범위 적정성 사전 심의)"]
        P3["제안요청서(RFP) 공고 및 평가기준 사전 공개"]
    end
    subgraph Selection["2. 사업자 선정 및 기준선 확정"]
        S1["기술평가(80~90%) 중심 협상에 의한 계약 체결"]
        S2["범위·일정·비용 프로젝트 계약 기준선(Baseline) 확정"]
    end
    subgraph ChangeInspect["3. 과업변경 및 인수 통제"]
        C1["과업변경심의위원회 (계약금액·기간 조정 공식 승인)"]
        C2["요구사항 추적표(RTM) 기반 전수 시험 및 검수"]
        C3["무상 과업변경 원천 배제 및 지식재산권 공동 활용"]
    end

    PreProcure --> Selection
    Selection --> ChangeInspect
    ChangeInspect -.->|변경 승인 시 계약 갱신| Selection
```

- 판정: 과업 변경 시 구두 지시를 배제하고 과업심의위원회를 통한 적정 대가·기간 조정이 제도화되어 있는가
- 대안: RFP 상세화 가이드라인 준수 · 과업심의위원회 의무화 · RTM 기반 전 주기 추적
- 검증: 과업변경심의 의결서 확인 · RTM-테스트 케이스 매핑 대조 · 분할발주 및 기술평가 비중(90%) 적용 확인
- 효과: 공공 SW 사업의 유찰·품질 저하 방지 및 발주자-수주자 간 분쟁 예방과 공정 상생 생태계 조성
