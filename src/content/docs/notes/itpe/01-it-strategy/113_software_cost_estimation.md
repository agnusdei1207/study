...
- 본질: 소프트웨어 비용 산정은 요구사항과 개발 범위를 바탕으로 필요한 노력과 사업비를 추정하는 활동이다.
- 메커니즘: 기능·규모를 정하고, 선택한 산정 방식과 근거자료로 노력·비용을 계산해 예산과 계약 기준에 반영한다.
...
> 소프트웨어 비용 산정의 목적과 기능점수 기반 산정 흐름, 산정 근거의 통제 방법을 설명하시오. (예상·10점)
...
| 구분 | 핵심 |
|---|---|
| 정의 | **소프트웨어 비용 산정(Software Cost Estimation)**은 요구사항과 범위에 맞춰 소프트웨어 개발에 필요한 노력과 비용을 추정하는 활동이다. |
| 목적 | 현실적인 예산과 계약 기준을 마련하고, 범위 변경의 비용 영향을 판단한다. |
...
| 항목 | 검토 내용 |
|---|---|
| 기능 규모 | 경계·요구사항·기능별 산정 근거 합의 |
| 개발비 | 적용 가이드 판본의 단가·보정 기준과 계약 적용 항목 확인 |
...
기능점수 측정은 사용자 관점에서 측정할 애플리케이션의 경계를 정하는 데서 시작한다. 내부·외부 데이터 기능인 **ILF**·**EIF** 와 입력·출력·조회 트랜잭션인 **EI**·**EO**·**EQ** 를 분류하고, 적용되는 측정 규칙으로 기능점수를 산출한다.
...
산식은 적용 가이드에 맞춰 계산한다. 적용 가이드의 기능점수당 단가와 보정항목은 연도·판본·사업조건에 따라 달라질 수 있으므로, 고정 숫자를 모든 사업에 적용하지 않는다. 예시 사업에서는 규모·현재 복잡성·성능·운영환경·보안성을 보정 검토 항목으로 사용한다.
...
| 문제 | 해결 방안 |
|---|---|
| 기능점수만 기록하고 요구사항·경계·가이드 판본을 남기지 않으면 견적을 재현하거나 변경분을 검증하기 어렵다. | 기준선에 요구사항 식별자, 측정경계, 기능별 산정근거, 단가·보정기준의 판본을 함께 기록한다. 변경요청은 영향 기능과 승인 이력을 갱신한 뒤 증분 산정과 계약절차에 연결한다. |
