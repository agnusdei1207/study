...
- **DRS(Disaster Recovery System)**: 재해 입력을 받아 대체 환경에서 서비스·데이터·운영절차를 복구하고 정상화 결과를 검증하는 시스템·인프라 체계이다.
- **BCP(Business Continuity Plan)**: 중단 상황의 업무 우선순위와 대응 책임을 정해 핵심 업무의 지속·복구 결과를 관리하는 전사 계획이다.
- **BIA(Business Impact Analysis)**: 업무별 중단 영향과 의존성을 분석하여 복구 우선순위·RTO·RPO를 결정하는 활동이다.
- **RTO(Recovery Time Objective)**: 서비스 중단 입력 후 허용되는 복구 목표 시간을 정해 복구 완료 시점을 판단하는 기준이다.
- **RPO(Recovery Point Objective)**: 복구에 사용할 복제·백업 데이터를 선택할 때 허용되는 데이터 손실 시점을 정하는 기준이다.
- **Failover**: 주 센터 장애 상태를 감지하여 대체 센터로 서비스 요청을 전환하고 가동 결과를 확인하는 절차이다.
- **Failback**: 주 센터 복구 후 변경 데이터를 동기화하고 검증을 거쳐 원래 환경으로 서비스를 되돌리는 절차이다.
- **GSLB(Global Server Load Balancing)**: 복수 센터의 상태와 정책을 입력받아 정상 센터로 사용자 트래픽을 분산·전환하는 기술이다.
...
```mermaid
flowchart TD
    subgraph Primary["1. 주 센터 (Primary DC)"]
        P_App["운영 App / Web"]
        P_DB[("운영 DB / Storage")]
    end
    subgraph Replication["2. 복제 계층 (Replication & Sync)"]
        Sync["실시간 동기/비동기 복제"]
        CG["Consistency Group 정합성 통제"]
    end
    subgraph Secondary["3. 재해복구 센터 (DR DC)"]
        DR_DB[("대체 DB / Storage")]
        DR_App["대기/동시 가동 App"]
        DR_Common["독립 공통서비스 (IAM / DNS / KMS)"]
    end
    subgraph Governance["4. 절체 및 복구 거버넌스"]
        GSLB["GSLB 트래픽 전환 (Failover)"]
        Runbook["자동화 Runbook / IaC"]
        Failback["역방향 동기화 후 정상 원복 (Failback)"]
    end

    P_DB -->|스토리지 볼륨 복제| Sync --> DR_DB
    P_App -.->|장애 감지| GSLB
    GSLB -->|트래픽 우회 절체| DR_App
    DR_App --- DR_Common
    DR_DB -.->|복구 후 역동기화| Failback --> P_DB
```

- **판정 기준**: 재해 선포 후 목표 RTO 이내 핵심 서비스 정상 가동률 및 Failback 시 데이터 역전송 정합성 검증
- **공학적 대안**: 레거시 Standby 탈피, 클라우드 네이티브 멀티 리전 액티브-액티브 DRS 및 GSLB 기반 자동 절체 구현
- **검증 절차**: 네트워크 인위적 차단 환경에서 전사 공통 인증(IAM), 암호화 키(KMS) 서비스의 독립 기동 및 E2E 트랜잭션 검증
- **기대 효과**: 광역 재해 상황에서도 핵심 서비스 연속성 보장, 대외 신인도 방어 및 전자금융감독규정 등 컴플라이언스 준수
