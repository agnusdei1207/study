...
- 본질: **국정자원 화재와 공공 회복탄력성**은 단일 데이터센터 물리 설비 집중(SPOF)의 한계를 극복하고, 센터 상실 시에도 대민 행정서비스를 무중단 유지하는 다중 거점 연속성 체계이다.
...
- **SPOF(Single Point of Failure)**: 단일 구성요소 고장이 시스템 전체 마비로 파급되는 단일 실패점. 이는 해당 용어의 역할과 작동을 설명한다.
- **공통원인 장애(Common Cause Failure)**: 하나의 물리적 사건(화재·정전)이 공유 인프라를 통해 다수 서비스로 전파되는 장애. 이는 해당 용어의 역할과 작동을 설명한다.
- **GSLB(Global Server Load Balancing)**: DNS 기반 헬스체크를 통해 장애 센터를 배제하고 정상 거점으로 트래픽을 자동 분산. 이는 해당 용어의 역할과 작동을 설명한다.
- **Active-Active DR(이중운영체계)**: 복수 거점이 서비스를 운영하여 대기형 DR보다 전환시간을 줄이는 구성. 이는 해당 용어의 역할과 작동을 설명한다.
- **RTO(Recovery Time Objective)**: 재해 발생 시 서비스가 정상 수준으로 복구되기까지 허용되는 최대 시간. 이는 해당 용어의 역할과 작동을 설명한다.
- **RPO(Recovery Point Objective)**: 재해 발생 시 유실을 허용할 수 있는 최대 데이터 시점 간격. 이는 해당 용어의 역할과 작동을 설명한다.
- **에어갭(Air-Gap)**: 백업 저장소의 네트워크 연결을 물리적·논리적으로 단절해 악성코드와 원격 침해의 전파를 차단하는 방식. 센터 화재 등 물리 재난에는 별도 거점의 지리적 분리가 필요함. 이는 해당 용어의 역할과 작동을 설명한다.
- **Split-Brain**: 네트워크 단절 시 양 센터가 상호 다운으로 오판하여 독자 쓰기를 수행하며 발생하는 데이터 불일치 현상. 이는 해당 용어의 역할과 작동을 설명한다.
...
> 완전한 건물 이중화보다 언제든 한쪽 센터를 분리할 수 있는 **무상태(Stateless) 분산 구조**와 **실전 불시 절체 검증**이 회복탄력성의 본질임.
