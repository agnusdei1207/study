...
- 목적: 정보시스템 구축의 효율성·안전성 확보, 예산 낭비 선제적 방지 및 대국민 서비스 신뢰 보증
...
| **감리 관점** | **적합성(Compliance)**, **완전성(Completeness)**, **추적성(Traceability)**, **안전성(Security)** | 지침 표준 준수, 요건 누락 여부, RTM 추적 일관성, 재해복구(DR) |
...
| 실무 통제 위험 | 주요 발생 원인 | 대책 (통제 방안) | 효과 (검증 기준) |
|---|---|---|---|
| **형식적 서류 감리** | 짧은 현장감리 기간과 실측 도구 부재로 단순 문서 열람에 치중 | 정적 코드 분석(SonarQube), 부하 테스트(JMeter) 등 도구 활용 실측 감리 의무화 | 소스코드 결함 및 응답 지연 사전 제거 |
| **종료 단계 조치 지연** | 개통 일정 압박으로 감리 지적 미반영 상태에서 조건부 준공 검수 승인 | **감리결과 조치확인서**를 최종 준공 검수 대금 지급 조건과 법적으로 연계 | 미조치 항목의 공식 승인·책임 기록 |
| 감리-PMO 간 R&R 갈등 | 상시 사업관리 주체(PMO)와 독립 감사인(감리) 간 권한 및 의견 충돌 | 감리는 독립적 품질 평가, PMO는 지적 사항의 조치계획 실행 지원으로 역할 명문화 | 기관 간 갈등 방지 및 사업관리 시너지 창출 |
...
- 목적: 대규모 공공 정보시스템의 효율성·안전성 확보 및 서비스 신뢰 보증
