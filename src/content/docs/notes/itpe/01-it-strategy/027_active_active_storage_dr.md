...
author: "Codex"
date: "2026-09-21T22:31:00+09:00"
tags:
  - "notes-it-strategy"
sidebar:
  badge:
...
- 본질: **액티브-액티브 이중화와 스토리지 DR**은 복수 거점이 평시 워크로드를 함께 처리하고, 복제·쿼럼·펜싱으로 한 거점 장애의 서비스·데이터 영향을 줄이는 재해복구 구조
...
```mermaid
flowchart TD
    A["트래픽 분산<br/>활동: GSLB 상태 확인<br/>산출: 분산 정책"] --> B["애플리케이션 분산<br/>활동: 상태 외부화<br/>산출: 배치 구조"]
    B --> C["데이터 복제<br/>활동: 동기·비동기 선택<br/>산출: 복제 정책"]
    C --> D["쿼럼 감시<br/>활동: 거점·회선 상태 판정<br/>산출: Witness 로그"]
    D --> E["장애 격리<br/>활동: I/O Fencing·전환<br/>산출: 절체 결과"]
```
...
```mermaid
flowchart TD
    A["문제: 미검증 절체·Split-Brain"] --> B["대안: Witness·Fencing·N-1"]
    B --> C["판정: RTO·RPO·정합성·용량"]
    C --> D["효과: 장애 격리·연쇄 과부하 방지"]
```
