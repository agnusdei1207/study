...
- **RTO(Recovery Time Objective)**: 재해 발생 후 업무 기능이나 시스템을 사전에 정한 목표 수준으로 재개해야 하는 목표 시간 (`RTO < MTPD`)이다.
- **RPO(Recovery Point Objective)**: 재해 발생 시 백업 시점 기준으로 허용 가능한 최대 데이터 유실 시점 간격이다.
- **MBCO(Minimum Business Continuity Objective)**: 중단 기간 동안 최소한으로 제공해야 하는 비즈니스 연속성 목표 수준이다.
- **CMP(Crisis Management Plan)**: 전사 지휘통제 및 대외 커뮤니케이션을 관장하는 최고경영진 위기관리 계획이다.
- **ERP(Emergency Response Plan)**: 인명 보호·대피 등 현장 비상상황의 초동 대응을 담당하는 비상대응 계획이다.
- **DRP(Disaster Recovery Plan)**: IT 인프라, 네트워크, 데이터베이스 복구를 전담하는 기술적 재해복구 계획이다.
...
```mermaid
flowchart LR
    Crisis["재해·비상사태 발생"] --> CMP["위기관리 (CMP 지휘)"]
    CMP --> ERP["초동대응 (ERP 인명대피)"]
    ERP --> Branch{"업무 복구 경로"}
    Branch -->|"비즈니스 우선업무"| BCP_Exec["대체인력·수작업 (MBCO 유지)"]
    Branch -->|"IT 인프라·데이터"| DRP_Exec["시스템 절체·복원 (RTO/RPO)"]
    BCP_Exec & DRP_Exec --> Normal["정상 복구 및 교훈 환류 (PDCA)"]
```
