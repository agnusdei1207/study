...
```mermaid
flowchart TD
    subgraph Inbound["1. 인바운드 혁신 (Outside-In)"]
        IN1["외부 기술 탐색 및 대학/연구소 특허 라이선싱"]
        IN2["CVC(기업벤처캐피털) 기반 신기술 스타트업 인수/지분투자"]
    end
    subgraph Outbound["2. 아웃바운드 혁신 (Inside-Out)"]
        OUT1["내부 미활용 R&D 지식재산권(IP) 외부 기술 매각 및 라이선스 아웃"]
        OUT2["사내 벤처 분사 (Spin-off)를 통한 독자 사업화"]
    end
    subgraph Coupled["3. 결합형 혁신 (Coupled)"]
        C1["오픈소스 소프트웨어(OSS) 커뮤니티 공동 기여 및 활용"]
        C2["산학연 컨소시엄 및 조인트 벤처(Joint Venture) 설립"]
    end

    Inbound & Outbound & Coupled --> VALUE["전사 혁신 속도(Time-to-Market) 가속화 및 생태계 선점"]
```
...
- [ ] Ⅰ: **개방형 혁신** 의 정의·목적을 두 줄로 재현할 수 있는가?
