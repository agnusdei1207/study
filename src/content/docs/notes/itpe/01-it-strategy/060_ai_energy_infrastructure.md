...
| 구분 | 핵심 |
|---|---|
| 정의 | **AI 에너지 인프라**는 AI 데이터센터의 전력 공급·배전·냉각·계측을 뒷받침하는 설비와 운영체계다. |
| 목적 | AI 연산을 안정적으로 공급하면서 에너지·환경 부담을 관리한다. |
...
> 냉각 방식은 고정된 성능 순위가 아니라 랙 밀도, 서버 호환성, 입지·운영 조건을 기준으로 선택한다.
...
| 문제 | 해결 방안 |
|---|---|
| 증설 일정이 계통 접속, 냉각수·열배출 여건, 환경 검토보다 앞서 확정되면 가동 시점과 운영비를 예측하기 어렵다. | 부지 확정 전에 계통사업자 협의, 냉각·용수 조건, 환경 검토와 단계별 수요예측을 함께 평가하고, 확인된 공급용량에 맞춰 투자를 나눈다. |
