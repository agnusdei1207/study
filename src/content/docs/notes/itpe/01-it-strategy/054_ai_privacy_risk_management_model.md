...
| 구분 | 핵심 |
|---|---|
| 정의 | **AI 프라이버시 위험관리 모델**은 AI 수명주기에서 개인정보 처리와 관련된 위험을 식별·평가·대응하는 관리 절차다. |
| 목적 | 정보주체의 권리를 보호하고 적법한 데이터 활용을 뒷받침한다. |
...
| 문제 | 해결 방안 |
|---|---|
| 위험 경감 뒤 남은 위험을 누가 어떤 근거로 수용했는지 기록하지 않으면 시스템·데이터 변경 후에도 이전 판단이 유지될 수 있다. | 잔여위험별 수용 책임자·근거·보완통제·재검토일을 위험기록에 남기고, 데이터·모델·사용목적이 바뀌면 재평가한다. |

차등프라이버시나 언러닝은 위험을 낮출 수 있는 선택지이며 모든 처리에 적용해야 하는 법정 의무로 보지 않는다.
