...
지식 위치: IT 전략·관리 → 디지털 혁신·전략 → **디지털 트랜스포메이션(DX)**

## 30초 인출

- 본질: **DX(Digital Transformation)** 는 문서를 **전산화** ( **Digitization** )하거나 업무를 자동화( **Digitalization** )하는 데서 멈추지 않고 사업모델과 조직까지 바꾸는 경영 혁신이다.
- 메커니즘: 현업 문제 정의 → **MVP** 가치 검증 → 제품화 → 표준·플랫폼 재사용한다.
- 산출물: 신규 디지털 수익원 · 통합 고객 접점( **CX** ) · 데이터 기반 제품 조직이다.
