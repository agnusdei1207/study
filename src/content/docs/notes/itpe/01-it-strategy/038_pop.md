...
```mermaid
flowchart TD
    subgraph ShopFloor["1. 현장 수집 계층 (Edge & IoT)"]
        S1["설비 PLC / CNC 센서 신호"]
        S2["작업자 바코드 / RFID 스캔 실적"]
        S3["OPC UA 게이트웨이 표준 변환"]
    end
    subgraph Reliability["2. 데이터 무결성 및 버퍼링 (Edge Control)"]
        R1["타임스탬프 동기화 및 입력 범위 유효성 검증"]
        R2["네트워크 장애 대비 Store-and-Forward 로컬 큐"]
        R3["멱등성 키(Idempotency Key) 기반 중복 전송 방지"]
    end
    subgraph Upper["3. 상위 연계 및 폐쇄 루프 (Closed Loop)"]
        U1["MES 실시간 생산·품질·설비 종합효율(OEE) 환류"]
        U2["ERP 계획-실적 정산 및 원가 차이 분석"]
        U3["품질 이상 발생 시 라인 인터락(Interlock) 역제어"]
    end

    ShopFloor --> Reliability
    Reliability --> Upper
    Upper -.->|인터락 및 작업지시 수정| ShopFloor
```

- 판정: 현장 이기종 설비의 실적이 표준화되어 무결하게 수집되며, 네트워크 단절 시에도 데이터 유실 없이 상위 시스템으로 환류되는가
- 대안: OPC UA 표준 프로토콜 적용 · 엣지 컴퓨팅 기반 Store-and-Forward 버퍼링 · 멱등키 검증
- 검증: 타임스탬프 동기화(NTP/PTP) 검증 · 패킷 유실 모의시험 · MES-POP 실적 정합성 대조
- 효과: 수기 입력 오류 원천 차단 및 설비종합효율(OEE) 극대화, 제조 추적성(Traceability) 100% 확보
