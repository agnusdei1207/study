...
- 목적: 행정전산망 단일장애점(SPOF) 차단, 대국민 무중단 서비스 확보
...
| **적합한 상황** | 단기 데이터센터 계약 만료, 레거시 COTS 패키지 | 지속적 변경 요구사항, 대규모 변동 트래픽 공공 서비스 |
...
| 위험 | 대책 | 효과 |
|---|---|---|
| **데이터베이스 강결합 분할 난항** | 도메인 주도 설계(DDD) 기반 바운디드 컨텍스트 도출 및 CDC 기반 데이터 동기화 | 트랜잭션 정합성 100% 검증 및 점진 분리 달성 |
| **단일 턴키 발주 관행 충돌** | 인프라/플랫폼 구축과 업무 모듈 현대화를 분리하는 '기능 단위 분할 발주' 적용 | 플랫폼 전문 기업 참여 보장 및 대형 SI 종속 탈피 |
