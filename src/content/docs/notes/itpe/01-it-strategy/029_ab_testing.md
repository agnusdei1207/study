...
- 본질: A/B 테스트는 사용자를 무작위로 나눠 두 대안의 성과 차이를 비교하는 실험이다. 기능 변경이 실제 지표를 바꿨는지 확인한다.
...
- **A/B 테스트(A/B Test)** : 대조군과 실험군에 단일 변수를 무작위 노출하여 지표 차이의 유의성을 검증하는 무작위 통제 실험
- **Causality(인과관계)** : 외생 변수를 통제한 상태에서 특정 기능 변경이 사용자 반응 변화를 유발했음을 입증하는 인과관계
- **SRM(Sample Ratio Mismatch)** : 실험군과 대조군의 실제 표본 할당 비율이 목표 비율과 통계적으로 불일치하는 표본 편향 현상
- **Peeking(피킹)** : 정해진 표본 수 도달 전에 p-value를 반복 확인하고 조기 종료하여 1종 오류를 유발하는 분석 오류
- **Feature Flag** : 코드 재배포 없이 특정 사용자 세그먼트에 기능을 동적으로 활성화·비활성화하는 기능 제어 플래그
- **p-value** : 귀무가설(차이가 없다)이 참이라는 전제하에 관측된 결과 이상의 극단값이 나타날 조건부 확률
- **MDE(Minimum Detectable Effect)** : 통계적 검정력을 유지하면서 유의미하게 감지하고자 하는 최소 개선 효과 크기
- **Guardrail Metrics(보호 지표)** : 비즈니스 목표 지표 최적화 과정에서 응답 지연이나 에러율 등 시스템 안정성이 훼손되지 않도록 감시하는 보호 지표
