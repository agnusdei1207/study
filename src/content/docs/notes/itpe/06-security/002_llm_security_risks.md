...
- **본질:** 모델·데이터·도구·애플리케이션에서 발생해 기밀성·무결성·가용성에 영향을 주는 LLM 위협
- **메커니즘:** 공격 입력·오염 자료 → 응답·검색·도구 호출에 영향; 검색·실행 권한과 출력을 별도 통제
- **핵심 대응:** 최소권한·서버 측 권한검사·도구 승인·민감정보 보호·출력 검증의 중첩 적용

<details>
<summary>핵심 용어</summary>

- **프롬프트 주입:** 입력·외부 콘텐츠의 지시로 모델 응답이나 행동을 의도와 다르게 유도하는 공격
- **LLM 보안 위험:** LLM 기반 서비스의 모델·데이터·연결 도구·애플리케이션에서 발생하는 정보보호 위협
- **간접 프롬프트 주입:** 검색 문서 등의 지시가 모델 컨텍스트에 들어가 응답·연결 기능에 영향을 주는 공격
- **과도한 에이전시:** LLM 애플리케이션에 필요 이상의 도구·데이터 권한이나 실행 자율성을 부여한 상태
- **RAG (Retrieval-Augmented Generation):** 외부 자료를 검색해 모델 입력 컨텍스트에 반영하는 생성 구조
</details>

---
## 1교시 예상문제 (10점)

> LLM 보안 위험의 주요 유형과 발생 경로, 기본 대응 원칙을 설명하시오. *(예상문제)*
...
| 구분 | 핵심 |
|---|---|
| 정의 | **LLM 보안 위험**은 모델·데이터·연결 도구·애플리케이션에서 발생해 정보자산과 서비스에 영향을 주는 위협 |
| 목적 | 위험의 발생 경로와 영향을 파악해 적절한 보호 통제를 적용 |
...
**제언:** 프롬프트 입력·검색 데이터·도구 실행의 권한 분리와 서버 측 검증

---
## 2~4교시 예상문제 (25점)

> LLM 보안 위험의 유형과 발생 경로를 설명하고 데이터·도구 연동을 포함한 대응방안을 제시하시오. *(25점 예상문제)*
...
모델 응답은 입력·컨텍스트에 따라 달라지는 특성. 모델 내부 가드레일만으로 서버 측 권한통제를 대체할 수 없음.
...
OWASP의 2025 Top 10은 LLM·생성형 AI 애플리케이션의 위험 분류. 아래는 주요 발생 지점과 결과.
...
검색 문서는 모델 컨텍스트가 되지만 신뢰된 지시는 아님. 문서 권한은 검색 단계에서 확인하고 모델의 도구 요청은 애플리케이션에서 별도 검증.
...
보안 통제는 모델 선택·데이터 준비부터 배포·운영까지 적용. OWASP 분류는 참고이며 제품별 위협모델·통제 검증은 별도 수행.
