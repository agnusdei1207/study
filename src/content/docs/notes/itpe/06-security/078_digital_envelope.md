...
| 단계 | 처리 |
|---|---|
| 생성 | CEK 생성 → 본문 암호화 → 수신자 공개키 방식으로 CEK 설정 → 암호문과 키 정보를 전달 |
| 개봉 | 개인키로 전달된 키를 복구하거나 KEM 공유 비밀에서 CEK를 도출 → 본문 복호화 |
...
sequenceDiagram
  ...
  S->>S: CEK 생성
  S->>S: CEK로 본문 암호화
  S->>R: 암호문 + 수신자용 키 설정 정보 전달
  R->>R: 개인키 기반 키 복구 또는 KEM 공유 비밀에서 CEK 도출
  R->>R: CEK로 본문 복호화
```
