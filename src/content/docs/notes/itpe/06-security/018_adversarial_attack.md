...
- **본질:** 적대적 공격은 입력이나 학습데이터를 조작해 머신러닝 모델의 예측·동작을 바꾸려는 공격
- **메커니즘:** 추론 입력 섭동 또는 학습데이터·과정 조작을 통한 모델 반응 유도
- **대응:** 위협모델 기반 적대적 훈련·강건성 시험·데이터 검증·운영 감시

<details>
<summary>핵심 용어</summary>

- **적대적 공격:** 입력·학습데이터·학습과정을 조작해 모델의 예측이나 성능을 의도와 다르게 만드는 공격
- **적대적 예제:** 특정 모델의 예측을 바꾸도록 의도적으로 변형한 입력
- **섭동:** 원본 입력에 더하는 공격 변경량으로, 크기와 허용범위는 위협모델에 따름
- **FGSM (Fast Gradient Sign Method):** 입력 기울기 부호로 섭동 방향을 정하는 한 단계 적대적 공격
- **PGD (Projected Gradient Descent):** 반복적으로 입력을 갱신하고 허용 영역에 사영하는 적대적 공격
...
> 적대적 공격의 개념과 주요 유형, 방어방안을 설명하시오. *(10점 예상문제)*
...
| 구분 | 핵심 |
|---|---|
| 정의 | **적대적 공격**은 입력이나 학습데이터를 의도적으로 조작해 머신러닝 모델의 예측·동작을 바꾸려는 공격 |
| 목적 | 오분류 유도 또는 모델·서비스의 신뢰성과 무결성 저하 |
...
방어 검증
위협모델 설정 → 공격 시험·데이터 검증 → 통제 보완
```

**제언:** 정확도만 보지 않고 실제 공격 가정과 입력조건에서 강건성을 시험한 뒤 배포
...
적대적 예제는 정의된 입력공간·섭동 제약에서 모델 반응을 바꾸도록 구성한 입력. 사람에게 반드시 보이지 않는다는 의미는 아님.
...
공격자는 정의된 손실함수와 입력 제약을 이용해 모델 출력이 바뀌도록 입력을 수정. FGSM은 손실의 입력 기울기 부호를 사용하는 한 단계 방법, PGD는 입력을 반복 갱신하고 허용 영역에 사영하는 방식. C&W는 별도의 최적화 공격 계열.
...
공격 분류축은 모델 지식·접근 권한·목표·생애주기 단계 등. NIST AI 100-2e2025도 시스템 유형·ML 생애주기·공격자 목표·능력·지식에 따른 분류를 제시.
...
방어는 지정된 위협모델에 따라 설계하고 공격 강도·허용 섭동·환경을 기록한 재현 가능한 시험으로 평가. 입력 전처리는 보조 수단이며 단독 방어로 일반화하지 않음.
