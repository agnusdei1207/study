...
| 관측 패턴 | 초기부터 발견되며 증가 속도가 점차 감소 | 초기 발견이 느리다가 증가한 뒤 포화 |
| 모델 선택 | 관측 자료가 오목한 누적 곡선에 가까운 경우 | 관측 자료가 지연 S자형에 가까운 경우 |
| 모수 해석 | $a$: 모형상 총 발견 가능 고장 수, $b$: 검출 속도 모수 | $a$: 모형상 총 발견 가능 고장 수, $b$: 검출 속도 모수 |
...
| **초기 지연의 표현** | 발견 강도가 초기부터 감소하는 형태이며 학습 부재를 전제하지 않음 | 발견 강도가 초기에는 증가하는 지연 형태를 표현 |
| **선택 기준** | 관측 누적 고장 수가 오목형 곡선에 가까운 경우 | 관측 누적 고장 수가 지연 S자형 곡선에 가까운 경우 |
...
| **추정치 계산** | 모형상 잔여 발견 건수 $a-m(t)$와 고장 발생 강도 $\lambda(t)$ 추정 |
| **판단 활용** | 자료·모델의 불확실성을 기록하고 다른 품질·위험 기준과 함께 검토 |
...
**제언:** 추정치 하나로 출시를 판정하지 않고 시험 범위·운영 위험과 함께 검토.
