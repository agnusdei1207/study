...
- 정의: 최단 경로 알고리즘은 가중치 그래프에서 정점 사이의 최소 비용 경로를 계산하는 알고리즘군이다.
- 목적: 경로를 모두 열거하지 않고, 가중치 조건과 탐색 범위에 맞는 알고리즘으로 효율적으로 경로를 찾는다.
...
- 제언: 음수 간선 유무와 단일·전체 정점 쌍 여부를 확인한 뒤 적합한 알고리즘을 선택한다.
---
