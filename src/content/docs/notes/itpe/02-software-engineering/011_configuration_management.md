...
| 구분 | 핵심 |
|---|---|
| 정의 | 형상항목(CI)을 식별하고 변경·상태·감사를 통해 기준선과 산출물의 일치 여부 관리 |
| 목적 | 어떤 버전에서 무엇이 왜 바뀌었는지 추적하고 승인되지 않은 변경을 방지 |
...
| 문제 | 제언 |
|---|---|
| 장애·감사 시 운영 산출물이 어떤 승인 변경에서 만들어졌는지 즉시 확인하기 어려움 | 제언: 변경요청부터 커밋, 빌드 산출물, 배포 버전까지 식별자를 연결해 릴리스 기준선으로 보존하고, 운영 인스턴스에서 해당 기록을 역추적할 수 있게 한다. |
---
