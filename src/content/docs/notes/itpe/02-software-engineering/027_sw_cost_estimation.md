...
- 정의: **SW 규모·비용 산정**은 구현 범위를 규모로 추정하고 공수·일정·비용에 연결하는 작업이다.
- 목적: 예산과 자원을 계획하고 과업 변경의 영향을 비교한다.
...
- **FP 구성**: 데이터 기능 ILF·EIF, 트랜잭션 기능 EI·EO·EQ를 구분한다.

제언: 초기 산정 가정을 명시하고 기능 범위가 구체화되면 추정치를 갱신한다.
...
| 구분 | 핵심 |
|---|---|
| 정의 | 요구사항에 맞는 규모 기준(LOC·FP 등)을 정해 개발 공수와 비용 추정 |
| 목적 | 예산·일정을 계획하고 과업이 바뀌면 추정 근거도 다시 검토 |
...
| 문제 | 해결 방안 |
|---|---|
| 요구사항 성숙도와 무관하게 한 가지 산정 기법만 적용함 | 제언: 초기에는 유사 과업·전문가 판단과 가정을 이용해 범위를 제시하고, 기능 요구가 안정되면 정해진 규모 산정 기준과 생산성 자료로 상세 산정한다. |
---
