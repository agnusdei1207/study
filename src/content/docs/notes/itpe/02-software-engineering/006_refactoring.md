...
> **리팩토링**은 외부 동작을 유지하면서 내부 코드 구조를 개선하는 작업.
...
> **코드스멜**은 설계 문제의 징후. 실제 변경 비용과 위험을 확인해 개선 우선순위 결정.
...
> 변경을 작게 나누고 매 단계에서 외부 동작 유지 여부 확인.
...
> 변경 전 동작을 확인한 뒤 작은 단위로 구조를 바꾸고 회귀 테스트로 동작 보존 확인.
