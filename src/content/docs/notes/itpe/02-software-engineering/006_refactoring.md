...
| 구분 | 핵심 |
|---|---|
| 정의 | **외부 동작은 그대로** 두고 코드를 이해·수정하기 쉽게 재구성 |
| 목적 | 코드스멜을 줄여 변경 비용과 기술 부채를 관리 |
...
| 문제 | 제언 |
|---|---|
| 테스트 없이 구조를 바꾸면 회귀 결함을 놓칠 수 있음 | 기존 동작을 재현하는 테스트를 먼저 확보하고 변경 후 실행 |
| 기능 추가와 구조 개선이 섞이면 결함 원인 파악이 어려움 | 작업과 커밋을 구분하고 CI에서 회귀 테스트 결과 확인 |
---
