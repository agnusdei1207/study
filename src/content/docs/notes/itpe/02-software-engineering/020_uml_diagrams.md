...
| 구분 | 핵심 |
|---|---|
| 정의 | 클래스·상호작용·활동 등 관점별 표기법으로 시스템을 모델링 |
| 목적 | 요구사항·설계 의도를 관계자와 공유하고 서로 다른 모델의 일관성을 확인 |
...
| 문제 | 해결 방안 |
|---|---|
| 여러 이해관계자가 서로 다른 설계 질문을 같은 모델에서 찾으려 함 | 제언: 이해관계자별로 확인할 질문을 먼저 적고, 구조·상호작용·행동 중 답하는 관점에 맞춰 UML 뷰를 선택해 유지 책임자와 갱신 시점을 정한다. |
---
