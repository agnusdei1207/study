...
> 순차 다이어그램(Sequence Diagram)의 정의와 목적, 핵심 메커니즘을 설명하시오. (예상)
...
- 정의: 시스템 컴포넌트나 객체 간의 메서드 호출 순서와 생명주기를 파악하기 위해, 수평축에 참여 객체를 배치하고 수직축(시간 경과)을 따라 동기·비동기 메시지와 조건부 제어 흐름을 가시화하는 UML 동적 모델링 기법이다.
- 목적: 시간 순서에 따른 객체·서비스 간 메시지 교환을 표현해 동작과 인터페이스를 명확히 한다.
...
- 제언: 생명선과 메시지 순서를 먼저 그리고 대안·반복 분기를 표시해 API 상호작용을 검토한다.
---

## 2~4교시 예상문제 (25점)

> 순차 다이어그램(Sequence Diagram)의 개념과 목적을 설명하고, 핵심 메커니즘과 구성요소·절차, 적용 시 문제점과 대응 방안을 제시하시오. (25점, 예상)
