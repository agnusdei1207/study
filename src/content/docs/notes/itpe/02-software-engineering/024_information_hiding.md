...
- 정의: **정보은닉(Information Hiding)**은 변경될 가능성이 큰 설계 결정을 한 모듈 안에 감추는 분할 원리다.
- 목적: 내부 결정이 바뀌어도 다른 모듈의 수정 범위를 줄인다.
...
flowchart TD
    C["외부 모듈"] --> I["공개 인터페이스"]
    I --> S["내부 설계 결정: 자료구조·알고리즘"]
```

### 3. 핵심 통제

- **Parnas 분할**: 기능 순서가 아닌 '숨겨야 할 비밀'을 기준으로 모듈 경계 획정
- **경계 확인**: 외부가 내부 자료 표현에 직접 의존하는 곳을 찾아 공개 계약으로 옮김

제언: 변경될 설계 결정을 한 모듈이 소유하도록 하고 공개 인터페이스의 안정성을 변경 사례로 확인한다.
---
...
| 구분 | 핵심 |
|---|---|
| 정의 | 자료 표현·알고리즘 등 변경 가능한 결정을 한 모듈이 감추고 외부에는 안정된 인터페이스 제공 |
| 목적 | 내부 결정이 바뀌어도 다른 모듈의 수정 범위를 줄임 |
...
| 문제 | 해결 방안 |
|---|---|
| 내부 자료 표현을 여러 모듈이 직접 참조함 | 해당 결정을 소유할 모듈을 정하고 접근을 공개 인터페이스로 모음 |
| 인터페이스가 내부 구현을 그대로 드러냄 | 실제 변경 사례로 공개 계약의 안정성을 검토하고 누출된 구현 세부사항을 줄임 |
---
