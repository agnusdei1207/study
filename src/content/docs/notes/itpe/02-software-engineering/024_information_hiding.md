...
> 정보 은닉은 설계 원리, 캡슐화는 그 원리를 구현하는 수단이라는 구분.
...
| **상호 관계** | 캡슐화의 목적 중 하나로 구현 가능 | 데이터와 행위를 묶고 접근을 제한하여 정보은닉에 기여 |

## Ⅳ. 정보은닉 적용 한계·대응책

> 모듈 간 의존성과 실제 변경 사례를 통한 변경 국소화 여부 확인.
...
> 바뀔 가능성이 큰 설계 결정을 모듈 내부에 두고 외부에는 안정적인 계약만 공개.
...
- 연관 토픽: [객체지향 프로그래밍의 캡슐화](./083_oop.md), [모듈성(결합도·응집도)](./190_modularity.md)
