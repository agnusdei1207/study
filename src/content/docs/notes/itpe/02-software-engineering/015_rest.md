...
| 구분 | 핵심 |
|---|---|
| 정의 | **REST(Representational State Transfer)** 는 자원·표현과 제약조건으로 분산 시스템을 구성하는 아키텍처 스타일 |
| 목적 | 구성요소 독립성과 캐시·계층을 통한 확장성 지원 |
...
> REST의 개념과 핵심 제약조건을 설명하고, 자원·표현·HTTP 메서드의 관계 및 웹 API 설계에서의 적용 기준을 제시하시오. (25점, 예상)
...
> REST의 다섯 필수 제약과 선택적 Code on Demand 제약 구분.
...
> Richardson 성숙도 모델의 0~3단계는 API 설계 수준을 비교하는 참고 틀.
...
> API 계약에 자원·메서드·응답 의미를 명시하고 중복 요청·오류 응답의 일관성 검증.

| 한계 | 해결 방안 |
|---|---|
| 여러 API가 각자 계약을 정해 소비자 변경과 호환성 검증이 반복됨 | 자원·요청·응답·오류 형식의 계약 검토, 소비자 계약시험 및 호환성 확인을 배포 기준으로 적용 |
---
