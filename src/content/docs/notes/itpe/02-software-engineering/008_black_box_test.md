...
> **블랙박스 테스트**는 내부 코드 대신 요구사항과 입출력을 기준으로 동작을 확인하는 기법.
...
> 동등 분할은 입력 구간의 대표값, 경계값 분석은 구간 경계의 오류를 확인.
...
> 동등 분할과 경계값 분석은 같은 입력 공간을 서로 다른 위치에서 점검하는 보완적 기법.
...
> 복합 조건·상태 변화에 대해서는 결정표·상태 전이 등 다른 설계 기법과 조합 범위 통제.
...
> 입력 범위·조건을 정한 뒤 동등 분할·경계값·결정표를 결함 위험에 따라 조합.
...
## 출제 이력과 검증 출처
- [ISTQB CTFL v4.0.1](https://istqb.org/wp-content/uploads/2024/11/ISTQB_CTFL_Syllabus_v4.0.1.pdf): 명세 기반 블랙박스 기법과 경계값 분석

- 제134회 정보관리기술사 3교시: 소프트웨어 테스트의 명세·구조·경험 기반 기법
- 제137회 정보관리기술사 2교시: 동등 분할과 분류 트리 기법의 테스트 케이스 작성
- 제139회 정보관리기술사 3교시: 동등 분할·경계값 분석을 적용한 테스트 케이스 작성
