...
- **개념**: 로버트 C. 마틴(Uncle Bob)이 정립한 유지보수성, 확장성, 테스트 용이성을 극대화하기 위한 객체지향 소프트웨어 설계의 5대 핵심 기본 원칙.
- **배경**: 소프트웨어 변경 요구사항 발생 시 기존 검증 코드를 수정하지 않고 안전하게 기능을 확장(OCP)하며, 높은 응집도(High Cohesion)와 낮은 결합도(Low Coupling)를 아키텍처 수준에서 보장하기 위해 체계화됨.
