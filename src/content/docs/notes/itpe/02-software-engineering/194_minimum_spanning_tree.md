...
> 최소신장트리(Minimum Spanning Tree)의 정의와 목적, 핵심 메커니즘을 설명하시오. (예상)
...
- 정의: 무향 연결 가중치 그래프에서 모든 정점($V$)을 사이클 없이 연결하는 여러 신장 트리(Spanning Tree) 중에서, 간선들의 총 가중치 합이 최소가 되도록 정확히 $V-1$개의 간선을 탐욕적(Greedy)으로 선택하는 최적화 그래프 알고리즘
- 목적: 연결 그래프의 모든 정점을 최소 총 가중치로 연결하는 트리를 구한다.
...
- 제언: 그래프 크기와 표현 방식에 따라 Kruskal 또는 Prim을 선택하고 간선 수와 연결성을 확인한다.
---

## 2~4교시 예상문제 (25점)

> 최소신장트리(Minimum Spanning Tree)의 개념과 목적을 설명하고, 핵심 메커니즘과 구성요소·절차, 적용 시 문제점과 대응 방안을 제시하시오. (25점, 예상)
