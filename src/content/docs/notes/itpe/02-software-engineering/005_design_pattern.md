...
| 구분 | 핵심 |
|---|---|
| 정의 | **프록시 패턴(Proxy Pattern)** 은 실제 객체와 같은 인터페이스의 대리 객체가 요청을 받아 대상 객체로 위임하는 구조 패턴 |
...
- 한 줄 제언: 프록시 도입 시 실제 객체 호출 전후의 정책과 위임 여부를 시험으로 확인
---

## 2~4교시 예상문제 (25점)

> 프록시 패턴의 구조와 역할을 설명하고, 접근 제어·지연 생성·원격 호출 등 적용 목적과 설계 시 고려사항을 제시하시오. (예상)

---

## 2~4교시 25점 답안

## Ⅰ. 객체 접근을 대리하는 프록시 패턴의 개요
...
| 위험 | 대책 | 효과 |
|---|---|---|
| **Self-Invocation (내부 메서드 호출 시 프록시 우회)** | 자기 호출 메서드를 별도 서비스 컴포넌트로 분리하거나 호출 경로에 맞는 위빙 방식 검토 | 프록시를 통하지 않는 호출에서 트랜잭션·보안 AOP 적용 여부 확인 |
| **CGLIB의 final 제약 및 생성자 제약** | `final` 키워드 지양 규칙 및 Objenesis 라이브러리 연계 프록시 생성 | 런타임 프록시 생성 실패 방지 및 호환성 확보 |
| **지연 로딩 시점의 세션 종료 (LazyInitializationException)** | 트랜잭션 경계와 필요한 조회 범위를 정하고 fetch join 등 적절한 조회 전략 검토 | 세션 종료 후 접근 오류와 불필요한 추가 조회 위험 완화 |

### Ⅴ. 결론 — 투명한 접근 통제 중심의 기술사적 제언

| 한계 | 해결 방안 |
|---|---|
| 공통 관심사가 내부 호출까지 적용되지 않을 수 있음 | 외부 호출에는 프록시 기반 AOP, 내부 호출에는 서비스 경계 분리나 위빙 방식 검토 후 실제 호출 경로 시험 |
---
