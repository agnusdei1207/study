...
| 문제 | 해결 방안 |
|---|---|
| 공통 관심사를 모든 호출 방식에 같은 AOP 방식으로 적용하면 필요한 호출이 가로채이지 않을 수 있음 | 제언: 프록시 외부 호출에 한정되는 관심사는 프록시 기반 AOP를 쓰고, 내부 호출까지 가로채야 하는 경우는 서비스 경계 분리나 위빙 방식을 요구조건에 따라 선택해 실제 호출 경로로 검증한다. |
---
