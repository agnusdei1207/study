...
> **프록시 패턴**은 대리 객체가 실제 객체로 가는 접근을 제어하거나 부가 기능을 제공하는 구조.
...
> 클라이언트가 공통 인터페이스를 통해 프록시를 호출하고 프록시가 실제 객체에 요청을 전달하는 구조.
...
> 비슷한 래핑 구조라도 접근 제어·기능 확장 등 설계 의도에 따라 패턴을 선택.
...
> 프레임워크 기반 프록시 적용 시 내부 호출의 인터셉트 누락 등 구현 제약 확인.
