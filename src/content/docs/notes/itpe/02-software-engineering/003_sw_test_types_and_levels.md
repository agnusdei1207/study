...
> 테스트 유형은 확인할 품질 특성, 테스트 레벨은 검증 대상의 범위를 구분하는 두 축.
...
> 단위·통합·시스템·인수로 범위를 넓히며 각 레벨의 검증 대상과 기준을 구분.
...
> 기능 요구뿐 아니라 신뢰성·회복성·이식성 등 비기능 요구의 조건별 검증.
...
> 시스템 수준 시험의 환경·시나리오 유지 비용을 고려한 위험 기반 자동화 범위 선정.

| 위험 | 대책 | 효과 |
|---|---|---|
| **수작업 E2E 테스트 병목** | 변경 위험에 맞춰 단위·통합·E2E 테스트를 배분하고 반복 가치가 높은 검증을 자동화 | 빠른 피드백 확보 |
| **테스트-운영 환경 불일치** | Docker 컨테이너 및 **IaC** 기반 테스트 환경 표준화 | 환경 차이로 인한 배포 후 결함 사전 차단 |
| **코드 변경 시 회귀 결함 누출** | 반복 실행 가치가 높은 스모크·회귀 테스트를 CI에 연계하고 실패 시 원인 확인 | 기존 기능 변경의 회귀 위험 확인 |
...
- 제134회 정보관리기술사 3교시: 소프트웨어 테스트 원리, 블랙박스·화이트박스 테스트, 명세·구조·경험 기반 기법
- 제139회 정보관리기술사 3교시: 소프트웨어 테스트의 7대 원리, 화이트박스·블랙박스 비교와 테스트 케이스 작성
