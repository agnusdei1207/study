...
- **개념**: 소스코드에 의도적으로 결함(Mutant, 돌연변이)을 주입한 뒤 기존 테스트 케이스를 실행하여 결함을 감지·살해(Kill)하는지 검증함으로써 테스트 케이스 자체의 유효성과 결함 검출력을 평가하는 결함 기반(Fault-based) 테스팅 기법.
- **배경**: 코드 커버리지(구문, 분기)가 100%라 할지라도 단언문(Assertion)이 부실하면 결함을 잡지 못하는 커버리지의 맹점을 극복하고 테스트 스위트의 질적 무결성을 정량화하기 위해 도입됨.
