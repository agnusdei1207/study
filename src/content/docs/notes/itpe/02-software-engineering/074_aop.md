...
| **Joinpoint (결합점)** | Advice가 적용될 수 있는 실행 지점. Spring AOP는 빈의 메서드 실행을 대상으로 함 | AspectJ: 메서드·생성자·필드 지점 등; Spring AOP: 빈 메서드 실행 |
...
| **적용 가능 범위** | 메서드, 생성자, 필드 등 | 메서드, 생성자, 필드 등 | 스프링 빈의 메서드 실행; 프록시 유형·가시성·내부 호출에 제약 |
| **실행 성능** | 위빙된 코드 실행 비용은 구현·어드바이스에 따름 | 위빙된 코드 실행 비용은 구현·어드바이스에 따름 | 호출 시 프록시 체인 비용이 추가될 수 있음 |

#### 2. Spring AOP의 프록시 메커니즘 (JDK Dynamic vs CGLIB)
- **JDK Dynamic Proxy**: 인터페이스 기반 프록시를 선택한 경우 `java.lang.reflect.Proxy`로 해당 인터페이스의 프록시를 생성.
- **클래스 기반 프록시(CGLIB)**: 구체 클래스를 상속하는 프록시를 생성. 인터페이스가 없어도 사용할 수 있으며, 인터페이스가 있더라도 설정에 따라 선택할 수 있음.
---
