...
> 반응형 웹(Responsive Web)의 정의와 목적, 핵심 메커니즘을 설명하시오. (예상)
...
- 정의: 스마트폰·태블릿·데스크톱 등 다양한 기기 해상도마다 별도의 웹사이트(m.site.com)를 개별 구축·유지하는 비용을 제거하기 위해, 단일 HTML 소스에서 화면 크기에 맞춰 UI 레이아웃과 이미지가 유연하게 재배치(Reflow)되도록 하는 프론트엔드 웹 디자인 기법이다.
- 목적: 하나의 웹 콘텐츠를 다양한 화면 크기에 맞춰 읽기 쉽고 사용하기 좋게 제공한다.
...
- 제언: 모바일 우선 레이아웃과 적절한 중단점을 적용하고 실제 뷰포트별 표시·접근성을 시험한다.
---

## 2~4교시 예상문제 (25점)

> 반응형 웹(Responsive Web)의 개념과 목적을 설명하고, 핵심 메커니즘과 구성요소·절차, 적용 시 문제점과 대응 방안을 제시하시오. (25점, 예상)
