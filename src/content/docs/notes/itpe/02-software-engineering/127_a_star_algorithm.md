...
- 정의: 실제 누적 비용 $g(n)$과 목표까지의 추정 비용 $h(n)$을 더한 $f(n)=g(n)+h(n)$으로 다음 탐색 노드를 고르는 최단 경로 탐색 알고리즘
- 목적: 다익스트라처럼 주변을 전방위로 탐색하는 비용을 줄이면서 최적 경로를 찾는다.
...
- 제언: 최적 경로가 필요하면 허용 가능한 휴리스틱을 쓰고, 탐색 비용과 경로 최적성을 함께 확인한다.
---
