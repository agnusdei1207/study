...
> 그리디 알고리즘(Greedy Algorithm)의 정의와 목적, 핵심 메커니즘을 설명하시오. (예상)
...
- 정의: 전체 경우의 수를 전수 검토하거나 이전 선택을 되돌아보지 않고, 매 의사결정 단계마다 당장 눈앞에 보이는 가장 최적의 선택(지역 최적해)을 직진하듯 결정해 나감으로써, '탐욕적 선택 속성'과 '최적 부분 구조'를 만족하는 문제에 대해 다항 시간 내에 전역 최적해(Global Optimum)를 도출하는 고속 알고리즘 설계 패러다임
- 목적: 매 단계의 최선 선택을 누적해 해를 구성하는 최적화 알고리즘 설계 방식이다.
...
- 제언: 탐욕 선택 성질과 최적 부분 구조를 증명한 문제에 적용하고 반례로 정확성을 검증한다.
---

## 2~4교시 예상문제 (25점)

> 그리디 알고리즘(Greedy Algorithm)의 개념과 목적을 설명하고, 핵심 메커니즘과 구성요소·절차, 적용 시 문제점과 대응 방안을 제시하시오. (25점, 예상)
