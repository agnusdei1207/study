...
> OpenTelemetry(관측성 표준)의 정의와 목적, 핵심 메커니즘을 설명하시오. (예상)
...
- 정의: 특정 모니터링 상용 벤더(APM)에 코드가 종속되는 것을 방지하고, 분산 시스템의 3대 텔레메트리(Traces, Metrics, Logs)를 단일한 표준 방식으로 계측·수집·가공·전송하는 CNCF 오픈소스 관측성 표준이다.
- 목적: 다양한 관측 도구에서 분산 추적·지표·로그를 일관되게 수집하고 벤더 종속을 줄인다.
...
- 제언: OTLP와 Collector 파이프라인을 표준화하고 서비스 간 Trace Context 전파와 데이터 품질을 점검한다.
---

## 2~4교시 예상문제 (25점)

> OpenTelemetry(관측성 표준)의 개념과 목적을 설명하고, 핵심 메커니즘과 구성요소·절차, 적용 시 문제점과 대응 방안을 제시하시오. (25점, 예상)
