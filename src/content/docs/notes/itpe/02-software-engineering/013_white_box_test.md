...
| 구분 | 핵심 |
|---|---|
| 정의 | 소스코드의 제어 흐름·데이터 흐름을 보고 테스트 입력과 기대 결과 설계 |
| 목적 | 실행되지 않은 구문·분기·조건을 찾아 테스트 공백을 줄임 |
...
| 문제 | 해결 방안 |
|---|---|
| 안전·보안·금전 영향이 큰 코드에 시험 자원을 우선 배치하기 어려움 | 제언: 영향이 큰 요구사항을 먼저 골라 명세 기반 기대 결과와 구조 기반 경로 증거를 함께 추적하고, 커버리지 수치만으로 결함 부재를 판단하지 않는다. |
---
