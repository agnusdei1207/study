...
> 간선 E, 노드 N, 연결 성분 P로 계산하는 복잡도. 단일 흐름 그래프에서는 이진 분기 수 D와 영역 수 R로도 확인 가능.

| 계산 방식 | 공식 | 비고 |
|---|---|---|
| 간선·노드 기반 | `V(G) = E - N + 2P` | P는 연결 성분 수. 단일 모듈은 P=1이므로 `V(G) = E - N + 2` |
...
> 선형 독립 경로를 골라 기본 경로 테스트를 설계할 때 활용하는 기준.
