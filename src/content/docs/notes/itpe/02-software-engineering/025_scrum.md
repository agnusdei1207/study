...
- **스크럼(Scrum)**: 복잡한 적응형 문제를 해결하고 가치를 창출하도록 돕는 경량 프레임워크
- **Product Owner (PO)**: 제품 가치를 극대화하고 제품 백로그의 순서를 정하는 책무
...
| 구분 | 핵심 |
|---|---|
| 정의 | **스크럼(Scrum)** 은 팀이 짧은 스프린트마다 증분을 만들고 점검·적응하는 경험주의 기반 프레임워크 |
| 목적 | 불확실한 과업의 진행 상태를 드러내고 제품 가치를 점진적으로 높임 |

### Ⅱ. 스크럼 구성

| 구성 | 핵심 |
|---|---|
| 책무 | Product Owner·Scrum Master·Developers |
| 이벤트 | 스프린트 안에서 계획·일일 점검·리뷰·회고 |
| 산출물 | 제품 백로그·스프린트 백로그·증분과 각 산출물의 약속 |

**핵심 통제**

- **완료의 정의(DoD)**: 증분이 충족해야 할 품질 상태에 대한 공통 기준
- **점검·적응**: 증분과 목표에 대한 검토 결과의 다음 작업 계획 반영

- 한 줄 제언: 리뷰에서 제품 목표와 증분의 차이를 확인해 다음 제품 백로그에 반영
---

## 2~4교시 예상문제 (25점)

> 스크럼의 경험주의 원리와 3대 책무·5개 이벤트·3개 산출물의 관계를 설명하고, 완료의 정의(DoD)와 스프린트 목표를 운영에 적용하는 방안을 제시하시오. (25점, 예상)

---

## 2~4교시 25점 답안

## Ⅰ. 불확실성을 다루는 경험주의 기반 프레임워크, 스크럼의 개요
...
> 스크럼은 3가지 책무, 5가지 이벤트, 3가지 산출물과 각 산출물의 약속으로 구성되는 틀
...
| **제품 증분 (Increment)** | **완료의 정의 (Definition of Done)** | 완료의 정의를 충족해 사용 가능한 상태의 제품 증분 |
...
> **완료의 정의(DoD)**는 증분이 충족해야 할 품질 기준에 관한 스크럼 팀의 공통 약속.
...
## Ⅴ. 스프린트 목표를 기준으로 한 계획 조정 제언

> 이벤트별 목적에 따라 증분·진행 상태를 점검하고 스프린트 목표에 맞게 계획 조정.

| 한계 | 해결 방안 |
|---|---|
| 스프린트 중 새 정보가 생길 때 목표와 작업 범위 중 무엇을 우선할지 불명확함 | Developers와 Product Owner의 스프린트 목표 기준 범위 조정, 목표가 무효화된 경우 Product Owner의 취소 판단 |
---
