...
> 다익스트라 알고리즘(Dijkstra Algorithm)의 정의와 목적, 핵심 메커니즘을 설명하시오. (예상)
---
...
- 정의: 간선의 가중치가 음수가 아닌 유향/무향 그래프에서 단일 시작점으로부터 도달 가능한 다른 모든 정점까지의 최단 경로를 구하기 위해, 미방문 정점 중 시작점과의 거리가 가장 짧은 노드를 탐욕적(Greedy)으로 확정하고 간선 완화(Edge Relaxation)를 반복 수행하여 $O((V+E)\log V)$ 시간에 최단 거리를 산출하는 그래프 알고리즘
- 목적: 인터넷 라우터 간에 링크 상태(Link-State) 정보를 교환한 후, 각 라우터가 목적지 네트워크까지의 최단 패킷 전달 경로를 계산하기 위해 다익스트라 알고리즘을 사용하는 대표적 표준 라우팅 프로토콜
...
> 다익스트라 알고리즘(Dijkstra Algorithm)의 개념과 목적을 설명하고, 핵심 메커니즘과 구성요소·절차, 적용 시 문제점과 대응 방안을 제시하시오. (25점, 예상)
---
