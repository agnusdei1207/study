...
- 정의: 대규모 소프트웨어의 복잡성을 통제하고 분할 정복(Divide & Conquer)을 실현하기 위해, 소프트웨어를 자율적이고 독립적인 모듈 단위로 분해하고, 모듈 간 상호 의존성을 최소화(저결합도: Loose Coupling)하며 모듈 내부의 책임 집중도를 극대화(고응집도: High Cohesion)하여 변경 파급효과를 완벽히 격리하는 핵심 소프트웨어 공학 설계 원리
- 목적: 대규모 소프트웨어의 복잡성을 통제하고 분할 정복(Divide & Conquer)을 실현하기 위해, 소프트웨어를 자율적이고 독립적인 모듈 단위로 분해하고, 모듈 간 상호 의존성을 최소화(저결합도: Loose Coupling)하며 모듈 내부의 책임 집중도를 극대화(고응집도: High Cohesion)하여 변경 파급효과를 완벽히 격리하는 핵심 소프트웨어 공학 설계 원리
