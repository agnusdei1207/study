...
> **ATAM**은 품질속성 시나리오를 기준으로 아키텍처 결정의 위험과 절충점을 분석하는 평가 방법.
...
> **유틸리티 트리**는 품질 요구를 우선순위가 있는 평가 시나리오로 구체화하는 도구.
...
| **절충점 (Tradeoff Point)** | **복수 품질속성**에 걸쳐 한쪽을 높이면 다른 쪽에 영향을 주는 아키텍처 결정 | 암호화 방식 변경 시 보안성과 처리 지연의 함께 측정 |
