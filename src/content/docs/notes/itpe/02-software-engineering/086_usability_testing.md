...
- **개념**: 특정 맥락(Context of Use)에서 특정 사용자가 명시된 목표를 효과적(Effectiveness), 효율적(Efficiency), 만족스럽게(Satisfaction) 달성할 수 있는지를 정량적·정성적으로 측정하고 개선점을 도출하는 검증 활동 (ISO 9241-11).
- **배경**: 기능 중심의 개발에서 사용자 경험(UX) 중심으로 전환됨에 따라, 복잡한 인터페이스로 인한 조작 오류와 이탈을 방지하고 개발 후반부의 재작업 비용을 최소화하기 위해 필수 공정으로 정착됨.
