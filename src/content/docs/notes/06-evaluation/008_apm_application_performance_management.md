...
```text
[요청자] (클라이언트 트랜잭션 인입)
 └── [서비스 계층] (① Trace ID 발급 및 traceparent 전파)
      └── [계측 Agent] (② 구간별 Span 실행시간 계측)
           └── [OTel Collector] (③ 지연·오류 트랜잭션 선별 샘플링)
                └── [분석 저장소] (④ 메트릭·로그 연계 상관 분석)
```

- 분기 결과: 정상 트레이스는 설정 비율로 희소 수집되고 지연·오류 트랜잭션은 100% 선별 적재되어 상세 원인이 규명된다.
...
- **기술 위상/발전**: 단편적 인프라 감시를 넘어 분산 서비스 전 구간의 호출 맥락을 가시화하는 **엔드투엔드 관측성(Observability) 및 OpenTelemetry 표준 플랫폼**으로 안착, 향후 eBPF 커널 계측 및 AIOps 기반 자동 장애 원인 분석 결합 전망
- **실무 적용/통제**: 엔터프라이즈 도입 시 **벤더 중립적 OTel Collector 파이프라인 구축**, 비동기 큐 전 구간 `traceparent` 헤더 전파 및 **꼬리 기반 샘플링(Tail-based Sampling)을 통한 저장 비용·분석 효율 최적화** 필수
