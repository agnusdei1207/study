...
```text
[의존성 분석] (종단 의존성 맵 작성 및 잠재 SPOF 탐색)
 └── [장애 도메인 분리] (① 전원·회선·3개 가용영역 Multi-AZ 물리 격리)
      └── [계층별 이중화] (② VRRP L4, Redis 클러스터, DB 3중화 및 N+1 확보)
           └── [외부 연계 격리] (③ Resilience4j 서킷 브레이커 및 Fallback 적용)
                └── [카오스 실증] (④ Chaos Mesh 피크 부하 중 AZ 단절 주입)
                     └── [상용 운영 승인] (⑤ 에러율 0.001% 미만 검증 및 무중단 인가)
```

- 분기 결과: 카오스 주입 시 무단절 수용 확인되면 프로덕션 배포, 결함 전파 식별 시 해당 계층 아키텍처 재설계.
...
| 문제 | 대책 | 효과 |
|:---|:---|:---|
| 사내 공통 DNS 서버 다운으로 **전체 시스템 전면 마비** | 종단 의존성 맵 작성 및 **CoreDNS Multi-AZ Anycast 이중화** | **숨은 인프라 의존성** 선제 제거 |
| 단일 PDU 전원 단락으로 **이중화 서버 동시 다운(CCF)** | **Dual Power Supply를 독립 A/B PDU** 및 LACP 연결 | **전원·스위치 공통 고장** 방지 |
| 외부 결제 API 지연으로 **내부 WAS 스레드 200개 전부 고갈** | **Resilience4j 서킷 브레이커** 및 1초 타임아웃 Fallback | **외부 장애의 내부 전파** 차단 |
...
- **기술 위상/발전**: 종단 의존성을 가시화하고 단일 결함점을 전수 격리하여 서비스 연속성을 확보하는 **내결함성(Fault Tolerance) 공학 표준**으로 정착, 카오스 엔지니어링 및 클라우드 복원력과 연계 발전
- **실무 적용/통제**: 아키텍처 설계 시 **전 구간 종단 의존성 맵 상시 최신화**, 전원(Dual PDU) 및 스위치를 분리하는 **물리적 장애 도메인 격리**, **외부 연계 구간 서킷 브레이커 적용** 및 **카오스 장애 주입 실증** 필수
