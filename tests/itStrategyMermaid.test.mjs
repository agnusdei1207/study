...
async function targetNotes() {
  ...
  return names.filter((name) => /^(?:00[1-9]|0[1-4][0-9]|05[0-4])_.+\.md$/u.test(name)).sort().map((name) => path.join(notesDir, name));
}
...
test('IT strategy notes 001 through 054 use Mermaid instead of legacy visual markup', async () => {
  const files = await targetNotes();
  assert.equal(files.length, 46, '현재 카탈로그의 001~054 범위에는 46개 노트가 있어야 합니다.');
