...
async function targetNotes() {
  ...
  return names.filter((name) => /^(?:00[1-9]|0[1-4][0-9]|05[0-4]|05[8-9]|06[0-9]|07[0-5])_.+\.md$/u.test(name)).sort().map((name) => path.join(notesDir, name));
}
...
test('IT strategy notes 001 through 075 use Mermaid instead of legacy visual markup', async () => {
  const files = await targetNotes();
  assert.equal(files.length, 58, '현재 카탈로그의 001~075 범위에는 58개 노트가 있어야 합니다.');
